...
int GetHttpPort(void *data, unsigned int len)
{
	...
	printf("SemWait(hndlDrvSem); Begin\n");
	SemWait(hndlDrvSem);
	printf("SemWait(hndlDrvSem); Done\n");
	msgsnd( gSysMsgId,&msgbuf,sizeof(msgbuf)-sizeof(long),0);/*send msg1*/
	printf("msgsnd Done\n");
	msgrcv( gSysMsgId, &msgbuf,sizeof(msgbuf)-sizeof(long), gSysProcId, 0);
	printf("msgrcv Done\n");
	length = len;
	printf("GetHttpPort Done\n");
	if(length != msgbuf.length)
		ret = -1;
	printf("GetHttpPort ret=%d\n",ret);
	if(ret == 0)
		ShareMemRead(msgbuf.offset, data, length);
	SemRelease(hndlDrvSem);
	return ret;
...
* @return message buffer length.
*/
int SetTime(void *data, unsigned int len)
{
	SYS_MSG_BUF msgbuf;
	memset(&msgbuf,0,sizeof(msgbuf));
	msgbuf.Des = MSG_TYPE_MSG1;
	msgbuf.src = gSysProcId;
	msgbuf.cmd = SYS_MSG_SET_TIME;
	msgbuf.length = len;
	msgbuf.offset = (gSysProcId - MSG_TYPE_MSG1 - 1) * PROC_MEM_SIZE;
	SemWait(hndlDrvSem);
	ShareMemWrite(msgbuf.offset, data, len);
...
int SetSysGBCE(void *data, unsigned int len)
{
	SYS_MSG_BUF msgbuf;
	memset(&msgbuf,0,sizeof(msgbuf));
	msgbuf.Des = MSG_TYPE_MSG1;
	msgbuf.src = gSysProcId;
	msgbuf.cmd = SYS_MSG_SET_GBCE;
	msgbuf.length = len;
	msgbuf.offset = (gSysProcId - MSG_TYPE_MSG1 - 1) * PROC_MEM_SIZE;
	SemWait(hndlDrvSem);
	ShareMemWrite(msgbuf.offset, data, len);
...
/**
* @brief set Video Codec Mode selection.

* @param data [I ] pointer to input data.
* @param len [I ] data length.
* @return message buffer length.
*/
int SetSysVideoCodecMode(void *data, unsigned int len)
{
	SYS_MSG_BUF msgbuf;
	memset(&msgbuf,0,sizeof(msgbuf));
	msgbuf.Des = MSG_TYPE_MSG1;
	msgbuf.src = gSysProcId;
	msgbuf.cmd = SYS_MSG_SET_VIDEOCODEC_MODE;
	msgbuf.length = len;
	msgbuf.offset = (gSysProcId - MSG_TYPE_MSG1 - 1) * PROC_MEM_SIZE;
	SemWait(hndlDrvSem);
	ShareMemWrite(msgbuf.offset, data, len);
...
* @return message buffer length.
*/
int SetSysTVcable(void *data, unsigned int len)
{
	SYS_MSG_BUF msgbuf;
	memset(&msgbuf,0,sizeof(msgbuf));
	msgbuf.Des = MSG_TYPE_MSG1;
	msgbuf.src = gSysProcId;
	msgbuf.cmd = SYS_MSG_SET_TVCABLE;
	msgbuf.length = len;
	msgbuf.offset = (gSysProcId - MSG_TYPE_MSG1 - 1) * PROC_MEM_SIZE;
	SemWait(hndlDrvSem);
	ShareMemWrite(msgbuf.offset, data, len);
...
int SetIpncPtz(void *data, unsigned int len)
{
	SYS_MSG_BUF msgbuf;
        ...
        printf("^^^^^^^^^^^^^^^SetIpncPtz Begin!!!\n");
        #if DEBUG==1023
        {
            int i;
            char *p=data;

            printf("SetIpncPtz data: ");
            for(i=0;i<len;i++)
            {
                printf("%x ",p[i]);
            }
	...
	msgbuf.offset = (gSysProcId - MSG_TYPE_MSG1 - 1) * PROC_MEM_SIZE;
        ...
        #if DEBUG==1023
        printf("SetIpncPtz offset=%d, length=%d\n",msgbuf.offset,msgbuf.length);
        #endif
...
int InitIpncPtz(void *data, unsigned int len)
{
	...
	msgrcv( gSysMsgId, &msgbuf,sizeof(msgbuf)-sizeof(long), gSysProcId, 0);
	SemRelease(hndlDrvSem);
	printf("^^^^^^^^^^^^^^^InitIpncPtz Begin!!!msgbuf.length=%d\n",msgbuf.length);
	return msgbuf.length;
}

int EnalbeRecCh(void *data, unsigned int len)
{
	printf("^^^^^^^^^^^^^^^EnalbeRecCh Begin!!!\n");
	SYS_MSG_BUF msgbuf;
	memset(&msgbuf,0,sizeof(msgbuf));
	msgbuf.Des = MSG_TYPE_MSG1;
	msgbuf.src = gSysProcId;
	msgbuf.cmd = SYS_MSG_ENALBE_RECCH;
	msgbuf.length = len;
	msgbuf.offset = (gSysProcId - MSG_TYPE_MSG1 - 1) * PROC_MEM_SIZE;
	SemWait(hndlDrvSem);
	ShareMemWrite(msgbuf.offset, data, len);
	...
	msgrcv( gSysMsgId, &msgbuf,sizeof(msgbuf)-sizeof(long), gSysProcId, 0);
	SemRelease(hndlDrvSem);
	printf("^^^^^^^^^^^^^^^EnalbeRecCh Done!!!msgbuf.length=%d\n",msgbuf.length);
	return msgbuf.length;
}
int GetPbRecDayData(void *data, unsigned int len)
{
	printf("^^^^^^^^^^^^^^^GetPbRecDayData Begin!!!\n");
	SYS_MSG_BUF msgbuf;
	int length;
	memset(&msgbuf,0,sizeof(msgbuf));
	msgbuf.Des = MSG_TYPE_MSG1;
	msgbuf.src = gSysProcId;
	msgbuf.cmd = SYS_MSG_GET_PBRECDAY;
	msgbuf.length = len;
	msgbuf.offset = (gSysProcId - MSG_TYPE_MSG1 - 1) * PROC_MEM_SIZE;
	SemWait(hndlDrvSem);
	ShareMemWrite(msgbuf.offset, data, len);
	...
	length = len;
	printf("^^^^^^^^^^^^^^^data:%s len:%d,msgbuf.length:%d!!!\n",(unsigned char*)data,len,msgbuf.length);
	if(length > msgbuf.length)
		length = msgbuf.length;
	if(length > 0)
		ShareMemRead(msgbuf.offset, data, length);
	...
	printf("^^^^^^^^^^^^^^^GetPbRecDayData Done!!!msgbuf.length=%d\n",msgbuf.length);
	return msgbuf.length;
}
int GetPbRecHourData(void *data, unsigned int len)
{
	printf("^^^^^^^^^^^^^^^GetPbRecHourData Begin!!!\n");
	SYS_MSG_BUF msgbuf;
	int length;
	memset(&msgbuf,0,sizeof(msgbuf));
	msgbuf.Des = MSG_TYPE_MSG1;
	msgbuf.src = gSysProcId;
	msgbuf.cmd = SYS_MSG_GET_PBRECHOUR;
	msgbuf.length = len;
	msgbuf.offset = (gSysProcId - MSG_TYPE_MSG1 - 1) * PROC_MEM_SIZE;
	SemWait(hndlDrvSem);
	ShareMemWrite(msgbuf.offset, data, len);
	...
	if(length > 0)
		ShareMemRead(msgbuf.offset, data, length);
	SemRelease(hndlDrvSem);
	printf("^^^^^^^^^^^^^^^GetPbRecHourData Done!!!msgbuf.length=%d\n",msgbuf.length);
	return msgbuf.length;
}
int StartPlaybackCh(void *data, unsigned int len)
{
	SYS_MSG_BUF msgbuf;
	memset(&msgbuf,0,sizeof(msgbuf));
	msgbuf.Des = MSG_TYPE_MSG1;
	msgbuf.src = gSysProcId;
	msgbuf.cmd = SYS_MSG_START_PLAYBACKCH;
	msgbuf.length = len;
	msgbuf.offset = (gSysProcId - MSG_TYPE_MSG1 - 1) * PROC_MEM_SIZE;
	SemWait(hndlDrvSem);
	ShareMemWrite(msgbuf.offset, data, len);
...
int StopPlaybackCh(void *data, unsigned int len)
{
	SYS_MSG_BUF msgbuf;
	memset(&msgbuf,0,sizeof(msgbuf));
	msgbuf.Des = MSG_TYPE_MSG1;
	msgbuf.src = gSysProcId;
	msgbuf.cmd = SYS_MSG_STOP_PLAYBACKCH;
	msgbuf.length = len;
	msgbuf.offset = (gSysProcId - MSG_TYPE_MSG1 - 1) * PROC_MEM_SIZE;
	SemWait(hndlDrvSem);
	ShareMemWrite(msgbuf.offset, data, len);
...
int PausePlaybackCh(void *data, unsigned int len)
{
	SYS_MSG_BUF msgbuf;
	memset(&msgbuf,0,sizeof(msgbuf));
	msgbuf.Des = MSG_TYPE_MSG1;
	msgbuf.src = gSysProcId;
	msgbuf.cmd = SYS_MSG_PAUSE_PLAYBACKCH;
	msgbuf.length = len;
	msgbuf.offset = (gSysProcId - MSG_TYPE_MSG1 - 1) * PROC_MEM_SIZE;
	SemWait(hndlDrvSem);

        #if DEBUG==0
	printf("PausePlaybackCh data=%s  len=%d\n",(char *)data,len);
        #endif
...
int ResumePlaybackCh(void *data, unsigned int len)
{
	SYS_MSG_BUF msgbuf;
	memset(&msgbuf,0,sizeof(msgbuf));
	msgbuf.Des = MSG_TYPE_MSG1;
	msgbuf.src = gSysProcId;
	msgbuf.cmd = SYS_MSG_RESUME_PLAYBACKCH;
	msgbuf.length = len;
	msgbuf.offset = (gSysProcId - MSG_TYPE_MSG1 - 1) * PROC_MEM_SIZE;
	SemWait(hndlDrvSem);
	ShareMemWrite(msgbuf.offset, data, len);
	...
	SemRelease(hndlDrvSem);
        ...
        #if DEBUG==0
	printf("ResumePlaybackCh data=%s  len=%d\n",(char *)data,len);
        #endif
