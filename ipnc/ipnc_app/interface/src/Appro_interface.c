...
int GetAVData( unsigned int field, int serial, AV_DATA * ptr )
{
	...
	switch(field){
		...
		case AV_OP_GET_MPEG4_1_SERIAL:
			...
			break;
		case AV_OP_GET_MPEG4_2_SERIAL:
			if(serial != -1){
				ret = RET_INVALID_PRM;
			} else {
				FrameInfo_t curframe = GetCurrentFrame(FMT_MPEG4_2);
				//printf("*******AV_OP_GET_MPEG4_SERIAL******* field: %d\n",curframe.serial_no);
				if(curframe.serial_no < 0){
					ret = RET_NO_VALID_DATA;
				} else {
			...
			break;
		case AV_OP_GET_MPEG4_3_SERIAL:
			if(serial != -1){
				ret = RET_INVALID_PRM;
			} else {
				FrameInfo_t curframe = GetCurrentFrame(FMT_MPEG4_3);
				//printf("*******AV_OP_GET_MPEG4_SERIAL******* field: %d\n",curframe.serial_no);
				if(curframe.serial_no < 0){
					ret = RET_NO_VALID_DATA;
				} else {
		...
		case AV_OP_WAIT_NEW_MPEG4_SERIAL:
			...
			break;
		case AV_OP_WAIT_NEW_MPEG4_1_SERIAL:
			if(serial != -1){
				ret = RET_INVALID_PRM;
			} else {
				FrameInfo_t curframe = WaitNewFrame(FMT_MPEG4_1);
				if(curframe.serial_no < 0){
					ret = RET_NO_VALID_DATA;
				} else {
					ptr->serial = curframe.serial_no;
			...
			break;
		case AV_OP_WAIT_NEW_MPEG4_2_SERIAL:
			if(serial != -1){
				ret = RET_INVALID_PRM;
			} else {
				FrameInfo_t curframe = WaitNewFrame(FMT_MPEG4_2);
				if(curframe.serial_no < 0){
					ret = RET_NO_VALID_DATA;
				} else {
					ptr->serial = curframe.serial_no;
			...
			break;
		case AV_OP_WAIT_NEW_MPEG4_3_SERIAL:
			if(serial != -1){
				ret = RET_INVALID_PRM;
			} else {
				FrameInfo_t curframe = WaitNewFrame(FMT_MPEG4_3);
				if(curframe.serial_no < 0){
					ret = RET_NO_VALID_DATA;
				} else {
					ptr->serial = curframe.serial_no;
		...
		case AV_OP_WAIT_NEW_MPEG4_CIF_SERIAL:
			...
			break;
		case AV_OP_WAIT_NEW_MPEG4_1_CIF_SERIAL:
			if(serial != -1){
				ret = RET_INVALID_PRM;
			} else {
				FrameInfo_t curframe = WaitNewFrame(FMT_MPEG4_1);
			...
			break;
		case AV_OP_WAIT_NEW_MPEG4_2_CIF_SERIAL:
			if(serial != -1){
				ret = RET_INVALID_PRM;
			} else {
				FrameInfo_t curframe = WaitNewFrame(FMT_MPEG4_2);
			...
			break;
		case AV_OP_WAIT_NEW_MPEG4_3_CIF_SERIAL:
			if(serial != -1){
				ret = RET_INVALID_PRM;
			} else {
				FrameInfo_t curframe = WaitNewFrame(FMT_MPEG4_3);
		...
		case AV_OP_LOCK_MP4_1_VOL:
		{
			FrameInfo_t frame;
			ret = GetVolInfo(&frame,FMT_MPEG4_1);
			if( ret > 0 )
			{
				memcpy( VolInfo,((unsigned char *)virptr + frame.offset),ret);
			}
		...
		case AV_OP_LOCK_MP4_2_VOL:
		{
			FrameInfo_t frame;
			ret = GetVolInfo(&frame,FMT_MPEG4_2);
			if( ret > 0 )
			{
				memcpy( VolInfo,((unsigned char *)virptr + frame.offset),ret);
			}
		...
		case AV_OP_LOCK_MP4_3_VOL:
		{
			FrameInfo_t frame;
			ret = GetVolInfo(&frame,FMT_MPEG4_3);
			if( ret > 0 )
			{
				memcpy( VolInfo,((unsigned char *)virptr + frame.offset),ret);
			}
		...
		case AV_OP_LOCK_MP4_1_IFRAME:
		{
			FrameInfo_t curframe;
			LockMpeg4IFrame(&curframe, FMT_MPEG4_1);
			if(curframe.serial_no == -1){
				ret = RET_NO_VALID_DATA;
			} else if(curframe.serial_no == -2){
				ret = RET_NO_MEM;
		...
		case AV_OP_LOCK_MP4_2_IFRAME:
		{
			FrameInfo_t curframe;
			LockMpeg4IFrame(&curframe, FMT_MPEG4_2);
			if(curframe.serial_no == -1){
				ret = RET_NO_VALID_DATA;
			} else if(curframe.serial_no == -2){
				ret = RET_NO_MEM;
		...
		case AV_OP_LOCK_MP4_3_IFRAME:
		{
			FrameInfo_t curframe;
			LockMpeg4IFrame(&curframe, FMT_MPEG4_3);
			if(curframe.serial_no == -1){
				ret = RET_NO_VALID_DATA;
			} else if(curframe.serial_no == -2){
				ret = RET_NO_MEM;
