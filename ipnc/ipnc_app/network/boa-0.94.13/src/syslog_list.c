...
char html_content2[100];
char html_end2[] = "<HR>HTTP Server</A><BR></PRE></BODY></HTML>";
char html_empty2[] = "NO FILE MATCH *.AVI *.JPG *.YUV";
...
int SysLog_List_To_html(void *pOutMem, int MaxSize)
{
	...
	strncpy( pOutMem, html_header2, MaxSize );
        ...
        if(check_magic_num(fp) == SUCCESS)
	    {
            while(count < NUM_LOG_PER_PAGE * MAX_LOG_PAGE_NUM)
		    {
			   ...
			   if(strstr(log.event, "login") == NULL)
			   {
			        ...
			        sprintf(html_content2,"A(\"%d\",\"%s\",\"%s\");", ctr++, timeStr, log.event);
	                if( MaxSize > strlen(pOutMem) )
		                strncat(pOutMem, html_content2, MaxSize-strlen(pOutMem));
			        else
			        {
