...
void set_ipncptz(request *req, COMMAND_ARGUMENT *argm)
{
	...
	do {
		...
		#if DEBUG==1023
		printf("~set_ipncptz Done!!!argm->value=%s\n",argm->value);
        	#endif
...
void init_ipncptz(request *req, COMMAND_ARGUMENT *argm)
{
	...
	do {
		...
		#if DEBUG==1
		printf("~init_ipncptz:argm->value=%s,argm->name=%s!!!\n",argm->value,argm->name);
		printf("~init_ipncptz Done!!!\n");
		#endif
...
#if 1
		/* hack need to have a proper fix */
		{
			int alarmstatus = (pSysInfo->lan_config.alarmstatus>>1)&0x1C;
			alarmstatus |= (pSysInfo->lan_config.alarmstatus&3);
			req->buffer_end += sprintf(req_bufptr(req), OPTION_OK "%s=%05x\n", argm->name, alarmstatus);
		}
#else
		req->buffer_end += sprintf(req_bufptr(req), OPTION_OK "%s=%05x\n", argm->name, pSysInfo->lan_config.alarmstatus);
#endif
        if(pSysInfo->lan_config.alarmstatus&7) /* first 3 bits indicate alarm status */
        {
			alarm_status = 1;
        }else{
            if(alarm_status == 1)
 ...
 *                   //add by sxh                                         *
 ***************************************************************************/
 void enablerecordch(request *req, COMMAND_ARGUMENT *argm)
{
	#ifdef VANLINK_DVR_CONFIG_DEBUG
	printf("~enablerecordch Begin!!!\n");
	#endif
	unsigned char value = atoi(argm->value);
	do {
		if(ControlSystemData(SFIELD_ENABLE_RECCH, &value, sizeof(value)) < 0)
			break;
		req->buffer_end += sprintf(req_bufptr(req), OPTION_OK "%s\n", argm->name);
		return;
	} while (0);
 ...
 void get_pbrecdaydata(request *req, COMMAND_ARGUMENT *argm)
{
	#ifdef VANLINK_DVR_CONFIG_DEBUG
	printf("~get_pbrecdaydata Begin!!!\n");
	#endif
	unsigned char recDayData[32];
	bzero(recDayData,32);
	memcpy(recDayData,argm->value,7);
	printf("get_pbrecdaydata:recDayData=%s strlen(recDayData)=%d\n",recDayData,strlen(recDayData));
	do {
		if(ControlSystemData(SFIELD_GET_PBRECDAY, (void *)recDayData, strlen(recDayData)) < 0)
			break;
		req->buffer_end += sprintf(req_bufptr(req), OPTION_OK "%s=%s\n", argm->name,recDayData);
		return;
	} while (0);
	req->buffer_end += sprintf(req_bufptr(req), OPTION_NG "%s\n", argm->name);
}
  void get_pbrechourdata(request *req, COMMAND_ARGUMENT *argm)
{
	#ifdef VANLINK_DVR_CONFIG_DEBUG
	printf("~get_pbrechourdata Begin!!!\n");
	#endif
	unsigned char recHourData[24*60+1];
	bzero(recHourData,24*60+1);
	memcpy(recHourData,argm->value,12);
	printf("get_pbrecdaydata:recDayData=%s strlen(recDayData)=%d\n",recHourData,strlen(recHourData));
	do {
		if(ControlSystemData(SFIELD_GET_PBRECHOUR, (void *)recHourData, sizeof(recHourData)) < 0)
			break;
		req->buffer_end += sprintf(req_bufptr(req), OPTION_OK "%s=%s\n", argm->name,recHourData);
		return;
	} while (0);
	req->buffer_end += sprintf(req_bufptr(req), OPTION_NG "%s\n", argm->name);
}
 void start_playbackch(request *req, COMMAND_ARGUMENT *argm)
{
#ifdef VANLINK_DVR_CONFIG_DEBUG
  printf("~start_playbackch Begin!!!\n");
#endif
  
  do {
	  if(ControlSystemData(SFIELD_START_PLAYBACKCH, (void *)argm->value, strlen(argm->value)) < 0)
		  break;
	  req->buffer_end += sprintf(req_bufptr(req), OPTION_OK "%s\n", argm->name);
	  return;
  } while (0);
  ...
  void stop_playbackch(request *req, COMMAND_ARGUMENT *argm)
 {
#ifdef VANLINK_DVR_CONFIG_DEBUG
   printf("~stop_playbackch Begin!!!%s   %d\n",argm->value,strlen(argm->value));
#endif
   
   do {
	   if(ControlSystemData(SFIELD_STOP_PLAYBACKCH, (void *)argm->value, strlen(argm->value)) < 0)
		   break;
	   req->buffer_end += sprintf(req_bufptr(req), OPTION_OK "%s\n", argm->name);
	   return;
   } while (0);
  ...
  void pause_playbackch(request *req, COMMAND_ARGUMENT *argm)
{
#ifdef VANLINK_DVR_CONFIG_DEBUG
  printf("~pause_playbackch Begin!!!%s   %d\n",argm->value,strlen(argm->value));
#endif
  
  do {
	  if(ControlSystemData(SFIELD_PAUSE_PLAYBACKCH, (void *)argm->value, strlen(argm->value)) < 0)
		  break;
	  req->buffer_end += sprintf(req_bufptr(req), OPTION_OK "%s\n", argm->name);
	  return;
  } while (0);
  req->buffer_end += sprintf(req_bufptr(req), OPTION_NG "%s\n", argm->name);
   ...
   void resume_playbackch(request *req, COMMAND_ARGUMENT *argm)
  {
#ifdef VANLINK_DVR_CONFIG_DEBUG
	printf("~resume_playbackch Begin!!!%s   %d\n",argm->value,strlen(argm->value));
#endif
	
	do {
		if(ControlSystemData(SFIELD_RESUME_PLAYBACKCH, (void *)argm->value, strlen(argm->value)) < 0)
			break;
		req->buffer_end += sprintf(req_bufptr(req), OPTION_OK "%s\n", argm->name);
		return;
	} while (0);
...
void checkpassword(request *req, COMMAND_ARGUMENT *argm)
{
	...
	#ifdef VANLINK_DVR_CONFIG_DEBUG
	printf("~~~~~~~checkpassword:argm->value=%s\n",argm->value);
	#endif
	do {
		...
		*user_name++ = '\0';
		#ifdef VANLINK_DVR_CONFIG_DEBUG
		printf("~~~~~~~checkpassword:user_name=%s\n",user_name);
		#endif
		user_no = atoi(argm->value);
		#ifdef VANLINK_DVR_CONFIG_DEBUG
		printf("~~~~~~~checkpassword:user_no=%d\n",user_no);
		#endif
		//*--user_name = '.';
		...
		*user_passwd++ = '\0';
		#ifdef VANLINK_DVR_CONFIG_DEBUG
		printf("~~~~~~~checkpassword:user_passwd=%s\n",user_passwd);
		#endif
...
void set_videocodecmode(request *req, COMMAND_ARGUMENT *argm)
{
	...
	do {
		ControlSystemData(SFIELD_SET_VIDEOCODECMODE, (void *)&value, sizeof(value));
		req->buffer_end += sprintf(req_bufptr(req), OPTION_OK "%s\n", argm->name);
		return;
	} while (0);
	req->buffer_end += sprintf(req_bufptr(req), OPTION_NG "%s\n", argm->name);
...
{
	// add by sxh    not support it for timebeing
	req->buffer_end += sprintf(req_bufptr(req), OPTION_OK "%s\n", argm->name);
	return;
	// add by sxh    not support it for timebeing
	__u8 value = atoi(argm->value);
...
void set_gioouttype(request *req, COMMAND_ARGUMENT *argm)
{
 ...
 void set_ptz(request *req, COMMAND_ARGUMENT *argm)
{

	do {
		ControlSystemData(SFIELD_SET_PTZ, (void *)argm->value, strlen(argm->value));
		req->buffer_end += sprintf(req_bufptr(req), OPTION_OK "%s\n", argm->name);
		return;
	} while (0);
	req->buffer_end += sprintf(req_bufptr(req), OPTION_NG "%s\n", argm->name);
...
void get_ptzaddr(request *req, COMMAND_ARGUMENT *argm)
{
    do
    {
        int ptzCh=atoi(strtok(argm->value,"@"));
        ...
        req->buffer_end += sprintf(req_bufptr(req), OPTION_OK "%s=%d\n", argm->name, pSysInfo->ptz_channel[ptzCh].devaddr);
...
void get_ptzprotocol(request *req, COMMAND_ARGUMENT *argm)
{
    do
    {
        int ptzCh=atoi(strtok(argm->value,"@"));
        ...
        req->buffer_end += sprintf(req_bufptr(req), OPTION_OK "%s=%d\n", argm->name, pSysInfo->ptz_channel[ptzCh].protocol);
...
void get_ptzport(request *req, COMMAND_ARGUMENT *argm)
{
    do
    {
        int ptzCh=atoi(strtok(argm->value,"@"));
        ...
        req->buffer_end += sprintf(req_bufptr(req), OPTION_OK "%s=%d\n", argm->name, pSysInfo->ptz_channel[ptzCh].dev_no);
...
void get_ptzspeed(request *req, COMMAND_ARGUMENT *argm)
{
    #if DEBUG==1
    printf("get_ptzspeed argm->value=%s\n",argm->value);
    #endif
    ...
    do
    {
        ...
        req->buffer_end += sprintf(req_bufptr(req), OPTION_OK "%s=%d\n", argm->name, pSysInfo->ptz_channel[ptzCh].speed);

        #if DEBUG==1
        printf(OPTION_OK "%s=%d\n", argm->name, pSysInfo->ptz_channel[ptzCh].speed);
        #endif
...
void get_ptzdatabit(request *req, COMMAND_ARGUMENT *argm)
{
    do
    {
        int ptzCh=atoi(strtok(argm->value,"@"));
        ...
        req->buffer_end += sprintf(req_bufptr(req), OPTION_OK "%s=%d\n", argm->name, pSysInfo->ptz_channel[ptzCh].databit);
...
void get_ptzstopbit(request *req, COMMAND_ARGUMENT *argm)
{
    do
    {
        int ptzCh=atoi(strtok(argm->value,"@"));
        ...
        req->buffer_end += sprintf(req_bufptr(req), OPTION_OK "%s=%d\n", argm->name, pSysInfo->ptz_channel[ptzCh].stopbit);
...
void get_ptzparity(request *req, COMMAND_ARGUMENT *argm)
{
    do
    {
        int ptzCh=atoi(strtok(argm->value,"@"));
        ...
        req->buffer_end += sprintf(req_bufptr(req), OPTION_OK "%s=%d\n", argm->name, pSysInfo->ptz_channel[ptzCh].parity);
...
void get_ptzstreamctrl(request *req, COMMAND_ARGUMENT *argm)
{
    do
    {
        int ptzCh=atoi(strtok(argm->value,"@"));
 ...
 void set_rs485(request *req, COMMAND_ARGUMENT *argm)
{
	...
	do {
		ControlSystemData(SFIELD_SET_RS485, (void *)argm->value, strlen(argm->value));
		req->buffer_end += sprintf(req_bufptr(req), OPTION_OK "%s\n", argm->name);
		return;
	} while (0);
	req->buffer_end += sprintf(req_bufptr(req), OPTION_NG "%s\n", argm->name);
}
 void get_rs485dev_num(request *req, COMMAND_ARGUMENT *argm)
{
	__u8 value = atoi(argm->value);
	do {
		SysInfo* pSysInfo = GetSysInfo();
		...
		if(value>=pSysInfo->rs485num)
			 break;
		req->buffer_end += sprintf(req_bufptr(req), OPTION_OK "%s=%d\n", argm->name, pSysInfo->rs485devs[value]);
		return;
	} while (0);
	req->buffer_end += sprintf(req_bufptr(req), OPTION_NG "%s\n", argm->name);
}
 void set_rs485dev_add(request *req, COMMAND_ARGUMENT *argm)
 {
	 __u8 value = atoi(argm->value);
	 do {
		 ControlSystemData(SFIELD_SET_RS485_DEV_ADD, (void *)&value, sizeof(value));
		 req->buffer_end += sprintf(req_bufptr(req), OPTION_OK "%s\n", argm->name);
		 return;
	 } while (0);
	 req->buffer_end += sprintf(req_bufptr(req), OPTION_NG "%s\n", argm->name);
 }
 void set_rs485dev_del(request *req, COMMAND_ARGUMENT *argm)
 {
	 __u8 value = atoi(argm->value);
 
	 do {
		 ControlSystemData(SFIELD_SET_RS485_DEV_DEL, (void *)value, sizeof(value));
		 req->buffer_end += sprintf(req_bufptr(req), OPTION_OK "%s\n", argm->name);
		 return;
	 } while (0);
	 req->buffer_end += sprintf(req_bufptr(req), OPTION_NG "%s\n", argm->name);
...
void set_rs232(request *req, COMMAND_ARGUMENT *argm)
{
	do {
		ControlSystemData(SFIELD_SET_RS232, (void *)argm->value, strlen(argm->value));
		req->buffer_end += sprintf(req_bufptr(req), OPTION_OK "%s\n", argm->name);
		return;
	} while (0);
	req->buffer_end += sprintf(req_bufptr(req), OPTION_NG "%s\n", argm->name);
...
void set_rs232speed(request *req, COMMAND_ARGUMENT *argm)
{
 do {
	 ControlSystemData(SFIELD_SET_RS232SPEED, (void *)argm->value, strlen(argm->value));
	 req->buffer_end += sprintf(req_bufptr(req), OPTION_OK "%s\n", argm->name);
	 return;
 } while (0);
 req->buffer_end += sprintf(req_bufptr(req), OPTION_NG "%s\n", argm->name);
...
void set_rs232databit(request *req, COMMAND_ARGUMENT *argm)
{
 __u8 value = atoi(argm->value);
 do {

	 ControlSystemData(SFIELD_SET_RS232DATABIT, (void *)argm->value, strlen(argm->value));
	 req->buffer_end += sprintf(req_bufptr(req), OPTION_OK "%s\n", argm->name);
	 return;
 } while (0);
 req->buffer_end += sprintf(req_bufptr(req), OPTION_NG "%s\n", argm->name);
...
void set_rs232stopbit(request *req, COMMAND_ARGUMENT *argm)
{
 __u8 value = atoi(argm->value);
 do {

	 ControlSystemData(SFIELD_SET_RS232STOPBIT, (void *)argm->value, strlen(argm->value));
	 req->buffer_end += sprintf(req_bufptr(req), OPTION_OK "%s\n", argm->name);
	 return;
 } while (0);
 req->buffer_end += sprintf(req_bufptr(req), OPTION_NG "%s\n", argm->name);
...
void set_rs232parity(request *req, COMMAND_ARGUMENT *argm)
{
 __u8 value = atoi(argm->value);
 do {

	 ControlSystemData(SFIELD_SET_RS232PARITY, (void *)argm->value, strlen(argm->value));
	 req->buffer_end += sprintf(req_bufptr(req), OPTION_OK "%s\n", argm->name);
	 return;
 } while (0);
 req->buffer_end += sprintf(req_bufptr(req), OPTION_NG "%s\n", argm->name);
...
void set_rs232streamctrl(request *req, COMMAND_ARGUMENT *argm)
{
 __u8 value = atoi(argm->value);
 do {

	 ControlSystemData(SFIELD_SET_RS232STREAMCTRL, (void *)argm->value, strlen(argm->value));
	 req->buffer_end += sprintf(req_bufptr(req), OPTION_OK "%s\n", argm->name);
	 return;
 } while (0);
 req->buffer_end += sprintf(req_bufptr(req), OPTION_NG "%s\n", argm->name);
...
void get_rs485speed(request *req, COMMAND_ARGUMENT *argm)
{
	do {
		int rs485port=atoi(strtok(argm->value,"@"));
		int rs485Ch=atoi(strtok(NULL,"@"));
		SysInfo* pSysInfo = GetSysInfo();
	  	if(pSysInfo == NULL)
	 		 break;
		req->buffer_end += sprintf(req_bufptr(req), OPTION_OK "%s=%d\n", argm->name, pSysInfo->rs485_config[rs485port][rs485Ch].speed);
		return;
	} while (0);
	req->buffer_end += sprintf(req_bufptr(req), OPTION_NG "%s\n", argm->name);
}
void get_rs485databit(request *req, COMMAND_ARGUMENT *argm)
{
	do {
		int rs485port=atoi(strtok(argm->value,"@"));
		int rs485Ch=atoi(strtok(NULL,"@"));
		SysInfo* pSysInfo = GetSysInfo();
	  	if(pSysInfo == NULL)
	 		 break;
		req->buffer_end += sprintf(req_bufptr(req), OPTION_OK "%s=%d\n", argm->name, pSysInfo->rs485_config[rs485port][rs485Ch].databit);
		return;
	} while (0);
	req->buffer_end += sprintf(req_bufptr(req), OPTION_NG "%s\n", argm->name);
}
void get_rs485stopbit(request *req, COMMAND_ARGUMENT *argm)
{
	do {
			int rs485port=atoi(strtok(argm->value,"@"));
			int rs485Ch=atoi(strtok(NULL,"@"));
			...
			if(pSysInfo == NULL)
				 break;
			req->buffer_end += sprintf(req_bufptr(req), OPTION_OK "%s=%d\n", argm->name, pSysInfo->rs485_config[rs485port][rs485Ch].stopbit);
			return;
		} while (0);
		req->buffer_end += sprintf(req_bufptr(req), OPTION_NG "%s\n", argm->name);

}
void get_rs485parity(request *req, COMMAND_ARGUMENT *argm)
{
	do {
			int rs485port=atoi(strtok(argm->value,"@"));
			int rs485Ch=atoi(strtok(NULL,"@"));
			...
			if(pSysInfo == NULL)
				 break;
			req->buffer_end += sprintf(req_bufptr(req), OPTION_OK "%s=%d\n", argm->name, pSysInfo->rs485_config[rs485port][rs485Ch].parity);
			return;
		} while (0);
		req->buffer_end += sprintf(req_bufptr(req), OPTION_NG "%s\n", argm->name);

}
void get_rs485streamctrl(request *req, COMMAND_ARGUMENT *argm)
{
	do {
			int rs485port=atoi(strtok(argm->value,"@"));
			int rs485Ch=atoi(strtok(NULL,"@"));
			...
			if(pSysInfo == NULL)
				 break;
			req->buffer_end += sprintf(req_bufptr(req), OPTION_OK "%s=%d\n", argm->name, pSysInfo->rs485_config[rs485port][rs485Ch].streamctrl);
			return;
		} while (0);
		req->buffer_end += sprintf(req_bufptr(req), OPTION_NG "%s\n", argm->name);

}
void get_rs485devaddr(request *req, COMMAND_ARGUMENT *argm)
{
	do {
			int rs485port=atoi(strtok(argm->value,"@"));
			int rs485Ch=atoi(strtok(NULL,"@"));
			...
			if(pSysInfo == NULL)
				 break;
			req->buffer_end += sprintf(req_bufptr(req), OPTION_OK "%s=%d\n", argm->name, pSysInfo->rs485_config[rs485port][rs485Ch].devaddr);
			return;
		} while (0);
		req->buffer_end += sprintf(req_bufptr(req), OPTION_NG "%s\n", argm->name);
...
void get_rs232speed(request *req, COMMAND_ARGUMENT *argm)
{
	do {
		 int rs232_num=atoi(argm->value);
		 //printf("rs232_num=%d",rs232_num);
		 SysInfo* pSysInfo = GetSysInfo();
		 if(pSysInfo == NULL)
			 break;
		req->buffer_end += sprintf(req_bufptr(req), OPTION_OK "%s=%d\n", argm->name, pSysInfo->rs232_config[rs232_num].speed);
		//printf("pSysInfo->rs232_config[rs232_num].speed=%d",pSysInfo->rs232_config[rs232_num].speed);
		//printf("req->buffer_end=%d",req->buffer_end);
		return;
	} while (0);
	req->buffer_end += sprintf(req_bufptr(req), OPTION_NG "%s\n", argm->name);
}
void get_rs232databit(request *req, COMMAND_ARGUMENT *argm)
{
	do {
			 int rs232_num=atoi(argm->value);
			 SysInfo* pSysInfo = GetSysInfo();
			 if(pSysInfo == NULL)
				 break;
			req->buffer_end += sprintf(req_bufptr(req), OPTION_OK "%s=%d\n", argm->name, pSysInfo->rs232_config[rs232_num].databit);
			return;
		} while (0);
		req->buffer_end += sprintf(req_bufptr(req), OPTION_NG "%s\n", argm->name);

}
void get_rs232stopbit(request *req, COMMAND_ARGUMENT *argm)
{
	do {
			 int rs232_num=atoi(argm->value);
			 SysInfo* pSysInfo = GetSysInfo();
			 if(pSysInfo == NULL)
				 break;
			req->buffer_end += sprintf(req_bufptr(req), OPTION_OK "%s=%d\n", argm->name, pSysInfo->rs232_config[rs232_num].stopbit);
			return;
		} while (0);
		req->buffer_end += sprintf(req_bufptr(req), OPTION_NG "%s\n", argm->name);

}
void get_rs232parity(request *req, COMMAND_ARGUMENT *argm)
{
	do {
			 int rs232_num=atoi(argm->value);
			 SysInfo* pSysInfo = GetSysInfo();
			 if(pSysInfo == NULL)
				 break;
			req->buffer_end += sprintf(req_bufptr(req), OPTION_OK "%s=%d\n", argm->name, pSysInfo->rs232_config[rs232_num].parity);
			return;
		} while (0);
		req->buffer_end += sprintf(req_bufptr(req), OPTION_NG "%s\n", argm->name);

}
void get_rs232streamctrl(request *req, COMMAND_ARGUMENT *argm)
{
	do {
			 int rs232_num=atoi(argm->value);
			 SysInfo* pSysInfo = GetSysInfo();
			 if(pSysInfo == NULL)
				 break;
			req->buffer_end += sprintf(req_bufptr(req), OPTION_OK "%s=%d\n", argm->name, pSysInfo->rs232_config[rs232_num].streamctrl);
			return;
		} while (0);
		req->buffer_end += sprintf(req_bufptr(req), OPTION_NG "%s\n", argm->name);
}
 ...
 void set_plc_tcp(request *req, COMMAND_ARGUMENT *argm)
{
    #if DEBUG==1
    printf("set_plc_tcp\n");
    #endif
    ...
    do 
    {
        ControlSystemData(SFIELD_SET_PLC_TCP, (void *)argm->value, strlen(argm->value));
 ...
 void set_plc_serial(request *req, COMMAND_ARGUMENT *argm)
{
    #if DEBUG==1
    printf("set_plc_serial\n");
    #endif
    
    do 
    {
        ControlSystemData(SFIELD_SET_PLC_SERIAL, (void *)argm->value, strlen(argm->value));
...
static int get_plcTcp(request *req, COMMAND_ARGUMENT *argm)
{
    int plcChl=atoi(strtok(argm->value,"@"));

    #if DEBUG==1
    printf("get_pTcp plcChl=%d\n",plcChl);
    #endif
    ...
    if(plcChl>=MODBUSTCP_NUM)
    {
        return -1;
    }
    
    req->buffer_end += sprintf(req_bufptr(req), OPTION_OK "%s=%d@%d@%s@%d@%d@%d@%d@%d@%d@%d@%d\n", argm->name, 
                                                    plcChl, 
                                                    pSysInfo->data_source.md_tcp[plcChl].flag, 
                                                    pSysInfo->data_source.md_tcp[plcChl].ip_addr,
                                                    pSysInfo->data_source.md_tcp[plcChl].tcp_port,
                                                    pSysInfo->data_source.md_tcp[plcChl].poll_intval,
                                                    pSysInfo->data_source.md_tcp[plcChl].expire,
                                                    pSysInfo->data_source.md_tcp[plcChl].dev_id,
                                                    pSysInfo->data_source.md_tcp[plcChl].data_type,
                                                    pSysInfo->data_source.md_tcp[plcChl].plc_ind,
                                                    pSysInfo->data_source.md_tcp[plcChl].reg_cnt,
                                                    pSysInfo->data_source.md_tcp[plcChl].start_addr);

    #if DEeBUG==0
    printf(OPTION_OK "%s=%d@%d@%s@%d@%d@%d@%d@%d@%d@%d@%d\n", argm->name, 
                                                    plcChl,
                                                    pSysInfo->data_source.md_tcp[plcChl].flag, 
                                                    pSysInfo->data_source.md_tcp[plcChl].ip_addr,
                                                    pSysInfo->data_source.md_tcp[plcChl].tcp_port,
...
static int get_plcSerial(request *req, COMMAND_ARGUMENT *argm)
{
    int plcPort=atoi(strtok(argm->value,"@"));
    int plcChl=atoi(strtok(NULL,"@"));

    #if DEBUG==1
    printf("get_plcSerial plcPort=%d, plcChl=%d\n",plcPort,plcChl);
    #endif
    ...
    if(plcPort>=RS_PORT_NUM)
    {
        return FALSE;
    }
    
    if(plcChl>=CHANNEL_NUM)
    {
        return -1;
    }
    
    req->buffer_end += sprintf(req_bufptr(req), OPTION_OK "%s=%d@%d@%d@%d@%d@%d@%d@%d@%d@%d@%d@%d@%d@%d@%d@%d\n", 
                                                    argm->name, 
                                                    plcPort,
                                                    plcChl,
                                                    pSysInfo->data_source.md_rs[plcPort].protocol,
                                                    pSysInfo->data_source.md_rs[plcPort].poll_intval,
                                                    pSysInfo->data_source.md_rs[plcPort].expire,
                                                    pSysInfo->data_source.md_rs[plcPort].md[plcChl].flag,
                                                    pSysInfo->data_source.md_rs[plcPort].md[plcChl].dev_id,
                                                    pSysInfo->data_source.md_rs[plcPort].md[plcChl].data_type,
                                                    pSysInfo->data_source.md_rs[plcPort].md[plcChl].plc_ind,
                                                    pSysInfo->data_source.md_rs[plcPort].md[plcChl].reg_cnt,
                                                    pSysInfo->data_source.md_rs[plcPort].md[plcChl].start_addr,
                                                    pSysInfo->data_source.md_rs[plcPort].md[plcChl].speed,
                                                    pSysInfo->data_source.md_rs[plcPort].md[plcChl].databit,
                                                    pSysInfo->data_source.md_rs[plcPort].md[plcChl].stopbit,
                                                    pSysInfo->data_source.md_rs[plcPort].md[plcChl].parity,
                                                    pSysInfo->data_source.md_rs[plcPort].md[plcChl].streamctrl);

    #if DEBUG==1
    printf( OPTION_OK "%s=%d@%d@%d@%d@%d@%d@%d@%d@%d@%d@%d@%d@%d@%d@%d@%d\n", 
                                                    argm->name, 
                                                    plcPort,
                                                    plcChl,
                                                    pSysInfo->data_source.md_rs[plcPort].protocol,
                                                    ...
                                                    pSysInfo->data_source.md_rs[plcPort].md[plcChl].streamctrl);
    {
        ...
        p=&pSysInfo->data_source.md_rs[plcPort];

        printf("pSysInfo->data_source.md_rs[%d]: ",plcPort);
        for(i=0;i<sizeof(pSysInfo->data_source.md_rs[plcPort])/sizeof(int);i++)
        {
            printf("%d ",p[i]);
        }
        printf("\n");
    ...
    printf( "pcPort=%d, plcChl=%d, protocol=%d, pool_interval=%d, expire=%d, flag=%d, dev_id=%d, data_type=%d, plc_ind=%d, reg_cnt=%d, start_addr=%d, "
                "speed=%d, databit=%d, stopbit=%d, parity=%d, streanctrl=%d\n", 
                                                    plcPort,
                                                    plcChl,
                                                    pSysInfo->data_source.md_rs[plcPort].protocol,
                                                    pSysInfo->data_source.md_rs[plcPort].poll_intval,
...
void get_plc_tcp(request *req, COMMAND_ARGUMENT *argm)
{
    #if DEBUG==1
    printf("get_plc_tcp, value=%s\n",argm->value);
    #endif
    
    do
    {
        if(get_plcTcp(req,argm)==-1)
        {
            break;
        }
...
void get_plc_serial(request *req, COMMAND_ARGUMENT *argm)
{
    #if DEBUG==1
...
static int get_schedulechl(request *req, COMMAND_ARGUMENT *argm)
{
    ...
    #if DEBUG==1
    printf("get_storagerec videoChl=%d\n",videoChl);
    #endif
    SysInfo* pSysInfo = GetSysInfo();
 ...
 //  for(i = 0; i < SCHDULE_NUM; i ++){
			pSchedule = &(pSysInfo->lan_config.aSchedules[videoChl][index]);
			req->buffer_end += sprintf(req_bufptr(req),
				OPTION_OK "%02d%d%02d%02d%02d%02d%02d%02d%02d\n", index,
 ...
 void set_storage_rec(request *req, COMMAND_ARGUMENT *argm)
{
    ...
    do 
    {
        ControlSystemData(SFIELD_SET_STORAGE_REC, (void *)argm->value, strlen(argm->value));
...
static int get_storagerec(request *req, COMMAND_ARGUMENT *argm)
{
   ...
   req->buffer_end += sprintf(req_bufptr(req), OPTION_OK "%s=%d@%d@%d\n", argm->name, 
                                                    videoChl, 
                                                    pSysInfo->lan_config.nScheduleInfiniteEnable[videoChl], 
                                                    pSysInfo->lan_config.nScheduleRepeatEnable[videoChl]);

    #if DEBUG==0
    printf(OPTION_OK "%s=%d@%d@%s\n", argm->name, 
                                                    videoChl, 
                                                    pSysInfo->lan_config.nScheduleInfiniteEnable[videoChl], 
                                                    pSysInfo->lan_config.nScheduleRepeatEnable[videoChl]);
    #endif
...
void set_darkblankalarm(request *req, COMMAND_ARGUMENT *argm)
{
	unsigned char value = atoi(argm->value);
	do {
		if(ControlSystemData(SFIELD_SET_DARKBLANK, &value, sizeof(value)) < 0)
			break;
		req->buffer_end += sprintf(req_bufptr(req), OPTION_OK "%s\n", argm->name);
		return;
	} while (0);
...
void set_accessclearcmd(request *req, COMMAND_ARGUMENT *argm)
{
 ...
 //**************add by wbb ******************//
/****svae one preset note every time**************/
 void set_presetnote(request *req, COMMAND_ARGUMENT *argm)
{
    #if DEBUG==1024
    printf("set_note\n");
    #endif
    ...
    do 
    {
        ...
        ControlSystemData(SFIELD_SET_PRESETNOTE, (void *)argm->value, strlen(argm->value));
        req->buffer_end += sprintf(req_bufptr(req), OPTION_OK "%s\n", argm->name);
        return;
    } while (0);
    req->buffer_end += sprintf(req_bufptr(req), OPTION_NG "%s\n", argm->name);
 ...
 /***pass all  notes to client*********************/
static int get_presetnotes(request *req, COMMAND_ARGUMENT *argm)
{

    int ptzChl=atoi(strtok(argm->value,"@"));
    ...
    int i;
    req->buffer_end += sprintf(req_bufptr(req), OPTION_OK "%s=", argm->name);
    for(i=1;i<20;i++)
    {
           req->buffer_end += sprintf(req_bufptr(req), "%s@", pSysInfo->ptz_channel[ptzChl].note[i]);
    }
    req->buffer_end += sprintf(req_bufptr(req), "%s\n", pSysInfo->ptz_channel[ptzChl].note[i]); 
...
HTTP_OPTION HttpOptionTable [] =
{
	...
	{ "ipncptz"             , set_ipncptz               , AUTHORITY_VIEWER, FALSE,  TRUE, NULL },
	{ "ptzpopup"            , init_ipncptz              , AUTHORITY_VIEWER, FALSE,  TRUE, NULL },
	...
	/*      LIVE VIDEO SCREEN */
//***add by wbb***//
       { "setpresetnote"				, set_presetnote				, AUTHORITY_OPERATOR, FALSE,  TRUE, NULL },
       { "getpresetnotes"				, get_presetnotes				, AUTHORITY_OPERATOR, FALSE,  TRUE, NULL },
	...
	//add by sxh
	{ "enablerecordch"		, enablerecordch	   	    , AUTHORITY_OPERATOR, FALSE,  TRUE, NULL },
	
	{ "pbrecdaydata"		, get_pbrecdaydata	   	    , AUTHORITY_OPERATOR, FALSE,  TRUE, NULL },
	{ "pbrechourdata"		, get_pbrechourdata	   	    , AUTHORITY_OPERATOR, FALSE,  TRUE, NULL },
	{ "startplaybackch"		, start_playbackch	   	    , AUTHORITY_VIEWER, FALSE,  TRUE, NULL },
	{ "stopplaybackch"		, stop_playbackch	   	    , AUTHORITY_OPERATOR, FALSE,  TRUE, NULL },
	{ "pauseplaybackch"		, pause_playbackch	   	    , AUTHORITY_OPERATOR, FALSE,  TRUE, NULL },
	{ "resumeplaybackch"	, resume_playbackch	   	    , AUTHORITY_OPERATOR, FALSE,  TRUE, NULL },
	...
	{ "lbce"     				, set_gbce          		, AUTHORITY_OPERATOR, FALSE,  TRUE, NULL },
	...
	{ "setptz"				, set_ptz					, AUTHORITY_OPERATOR, FALSE,  TRUE, NULL },
	{ "getptzdevaddr" 	        , get_ptzaddr          , AUTHORITY_VIEWER , NULL },
	{ "getptzprotocol"           , get_ptzprotocol          , AUTHORITY_VIEWER , NULL },
	{ "getptzport"                 , get_ptzport          , AUTHORITY_VIEWER , NULL },
	{ "getptzspeed"		, get_ptzspeed  		 , AUTHORITY_VIEWER , NULL },
        { "getptzdatabit"		, get_ptzdatabit  		 , AUTHORITY_VIEWER , NULL },
        { "getptzstopbit"		, get_ptzstopbit  		 , AUTHORITY_VIEWER , NULL },
	{ "getptzparity"    	        , get_ptzparity             , AUTHORITY_VIEWER , NULL },
	{ "getptzstreamctrl"        , get_ptzstreamctrl         , AUTHORITY_VIEWER , NULL },

        #if 0
	{ "rs485"				, set_rs485					, AUTHORITY_OPERATOR, FALSE,  TRUE, NULL },
	{ "getrs485devnum"				, get_rs485dev_num					, AUTHORITY_OPERATOR, FALSE,  TRUE, NULL },
	{ "rs485devadd"				, set_rs485dev_add					, AUTHORITY_OPERATOR, FALSE,  TRUE, NULL },
	{ "rs485devdel"				, set_rs485dev_del					, AUTHORITY_OPERATOR, FALSE,  TRUE, NULL },
	{ "rs232"				, set_rs232 				, AUTHORITY_OPERATOR, FALSE,  TRUE, NULL },

	{ "rs232speed"		    , set_rs232speed				, AUTHORITY_OPERATOR, FALSE,  TRUE, NULL },
	{ "rs232databit"			, set_rs232databit				, AUTHORITY_OPERATOR, FALSE,  TRUE, NULL },
	{ "rs232stopbit"			, set_rs232stopbit	        	, AUTHORITY_OPERATOR, FALSE,  TRUE, NULL },
	{ "rs232parity"			, set_rs232parity	     	, AUTHORITY_OPERATOR, FALSE,  TRUE, NULL },
	{ "rs232streamctrl"				, set_rs232streamctrl					, AUTHORITY_OPERATOR, FALSE,  TRUE, NULL },


	{ "getrs485speed"		, get_rs485speed  		 , AUTHORITY_VIEWER , NULL },
    { "getrs485databit"		, get_rs485databit  		 , AUTHORITY_VIEWER , NULL },
    { "getrs485stopbit"		, get_rs485stopbit  		 , AUTHORITY_VIEWER , NULL },
	{ "getrs485parity"    	 , get_rs485parity             , AUTHORITY_VIEWER , NULL },
	{ "getrs485streamctrl"	 , get_rs485streamctrl         , AUTHORITY_VIEWER , NULL },
	{ "getrs485devaddr" 	, get_rs485devaddr          , AUTHORITY_VIEWER , NULL },
	
	{ "getrs232speed"		, get_rs232speed  		 , AUTHORITY_VIEWER , NULL },
    { "getrs232databit"	, get_rs232databit  		 , AUTHORITY_VIEWER , NULL },
    { "getrs232stopbit"	, get_rs232stopbit  		 , AUTHORITY_VIEWER , NULL },
	{ "getrs232parity"     , get_rs232parity             , AUTHORITY_VIEWER , NULL },
	{ "getrs232streamctrl" , get_rs232streamctrl         , AUTHORITY_VIEWER , NULL },
        #endif

	{ "setplctcp"				, set_plc_tcp		, AUTHORITY_OPERATOR, FALSE,  TRUE, NULL },
	{ "setplcserial"				, set_plc_serial	, AUTHORITY_OPERATOR, FALSE,  TRUE, NULL },
	{ "getplctcp"				, get_plc_tcp		        , AUTHORITY_OPERATOR, FALSE,  TRUE, NULL },
	{ "getplcserial"				, get_plc_serial		, AUTHORITY_OPERATOR, FALSE,  TRUE, NULL },
    ...
    { "darkblankalarm"      , set_darkblankalarm   		, AUTHORITY_OPERATOR, FALSE,  TRUE, NULL },
