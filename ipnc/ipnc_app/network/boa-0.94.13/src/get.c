...
int process_get(request * req)
{
    ...
    if (req->filepos == req->filesize) { /* EOF */
		...
		if (req->http_stream == URI_STREAM_MPEG4_1 || req->http_stream == URI_STREAM_AVC_1) {
            int ret;
#if 1
#if 0
		while (audio_get(req) > 0);
...
#else
            if (audio_get(req, FMT_MPEG4_1) > 0)
			return 1;
#endif
#else
		req->busy_flag &= ~BUSY_FLAG_AUDIO;
#endif
            ret = GetAVData(AV_OP_LOCK_MP4_1, req->serial_book, &req->av_data);
            if (ret == RET_SUCCESS) {
                ...
                reset_output_buffer(req);
                req_write(req, "\r\n");
				if (req->http_stream == URI_STREAM_AVC_1)
                { print_avc_headers(req); }
				else
				{ print_mpeg4_headers(req); }
                return 1;
                ...
                GetAVData(AV_OP_GET_MPEG4_1_SERIAL, -1, &req->av_data );
                req->serial_book = req->av_data.serial;
                dbg("lock error ret=%d\n", ret);
                return 1;
            }
        }
		if (req->http_stream == URI_STREAM_MPEG4_2 || req->http_stream == URI_STREAM_AVC_2) {
            int ret;
#if 1
#if 0
		while (audio_get(req) > 0);
...
#else
            if (audio_get(req, FMT_MPEG4_2) > 0)
			return 1;
#endif
#else
		req->busy_flag &= ~BUSY_FLAG_AUDIO;
#endif
            ret = GetAVData(AV_OP_LOCK_MP4_2, req->serial_book, &req->av_data);
            if (ret == RET_SUCCESS) {
                ...
                reset_output_buffer(req);
                req_write(req, "\r\n");
				if (req->http_stream == URI_STREAM_AVC_2)
                { print_avc_headers(req); }
				else
				{ print_mpeg4_headers(req); }
                return 1;
                ...
                GetAVData(AV_OP_GET_MPEG4_2_SERIAL, -1, &req->av_data );
                req->serial_book = req->av_data.serial;
                dbg("lock error ret=%d\n", ret);
                return 1;
            }
        }
		if (req->http_stream == URI_STREAM_MPEG4_3 || req->http_stream == URI_STREAM_AVC_3) {
            int ret;
#if 1
#if 0
		while (audio_get(req) > 0);
...
#else
            if (audio_get(req, FMT_MPEG4_3) > 0)
			return 1;
#endif
#else
		req->busy_flag &= ~BUSY_FLAG_AUDIO;
#endif
            ret = GetAVData(AV_OP_LOCK_MP4_3, req->serial_book, &req->av_data);
            if (ret == RET_SUCCESS) {
                ...
                reset_output_buffer(req);
                req_write(req, "\r\n");
				if (req->http_stream == URI_STREAM_AVC_3)
                { print_avc_headers(req); }
				else
				{ print_mpeg4_headers(req); }
                return 1;
                ...
                GetAVData(AV_OP_GET_MPEG4_3_SERIAL, -1, &req->av_data );
                req->serial_book = req->av_data.serial;
                dbg("lock error ret=%d\n", ret);
                return 1;
            }
