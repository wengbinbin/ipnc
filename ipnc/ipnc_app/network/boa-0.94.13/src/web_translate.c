 ...
 *                                                                         *
 ***************************************************************************/
int para_supportstream7(char *data, char *arg)
{
	SysInfo* pSysInfo = GetSysInfo();
	if(pSysInfo == NULL)
		return -1;
	return sprintf(data, "%d", pSysInfo->lan_config.Supportstream7);
}
/***************************************************************************
 *                                                                         *
 ***************************************************************************/
int para_supportstream8(char *data, char *arg)
{
	SysInfo* pSysInfo = GetSysInfo();
	if(pSysInfo == NULL)
		return -1;
	return sprintf(data, "%d", pSysInfo->lan_config.Supportstream8);
}
...
int para_streamname4(char *data, char *arg)
{
	NET_IPV4 ip;
	SysInfo* pSysInfo = GetSysInfo();
	if(pSysInfo == NULL)
	...
	ip.int32 = net_get_ifaddr(nicname);
	//printf("");
	return sprintf(data, "%s(%dx%d)@rtsp://%d.%d.%d.%d:%d/%s", pSysInfo->stream_config[3].name,
		pSysInfo->stream_config[3].width,pSysInfo->stream_config[3].height,
		ip.str[0], ip.str[1], ip.str[2], ip.str[3],
		pSysInfo->stream_config[3].portnum, pSysInfo->stream_config[3].portname);
}


/*

int para_streamname5(char *data, char *arg)
{
	NET_IPV4 ip;
	SysInfo* pSysInfo = GetSysInfo();
	if(pSysInfo == NULL)
	...
	ip.int32 = net_get_ifaddr(nicname);
	//printf("");
	return sprintf(data, "%s(%dx%d)@rtsp://%d.%d.%d.%d:%d/%s", pSysInfo->stream_config[4].name,
		pSysInfo->stream_config[4].width,pSysInfo->stream_config[4].height,
		ip.str[0], ip.str[1], ip.str[2], ip.str[3],
		pSysInfo->stream_config[4].portnum, pSysInfo->stream_config[4].portname);
}
int para_streamname6(char *data, char *arg)
{
	NET_IPV4 ip;
	SysInfo* pSysInfo = GetSysInfo();
	if(pSysInfo == NULL)
	...
	ip.int32 = net_get_ifaddr(nicname);
	//printf("");
	return sprintf(data, "%s(%dx%d)@rtsp://%d.%d.%d.%d:%d/%s", pSysInfo->stream_config[5].name,
		pSysInfo->stream_config[5].width,pSysInfo->stream_config[5].height,
		ip.str[0], ip.str[1], ip.str[2], ip.str[3],
		pSysInfo->stream_config[5].portnum, pSysInfo->stream_config[5].portname);
}
int para_streamname7(char *data, char *arg)
{
	NET_IPV4 ip;
	SysInfo* pSysInfo = GetSysInfo();
	if(pSysInfo == NULL)
	...
	ip.int32 = net_get_ifaddr(nicname);
	//printf("");
	return sprintf(data, "%s(%dx%d)@rtsp://%d.%d.%d.%d:%d/%s", pSysInfo->stream_config[6].name,
		pSysInfo->stream_config[6].width,pSysInfo->stream_config[6].height,
		ip.str[0], ip.str[1], ip.str[2], ip.str[3],
		pSysInfo->stream_config[6].portnum, pSysInfo->stream_config[6].portname);
}
int para_streamname8(char *data, char *arg)
{
	NET_IPV4 ip;
	SysInfo* pSysInfo = GetSysInfo();
	if(pSysInfo == NULL)
	...
	ip.int32 = net_get_ifaddr(nicname);
	//printf("");
	return sprintf(data, "%s(%dx%d)@rtsp://%d.%d.%d.%d:%d/%s", pSysInfo->stream_config[7].name,
		pSysInfo->stream_config[7].width,pSysInfo->stream_config[7].height,
		ip.str[0], ip.str[1], ip.str[2], ip.str[3],
		pSysInfo->stream_config[7].portnum, pSysInfo->stream_config[7].portname);
}
...
int para_chrecenable(char *data, char *arg)
{
	SysInfo* pSysInfo = GetSysInfo();
	if(pSysInfo == NULL)
		return -1;
	return sprintf(data, "%d", pSysInfo->stream_config[0].chrecenable|pSysInfo->stream_config[1].chrecenable<<1|
								pSysInfo->stream_config[2].chrecenable<<2|pSysInfo->stream_config[3].chrecenable<<3);
}
...
int para_videocodecresname(char *data, char *arg)
{
	char *resname_list[] =
	{
		"H264:720;H264:D1;H264:SXGA;H264:1080",
		"MPEG4:720;MPEG4:D1;MPEG4:SXGA;MPEG4:1080",
		"JPG:2MP(1600x1200);JPG:3MP(2048x1536);JPG:5MP(2592x1920)",
		"H264:720,JPEG:VGA;H264:D1,JPEG:D1;H264:720,JPEG:720",
		"MPEG4:720,JPEG:VGA;MPEG4:D1,JPEG:D1;MPEG4:720,JPEG:720",
		"H264:720,H264:QVGA;H264:D1,H264:D1;H264:1080,H264:QVGA",
		"MPEG4:720,MPEG4:QVGA;MPEG4:D1,MPEG4:D1;MPEG4:1080,MPEG4:QVGA",
		"H264:D1,MPEG4:D1",
...
{
	"30,24,15,8;30,24,15,8;30,24,15,8;22,15,8",
	"30,24,15,8;30,24,15,8;30,24,15,8;22,15,8",
	"24,15,8;15,12,8;Auto",
	"30,24,15,8;30,24,15,8;30,24,15,8",
	"30,24,15,8;30,24,15,8;30,24,15,8",
	"30,24,15,8;30,24,15,8;22,15,8",
	"30,24,15,8;30,24,15,8;22,15,8",
	"30,24,15,8",
	"30,24,15,8", "30,24,15,8"
};
...
{
	"30,15,5;30,15,5;30,15,5;22,15,5",
	"30,15,5;30,15,5;30,15,5;22,15,5",
	"24,15,5;15,5;Auto",
	"30,15,5;30,15,5;30,15,5",
	"30,15,5;30,15,5;30,15,5",
	"30,15,5;30,15,5;22,15,5",
	"30,15,5;30,15,5;22,15,5",
	"30,15,5",
	"30,15,5", "30,15,5"
};
 ...
 int para_aviformatname(char *data, char *arg)
{
	...
	int supportMpeg4 = pSysInfo->lan_config.Supportstream6;
	int supportMpeg4cif = pSysInfo->lan_config.Supportstream7;
	int supportH264 = pSysInfo->lan_config.Supportstream1;
	int supportH264_1 = pSysInfo->lan_config.Supportstream2;
	int supportH264_2 = pSysInfo->lan_config.Supportstream3;
	int supportH264_3 = pSysInfo->lan_config.Supportstream4;
	int supportH264cif = pSysInfo->lan_config.Supportstream8;
	...
	if ( supportH264 ) {
		if (supportH264cif){
			return sprintf(data,"H.264(%d x %d);H.264(%d x %d)", avc1Xsize,avc1Ysize,avc5Xsize,avc5Ysize);
		}
		...
		if (supportH264cif){
			return sprintf(data,"H.264(%d x %d);H.264(%d x %d)", avc2Xsize,avc2Ysize,avc5Xsize,avc5Ysize);
		}
		else if(supportMpeg4cif){
			return sprintf(data,"H.264(%d x %d);MPEG4(%d x %d)",avc2Xsize,avc2Ysize,mpeg42Xsize,mpeg42Ysize);
		}
		else {
			return sprintf(data,"H.264(%d x %d)",avc2Xsize,avc2Ysize);
		}
	}else if ( supportH264_2 ) {
		if (supportH264cif){
			return sprintf(data,"H.264(%d x %d);H.264(%d x %d)", avc3Xsize,avc3Ysize,avc5Xsize,avc5Ysize);
		}
		else if(supportMpeg4cif){
			return sprintf(data,"H.264(%d x %d);MPEG4(%d x %d)",avc3Xsize,avc3Ysize,mpeg42Xsize,mpeg42Ysize);
		}
		else {
			return sprintf(data,"H.264(%d x %d)",avc3Xsize,avc3Ysize);
		}
	}else if ( supportH264_3 ) {
		if (supportH264cif){
			return sprintf(data,"H.264(%d x %d);H.264(%d x %d)", avc4Xsize,avc4Ysize,avc5Xsize,avc5Ysize);
		}
		else if(supportMpeg4cif){
			return sprintf(data,"H.264(%d x %d);MPEG4(%d x %d)",avc4Xsize,avc4Ysize,mpeg42Xsize,mpeg42Ysize);
		}
		else {
			return sprintf(data,"H.264(%d x %d)",avc4Xsize,avc4Ysize);
		}
	}
 ...
 int para_meconfigname(char *data, char *arg)
{
	...
	if ( *arg == '0' )
		...
		return sprintf(data, "HIGH SPEED;HIGH QUALITY");
#else
	if ( *arg == '0' )
		return sprintf(data, "AUTO");
	else if ( *arg == '1' )
		...
		return sprintf(data, "AUTO;CUSTOM");
#endif
  ...
  int para_encodingname(char *data, char *arg)
{
	...
	if ( *arg == '1' )
		...
		return sprintf(data, "G711;OTHERS");
#else
	else if ( *arg == 'a' || *arg == '\0' )
		return sprintf(data, "G711");
#endif
...
int para_lostalarm(char *data, char *arg)
{
 ...
 int para_darkblankalarm(char *data, char *arg)
{
	SysInfo* pSysInfo = GetSysInfo();
	if(pSysInfo == NULL)
		return -1;
	return sprintf(data, "%d", pSysInfo->lan_config.nDarkBlank);
}
...
HTML_ARGUMENT HttpArgument [] =
{
  ...
  { "minnamelen"		    , para_minnamelen			, AUTHORITY_VIEWER , NULL },
	...
	{ "supportstream7"		, para_supportstream7	, AUTHORITY_VIEWER , NULL },
	{ "supportstream8"		, para_supportstream8	, AUTHORITY_VIEWER , NULL },
	...
	{ "streamname4"			, para_streamname4			, AUTHORITY_VIEWER , NULL },
	//{ "streamname5"			, para_streamname5			, AUTHORITY_VIEWER , NULL },
	//{ "streamname6"			, para_streamname6			, AUTHORITY_VIEWER , NULL },
	//{ "streamname7"			, para_streamname7			, AUTHORITY_VIEWER , NULL },
	//{ "streamname8"			, para_streamname8			, AUTHORITY_VIEWER , NULL },
	...
	{ "chrecenable"			, para_chrecenable			, AUTHORITY_VIEWER , NULL },
	...
	{ "lbce"     				, para_gbce          		, AUTHORITY_VIEWER   	, NULL },
    ...
    { "darkblankalarm"      , para_darkblankalarm   , AUTHORITY_VIEWER , NULL },
...
int ShowAllWebValue(char *data, int max_size, AUTHORITY authority)
{
	...
	for(i = 0;i < HASH_TABLE_SIZE; i++){
		...
		#ifdef VANLINK_DVR_CONFIG_DEBUG
		//printf("~~~~~~~~~~~~htmp->handler wirte to data=%s\n",buf);
		#endif
		total_size += sprintf(data+total_size, "%s<br>", buf);
	}
	#ifdef VANLINK_DVR_CONFIG_DEBUG
	printf("~~~~~~~~~~~~ShowAllWebValue Done!!!\n");
	#endif
	return total_size;
}
