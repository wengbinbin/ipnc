...
int retrieve_system_storage_volumes_id_files_id(request *req, node_t *node)
{
	...
	pFilelist = Get_File_List( MMC_PATH, &count );
...
int retrieve_system_video_channels_id(request *req, node_t *node)
{
	...
	if(pSysInfo->lan_config.nTVcable == 0)
	   sprintf(VideoInputChannel.powerLineFrequencyMode,"60hz");
	else if(pSysInfo->lan_config.nTVcable == 1)
...
int update_system_video_channels_id(request *req, node_t *node)
{
	...
	while(tag) {
		if(strcmp(tag->name, TAG_id) == 0) {
			...
			if(strncmp(VideoInputChannel.powerLineFrequencyMode,"60hz",4) == 0)
               strcpy(VideoInputChannel.powerLineFrequencyMode,"0");
			else if(strncmp(VideoInputChannel.powerLineFrequencyMode,"50hz",4) == 0)
...
int retrieve_system_video_channels_id_overlays_text(request *req, node_t *node)
{
	...
	//fprintf(stderr,"\r\nInside retrieve_system_video_channels_id_overlays_text with nStreamType:%d\n",pSysInfo->lan_config.nStreamType);
...
int retrieve_system_video_channels_id_overlays_text_id(request *req, node_t *node)
{
	...
	//fprintf(stderr,"\r\nInside retrieve_system_video_channels_id_overlays_text_id with nStreamType:%d\n",pSysInfo->lan_config.nStreamType);
...
int update_system_time_ntpservers_id(request *req, node_t *node)
{
	...
	required = 0;
	while(tag) {
		if(strcmp(tag->name, TAG_addressingFormatType) == 0) {
	 ...
	 if(pSysInfo->lan_config.nStreamType == 0){
		...
		if(pSysInfo->lan_config.nVideocombo == 0){
		  ...
		  if(pSysInfo->lan_config.nVideocodecres == 0){
			sprintf(StreamingChannel.videoResolutionWidth,"352");//"1280");//add by sxh
	        sprintf(StreamingChannel.videoResolutionHeight,"240");//"720");
		  ...
		  sprintf(StreamingChannel.rtspPortNo,"554");
		...
		if (pSysInfo->lan_config.nVideocombo == 0){
		 ...
		 sprintf(StreamingChannel.videoResolutionWidth,"352");//"720");//add by sxh
	        sprintf(StreamingChannel.videoResolutionHeight,"240");//"480");
        }
	 }
	 else if (pSysInfo->lan_config.nStreamType == 2){
			...
			while(tag) {
			  if(strcmp(tag->name, TAG_enabled) == 0) {
				...
				if(strncmp(StreamingChannel.videoCodecType,"H.264",3) == 0)
                   strcpy(StreamingChannel.videoCodecType,"0");
				else if(strncmp(StreamingChannel.videoCodecType,"MPEG4",3) == 0)
                   strcpy(StreamingChannel.videoCodecType,"1");
				else if(strncmp(StreamingChannel.videoCodecType,"MegaPixel JPEG",3) == 0)
                   strcpy(StreamingChannel.videoCodecType,"2");

			    required |= xml_required_field(12);
				value = atoi(StreamingChannel.videoCodecType);
				...
				//fprintf(stderr,"\r\nInside TAG_videoResolutionWidth\n");
				...
				//fprintf(stderr,"\r\nInside TAG_constantBitRate\n");
			    strcpy(StreamingChannel.constantBitRate, tag->value);
			    required |= xml_required_field(17);
				value = atoi(StreamingChannel.constantBitRate);
                /*
				 if(channel_id ==0){
				   if(StreamingChannel.id != 0)
				    ControlSystemData(SFIELD_SET_MPEG41_BITRATE, (void *)&value, sizeof(value));
				 }
			     else if(channel_id == 1){
			       if(StreamingChannel.id != 0){
					 ControlSystemData(SFIELD_SET_MPEG42_BITRATE, (void *)&value, sizeof(value));
				   }
				 }
			     else if(channel_id == 2){
					ControlSystemData(SFIELD_SET_MPEG42_BITRATE, (void *)&value, sizeof(value));
				...
				*/
			    ...
			    required |= xml_required_field(2);
			    value = atoi(StreamingChannel.keyFrameInterval);

			    if(channel_id ==0)
			       ControlSystemData(SFIELD_SET_IPRATIO1, &value, sizeof(value));
			    else if(channel_id == 1)
			       ControlSystemData(SFIELD_SET_IPRATIO2, &value, sizeof(value));
			    else if(channel_id == 2)
			       ControlSystemData(SFIELD_SET_IPRATIO3, &value, sizeof(value));
		      }
...
int update_custom_video_advanced_channels(request *req, node_t *node)
{
	...
	while(tag) {
		if(strcmp(tag->name, TAG_VideoAdvancedSetting) == 0) {
			idflag = 1;
			update_custom_video_advanced_channels_id(req, node);
		    required |= xml_required_field(1);
			channel_id++;
		}
	 /* Protocol */
...
int update_custom_video_advanced_channels_id(request *req, node_t *node)
{
	int ret;
	tag_t	*tag;
	unsigned long required;
...
int retrieve_streaming_channels_id_http(request *req, node_t *node)
{
	...
	else if(strstr(req->request_uri, "5")){
		if(pSysInfo->lan_config.nVideocodecmode == 8)
		   uri_avccif_3(req);
		else
		   uri_avc_3(req);
...
int retrieve_streaming_channels_id_http_capabilities(request *req, node_t *node)
{
	...
	const char *videoCodecTypeStr;
	const char *videoResolutionWidthStr;
	const char *videoResolutionHeightStr;
	const char *videoQualityControlTypeStr = "opt=\"OFF,VBR,CBR\"";
	const char *maxFrameRateStr;
	const char *audioCompressionTypeStr = "opt=\"G.711ulaw\"";
	const char *audioBitRateStr = "opt=\"64Kbps,128Kbps\"";
	const char *audioSamplingRateStr = "opt=\"8Khz,16Khz\"";

	if(pSysInfo->lan_config.nStreamType == 0){
        videoCodecTypeStr = "opt=\"H.264,MPEG4,MegaPixel JPEG\"";
	    if(pSysInfo->lan_config.nVideocombo == 0){
	       videoResolutionWidthStr = "opt=\"H264:720,H264:D1,H264:SXGA,H264:1080\"";
		   if(pSysInfo->lan_config.nVideocodecres == 0)
	       ...
	       videoResolutionWidthStr = "opt=\"MPEG4:720,MPEG4:D1,MPEG4:SXGA,MPEG4:1080\"";
		   if(pSysInfo->lan_config.nVideocodecres == 0)
              maxFrameRateStr = "opt=\"30,24,15,8\"";
		   else if(pSysInfo->lan_config.nVideocodecres == 1)
	          maxFrameRateStr = "opt=\"30,24,15,8\"";
	          ...
	          maxFrameRateStr = "opt=\"21,15,8\"";
	     }
	     else if(pSysInfo->lan_config.nVideocombo == 2){
	       videoResolutionWidthStr = "opt=\"JPG:1600X1200,JPG:2048X1536\"";
		   if(pSysInfo->lan_config.nVideocodecres == 0)
              maxFrameRateStr = "\"opt=24,15,8\"";
		   else if(pSysInfo->lan_config.nVideocodecres == 1)
	          maxFrameRateStr = "\"opt=15,12,8\"";
	    ...
	    if(pSysInfo->lan_config.nVideocombo == 0){
	       ...
	       videoResolutionWidthStr = "opt=\"H264:720 H264:QVGA,H264:D1 H264:D1,H264:1080 H264:QVGA\"";
	     }
		 else if(pSysInfo->lan_config.nVideocombo == 3){
	       videoResolutionWidthStr = "opt=\"MPEG4:720 MPEG4:QVGA,MPEG4:D1 MPEG4:D1,MPEG4:1080 MPEG4:QVGA\"";
	     }
		 else if(pSysInfo->lan_config.nVideocombo == 4){
	    ...
	    if(pSysInfo->lan_config.nVideocombo == 0){
		   ...
		   maxFrameRateStr = "opt=\"30,24,15,8;15,12,8;15,12,8\"";
	     }
	     else if(pSysInfo->lan_config.nVideocombo == 1){
...
/*	if(pSysInfo->lan_config.nDarkBlankAlarm == 1){
       sprintf(EventTrigger.eventType,"Dark/Blank Image");
	   xadd_elem(req, TAG_eventType, EventTrigger.eventType);
    }
...
/*	if(pSysInfo->lan_config.nDarkBlankAlarm == 1){
      sprintf(EventTrigger.eventDescription,"Dark/Blank Image Alarm");
      xadd_elem(req, TAG_eventDescription, EventTrigger.eventDescription);
	}
*/	if(pSysInfo->lan_config.nExtAlarm == 1){
...
int retrieve_custom_event_notification(request *req, node_t *node)
{
	...
	SysInfo* pSysInfo = GetSysInfo();
	int supportMpeg4=0,supportMpeg4cif=0 ,supportH264=0,supportH264_1=0,supportH264_2=0,supportH264_3=0, supportH264cif=0,supportJPG=0;

	supportJPG = pSysInfo->lan_config.Supportstream5;
	supportMpeg4 = pSysInfo->lan_config.Supportstream6;
	supportMpeg4cif = pSysInfo->lan_config.Supportstream7;
	supportH264 = pSysInfo->lan_config.Supportstream1;
	supportH264_1 = pSysInfo->lan_config.Supportstream2;
	supportH264_2 = pSysInfo->lan_config.Supportstream3;
	supportH264_3 = pSysInfo->lan_config.Supportstream4;
	supportH264cif = pSysInfo->lan_config.Supportstream8;
	...
	int avc1Xsize=pSysInfo->lan_config.Avc1Xsize;//h264
	int avc1Ysize=pSysInfo->lan_config.Avc1Ysize;
	int avc2Xsize=pSysInfo->lan_config.Avc2Xsize;//h264_1
	int avc2Ysize=pSysInfo->lan_config.Avc2Ysize;
	int avc3Xsize=pSysInfo->lan_config.Avc3Xsize;//h264_2 //add by sxh
	int avc3Ysize=pSysInfo->lan_config.Avc3Ysize;
	int avc4Xsize=pSysInfo->lan_config.Avc4Xsize;//h264_3
	int avc4Ysize=pSysInfo->lan_config.Avc4Ysize;
	int avc5Xsize=pSysInfo->lan_config.Avc5Xsize;//h264_cif
	int avc5Ysize=pSysInfo->lan_config.Avc5Ysize;
	...
	if(supportH264){
		if(supportH264cif){
		   if(pSysInfo->lan_config.aviformat == 0)
	          sprintf(EvenNotificationMethods.videoClipFormatType,"H.264(%d x %d)",avc5Xsize,avc5Ysize);
		...
		if(supportH264cif){
		   ...
		   sprintf(EvenNotificationMethods.videoClipFormatType,"H.264(%d x %d)",avc2Xsize,avc2Ysize);
	}else if(supportH264_2){
		if(supportH264cif){
		   if(pSysInfo->lan_config.aviformat == 0)
	          sprintf(EvenNotificationMethods.videoClipFormatType,"H.264(%d x %d)",avc5Xsize,avc5Ysize);
	       else if(pSysInfo->lan_config.aviformat == 1)
	          sprintf(EvenNotificationMethods.videoClipFormatType,"H.264(%d x %d)",avc3Xsize,avc3Ysize);
		}
		else if(supportMpeg4cif){
		   if(pSysInfo->lan_config.aviformat == 0)
	          sprintf(EvenNotificationMethods.videoClipFormatType,"H.264(%d x %d)",avc3Xsize,avc3Ysize);
		   ...
		   sprintf(EvenNotificationMethods.videoClipFormatType,"H.264(%d x %d)",avc3Xsize,avc3Ysize);
	}else if(supportH264_3){
		if(supportH264cif){
		   if(pSysInfo->lan_config.aviformat == 0)
	          sprintf(EvenNotificationMethods.videoClipFormatType,"H.264(%d x %d)",avc5Xsize,avc5Ysize);
	       else if(pSysInfo->lan_config.aviformat == 1)
	          sprintf(EvenNotificationMethods.videoClipFormatType,"H.264(%d x %d)",avc4Xsize,avc4Ysize);
		}
		else if(supportMpeg4cif){
		   if(pSysInfo->lan_config.aviformat == 0)
	          sprintf(EvenNotificationMethods.videoClipFormatType,"H.264(%d x %d)",avc4Xsize,avc4Ysize);
...
int retrieve_custom_event_notification_capabilities(request *req, node_t *node)
{
	...
	int avc2Ysize=pSysInfo->lan_config.Avc2Ysize;
	int avc3Xsize=pSysInfo->lan_config.Avc3Xsize;//add by sxh
	int avc3Ysize=pSysInfo->lan_config.Avc3Ysize;
	int avc4Xsize=pSysInfo->lan_config.Avc4Xsize;
	int avc4Ysize=pSysInfo->lan_config.Avc4Ysize;
	int avc5Xsize=pSysInfo->lan_config.Avc5Xsize;
	int avc5Ysize=pSysInfo->lan_config.Avc5Ysize;


	if ( supportH264 ) {
		if ( supportH264cif ){
			sprintf(videoClipFormatTypeStr,"opt=\"H.264(%d x %d),H.264(%d x %d)\"", avc1Xsize,avc1Ysize,avc5Xsize,avc5Ysize);
		}
		else if( supportMpeg4cif ){
		...
		if ( supportH264cif ){
			sprintf(videoClipFormatTypeStr,"opt=\"H.264(%d x %d),H.264(%d x %d)\"", avc2Xsize,avc2Ysize,avc5Xsize,avc5Ysize);
		}
		else if( supportMpeg4cif ){
			sprintf(videoClipFormatTypeStr,"opt=\"H.264(%d x %d),MPEG4(%d x %d)\"",avc2Xsize,avc2Ysize,mpeg42Xsize,mpeg42Ysize);
		}else
			sprintf(videoClipFormatTypeStr,"opt=\"H.264(%d x %d)\"",avc2Xsize,avc2Ysize);
	}else if ( supportH264_2 ) {
		if ( supportH264cif ){
			sprintf(videoClipFormatTypeStr,"opt=\"H.264(%d x %d),H.264(%d x %d)\"", avc3Xsize,avc3Ysize,avc5Xsize,avc5Ysize);
		}
		else if( supportMpeg4cif ){
			sprintf(videoClipFormatTypeStr,"opt=\"H.264(%d x %d),MPEG4(%d x %d)\"",avc3Xsize,avc3Ysize,mpeg42Xsize,mpeg42Ysize);
		}else
			sprintf(videoClipFormatTypeStr,"opt=\"H.264(%d x %d)\"",avc3Xsize,avc3Ysize);
	}else if ( supportH264_3 ) {
		if ( supportH264cif ){
			sprintf(videoClipFormatTypeStr,"opt=\"H.264(%d x %d),H.264(%d x %d)\"", avc4Xsize,avc4Ysize,avc5Xsize,avc5Ysize);
		}
		else if( supportMpeg4cif ){
			sprintf(videoClipFormatTypeStr,"opt=\"H.264(%d x %d),MPEG4(%d x %d)\"",avc4Xsize,avc4Ysize,mpeg42Xsize,mpeg42Ysize);
		}else
			sprintf(videoClipFormatTypeStr,"opt=\"H.264(%d x %d)\"",avc4Xsize,avc4Ysize);
	}
	else if ( supportMpeg4 ){
		if ( supportH264cif ){
		...
		if(strcmp(tag->name, TAG_id) == 0) {
			...
			while(tag) {
			  if(strcmp(tag->name, TAG_positionX) == 0) {
			     ...
			     required |= xml_required_field(4);
                 value = atoi(FaceDetection.positionX);
				 ControlSystemData(SFIELD_SET_FDX, &value, sizeof(value));
			  }
			  else if(strcmp(tag->name, TAG_positionY) == 0) {
                 strcpy(FaceDetection.positionY, tag->value);
			     value = atoi(FaceDetection.positionY);
				 ControlSystemData(SFIELD_SET_FDY, &value, sizeof(value));
			  }
			  else if(strcmp(tag->name, TAG_width) == 0) {
                 strcpy(FaceDetection.width, tag->value);
			     required |= xml_required_field(6);
				 value = atoi(FaceDetection.width);
				 ControlSystemData(SFIELD_SET_FDW, &value, sizeof(value));
			  }
			  else if(strcmp(tag->name, TAG_height) == 0) {
                 strcpy(FaceDetection.height, tag->value);
			     required |= xml_required_field(7);
				 value = atoi(FaceDetection.height);
				 ControlSystemData(SFIELD_SET_FDH, &value, sizeof(value));
			  }
			  if(!tag->next)
				 break;
			  tag = tag->next;
...
int retrieve_custom_live_channels_id(request *req, node_t *node)
{
	...
	sprintf(LiveSetting.clickSnapFileName,"%d", pSysInfo->lan_config.nClickSnapFilename);
	...
	if((pSysInfo->lan_config.alarmstatus & (1 << 0)) || (pSysInfo->lan_config.alarmstatus & (1 << 1))) {
	  ...
	  if(pSysInfo->lan_config.nStreamType == 0){
		...
		if(pSysInfo->lan_config.nVideocombo == 0){
		  ...
		  if(pSysInfo->lan_config.nVideocodecres == 0){
		    ...
		    sprintf(VideoSetting.videoResolutionSel,"H264:SXGA");
		  }
		  else if(pSysInfo->lan_config.nVideocodecres == 3){
		  ...
		  if(pSysInfo->lan_config.nVideocodecres == 0){
		    ...
		    sprintf(VideoSetting.videoResolutionSel,"MPEG4:SXGA");
		  }
		  else if(pSysInfo->lan_config.nVideocodecres == 3){
	      ...
	      sprintf(VideoSetting.videoCodecSel,"MegaPixel JPEG");
...
int retrieve_custom_video_channels_id_capabilities(request *req, node_t *node)
{
	...
	videoCodecTypeListStr = "opt=\"H.264,MPEG4,MegaPixel JPEG @ H.264 + JPEG,MPEG4 + JPEG,Dual H.264,Dual MPEG4,H264 + MPEG4 @ Dual H264 + JPEG,Dual MPEG4 + JPEG\"";
    videoResolutionWidthListStr = "opt=\"H264:720,H264:D1,H264:SXGA,H264:1080 @ MPEG4:720,MPEG4:D1,MPEG4:SXVGA,MPEG4:1080 @ JPG:1600X1200,JPG:2048X1536 @ H264:720 JPEG:VGA,H264:D1 JPEG:D1,H264:720 JPEG:720 @ MPEG4:720 JPEG:VGA,MPEG4:D1 JPEG:D1,MPEG4:720 JPEG:720 @ H264:720 H264:QVGA,H264:D1 H264:D1,H264:1080 H264:QVGA @ MPEG4:720 MPEG4:QVGA,MPEG4:D1 MPEG4:D1,MPEG4:1080 MPEG4:QVGA @ H264:D1 MPEG4:D1 @ H264:720 JPEG:VGA H264:QVGA @ MPEG4:720 JPEG:VGA MPEG4:QVGA\"";

	if (pSysInfo->lan_config.nDayNight == 0) {
		sprintf(frameRateList1Str, "opt=\"Auto;Auto;Auto;Auto @ Auto;Auto;Auto;Auto @ Auto;Auto @ Auto;Auto;Auto @ Auto;Auto;Auto @ Auto;Auto;Auto @ Auto;Auto;Auto @ Auto @ Auto @ Auto\"");
	}
	else
	{
		char *resname_list1[] =
		{
			"opt=\"30,24,15,8;30,24,15,8;24,15,8;15,12,8", "30,24,15,8;30,24,15,8;24,15,8;21,15,8", "24,15,8;15,12,8",
			"30,24,15,8;30,24,15,8;30,24,15,8", "30,24,15,8;30,24,15,8;30,24,15,8",
			"30,24,15,8;30,24,15,8;15,12,8", "30,24,15,8;30,24,15,8;21,15,8",
			"30,24,15,8",
	...
	if (pSysInfo->lan_config.nDayNight == 0) {
		sprintf(frameRateList2Str, "opt=\"Auto;Auto;Auto;Auto @ Auto;Auto;Auto;Auto @ Auto;Auto @ Auto;Auto;Auto @ Auto;Auto;Auto @ Auto;Auto;Auto @ Auto;Auto;Auto @ Auto @ Auto @ Auto\"");
	}
	else
	{
		char *resname_list2[] =
		{
			"opt=\"NA;NA;NA", "NA;NA;NA;NA", "NA;NA",
			"15,12,8;30,24,15,8;15,12,8", "30,24,15,8;30,24,15,8;15,12,8",
			"30,24,15,8;30,24,15,8;15,12,8", "30,24,15,8;30,24,15,8;21,15,8",
			"30,24,15,8",
			"15,12,8", "30,24,15,8\""
		};
	...
	if (pSysInfo->lan_config.nDayNight == 0) {
		sprintf(frameRateList3Str, "opt=\"Auto;Auto;Auto;Auto @ Auto;Auto;Auto;Auto @ Auto;Auto @ Auto;Auto;Auto @ Auto;Auto;Auto @ Auto;Auto;Auto @ Auto;Auto;Auto @ Auto @ Auto @ Auto\"");
	}
	else
	{
		char *resname_list3[] =
...
int update_custom_video_channels(request *req, node_t *node)
{
	...
	while(tag) {
		if(strcmp(tag->name, TAG_VideoSetting) == 0) {
			update_custom_video_channels_id(req, node);
		    required |= xml_required_field(1);
			channel_id++;
		}
	 /* Protocol */
...
int update_custom_video_channels_id(request *req, node_t *node)
{
	tag_t	*tag;
	unsigned long required;
	VideoSetting_psia VideoSetting;
...
int retrieve_custom_camera_channels_id(request *req, node_t *node)
{
	...
	sprintf(CameraSetting.gbce,"%d",pSysInfo->lan_config.gbce);
	...
	xadd_elem(req, TAG_gbce, CameraSetting.gbce);
...
int update_custom_camera_channels_id(request *req, node_t *node)
{
	...
	while(tag) {
		if(strcmp(tag->name, TAG_id) == 0) {
			...
			if(strncmp(CameraSetting.blc,"Min",3) == 0)
               strcpy(CameraSetting.blc,"0");
			else if(strncmp(CameraSetting.blc,"Mid",3) == 0)
               strcpy(CameraSetting.blc,"1");
			else if(strncmp(CameraSetting.blc,"Max",2) == 0)
               strcpy(CameraSetting.blc,"2");
		...
		else if(strcmp(tag->name, TAG_gbce) == 0) {
			strcpy(CameraSetting.gbce, tag->value);
			required |= xml_required_field(5);
			value = atoi(CameraSetting.gbce);
...
int update_custom_network_channels_id(request *req, node_t *node)
{
    ...
    #if DEBUG==1
    printf("update_custom_network_channels_id\n");
    #endif
...
int retrieve_custom_alarm_triggers_id(request *req, node_t *node)
{
	...
	supportAVI = supportH264|supportH264_1|supportH264_2|supportH264_3|supportH264cif|supportMpeg4|supportMpeg4cif;
	...
	else if(pSysInfo->lan_config.nAlarmAudioFile == 0)
				...
				if(strcmp(tag->name, TAG_TimeBlock) == 0) {
				  ...
				  while(tag) {
					  if(strcmp(tag->name, TAG_enabled) == 0) {
						  ...
						  ControlSystemData(SFIELD_SET_SCHEDULE, &value, sizeof(value));
					  }
					  if(!tag->next)
				        break;
				      tag = tag->next;
