...
static int system_video_channels_id_overlays_text_id(request *req, node_t *node)
{
	...
	if(req->method == M_GET) {
		...
		req->status_code = update_system_video_channels_id_overlays_text_id(req, node);
		add_response_status(req, node);
		send_r_psia_get(req);
...
static int system_video_channels_id_overlays_image_id(request *req, node_t *node)
{
	...
	if(req->method == M_GET) {
		...
		req->status_code = update_system_video_channels_id_overlays_image_id(req, node);
		add_response_status(req, node);
		send_r_psia_get(req);
...
static int system_time_ntpservers_id(request *req, node_t *node)
{
	...
	if(req->method == M_GET) {
		...
		req->status_code = update_system_time_ntpservers_id(req, node);
		add_response_status(req, node);
		send_r_psia_get(req);
		ret = get_http_status(req);
		return 0;
...
static int security_aaa_users_id(request *req, node_t *node)
{
	...
	if(req->method == M_GET) {
		...
		req->status_code = update_security_aaa_users_id(req, node);
		add_response_status(req, node);
		send_r_psia_get(req);
		ret = get_http_status(req);
		return 0;
...
static int streaming_channels_id(request *req, node_t *node)
{
	...
	if(req->method == M_GET) {
		...
		req->status_code = update_streaming_channels_id(req, node);
		add_response_status(req, node);
		send_r_psia_get(req);
...
static int custom_video_advanced_channels_id(request *req, node_t *node)
{
	...
	if(req->method == M_GET) {
		...
		req->status_code = update_custom_video_advanced_channels_id(req, node);
		add_response_status(req, node);
		send_r_psia_get(req);
		ret = get_http_status(req);
		return 0;
...
static int custom_video_channels_id(request *req, node_t *node)
{
	...
	if(req->method == M_GET) {
		...
		req->status_code = update_custom_video_channels_id(req, node);
		add_response_status(req, node);
		send_r_psia_get(req);
...
int uri_avc_1(request * req)
{
	AV_DATA vol_data;

    printf(" BOA: H264\n");

	req->http_stream = URI_STREAM_AVC_1;
	req->status = WRITE;
	if(GetAVData(AV_OP_LOCK_MP4_1_VOL, -1, &vol_data) != RET_SUCCESS) {
		send_r_error(req);
		printf("Error on Get Vol data\n");
		return 0;
	}

	if(GetAVData(AV_OP_LOCK_MP4_1_IFRAME, -1, &req->av_data) != RET_SUCCESS) {
		send_r_error(req);
		printf("Error on AV_OP_LOCK_MP4_1_IFRAME\n");
		return 0;
	}
	...
	req_copy(req, vol_data.ptr, vol_data.size);
	GetAVData(AV_OP_UNLOCK_MP4_1_VOL, -1, NULL);
...
int uri_avc_2(request * req)
{
	AV_DATA vol_data;

    printf(" BOA: H264\n");

	req->http_stream = URI_STREAM_AVC_2;
	req->status = WRITE;
	if(GetAVData(AV_OP_LOCK_MP4_2_VOL, -1, &vol_data) != RET_SUCCESS) {
		send_r_error(req);
		printf("Error on Get Vol data\n");
		return 0;
	}

	if(GetAVData(AV_OP_LOCK_MP4_2_IFRAME, -1, &req->av_data) != RET_SUCCESS) {
		send_r_error(req);
		printf("Error on AV_OP_LOCK_MP4_IFRAME\n");
		return 0;
	}
	...
	req_copy(req, vol_data.ptr, vol_data.size);
	GetAVData(AV_OP_UNLOCK_MP4_2_VOL, -1, NULL);
...
int uri_avc_3(request * req)
{
	AV_DATA vol_data;

    printf(" BOA: H264\n");

	req->http_stream = URI_STREAM_AVC_3;
	req->status = WRITE;
	if(GetAVData(AV_OP_LOCK_MP4_3_VOL, -1, &vol_data) != RET_SUCCESS) {
		send_r_error(req);
		printf("Error on Get Vol data\n");
		return 0;
	}

	if(GetAVData(AV_OP_LOCK_MP4_3_IFRAME, -1, &req->av_data) != RET_SUCCESS) {
		send_r_error(req);
		printf("Error on AV_OP_LOCK_MP4_3_IFRAME\n");
		return 0;
	}
	...
	req_copy(req, vol_data.ptr, vol_data.size);
	GetAVData(AV_OP_UNLOCK_MP4_3_VOL, -1, NULL);
...
int uri_avccif_1(request * req)
{
	AV_DATA vol_data;

  printf(" BOA: H264 CIF\n");

	req->http_stream = URI_STREAM_AVCCIF_1;
	req->status = WRITE;
	if(GetAVData(AV_OP_LOCK_MP4_1_CIF_VOL, -1, &vol_data) != RET_SUCCESS) {
		send_r_error(req);
		dbg("Error on Get Vol data\n");
		return 0;
	}

	if(GetAVData(AV_OP_LOCK_MP4_1_CIF_IFRAME, -1, &req->av_data) != RET_SUCCESS) {
		send_r_error(req);
		dbg("Error on AV_OP_LOCK_MP4_CIF_IFRAME\n");
		return 0;
	}
	...
	req_copy(req, vol_data.ptr, vol_data.size);
	GetAVData(AV_OP_UNLOCK_MP4_1_CIF_VOL, -1, NULL);
...
int uri_avccif_2(request * req)
{
	AV_DATA vol_data;

  printf(" BOA: H264 CIF\n");

	req->http_stream = URI_STREAM_AVCCIF_2;
	req->status = WRITE;
	if(GetAVData(AV_OP_LOCK_MP4_2_CIF_VOL, -1, &vol_data) != RET_SUCCESS) {
		send_r_error(req);
		dbg("Error on Get Vol data\n");
		return 0;
	}

	if(GetAVData(AV_OP_LOCK_MP4_2_CIF_IFRAME, -1, &req->av_data) != RET_SUCCESS) {
		send_r_error(req);
		dbg("Error on AV_OP_LOCK_MP4_CIF_IFRAME\n");
		return 0;
	}
	...
	req_copy(req, vol_data.ptr, vol_data.size);
	GetAVData(AV_OP_UNLOCK_MP4_2_CIF_VOL, -1, NULL);
...
int uri_avccif_3(request * req)
{
	AV_DATA vol_data;

  printf(" BOA: H264 CIF\n");

	req->http_stream = URI_STREAM_AVCCIF_3;
	req->status = WRITE;
	if(GetAVData(AV_OP_LOCK_MP4_3_CIF_VOL, -1, &vol_data) != RET_SUCCESS) {
		send_r_error(req);
		dbg("Error on Get Vol data\n");
		return 0;
	}

	if(GetAVData(AV_OP_LOCK_MP4_3_CIF_IFRAME, -1, &req->av_data) != RET_SUCCESS) {
		send_r_error(req);
		dbg("Error on AV_OP_LOCK_MP4_3_CIF_IFRAME\n");
		return 0;
	}
	req->filesize = req->av_data.size+vol_data.size;
	...
	req_copy(req, vol_data.ptr, vol_data.size);
	GetAVData(AV_OP_UNLOCK_MP4_3_CIF_VOL, -1, NULL);
...
static void free_request(request ** list_head_addr, request * req)
{
    ...
    if (req->http_stream == URI_STREAM_JPEG) {
	...
	else if (req->http_stream == URI_STREAM_MPEG4_1 || req->http_stream == URI_STREAM_AVC_1) {
        if (req->serial_lock > 0)
            GetAVData(AV_OP_UNLOCK_MP4_1, req->serial_lock, NULL);
        if (req->audio_lock > 0)
            GetAVData(AV_OP_UNLOCK_ULAW, req->audio_lock, NULL);
    }
	else if (req->http_stream == URI_STREAM_MPEG4_2 || req->http_stream == URI_STREAM_AVC_1) {
        if (req->serial_lock > 0)
            GetAVData(AV_OP_UNLOCK_MP4_2, req->serial_lock, NULL);
        if (req->audio_lock > 0)
            GetAVData(AV_OP_UNLOCK_ULAW, req->audio_lock, NULL);
    }
	else if (req->http_stream == URI_STREAM_MPEG4_3 || req->http_stream == URI_STREAM_AVC_1) {
        if (req->serial_lock > 0)
            GetAVData(AV_OP_UNLOCK_MP4_3, req->serial_lock, NULL);
        if (req->audio_lock > 0)
            GetAVData(AV_OP_UNLOCK_ULAW, req->audio_lock, NULL);
    }
	 ...
	 else if (req->http_stream == URI_STREAM_MPEG4CIF_1 || req->http_stream == URI_STREAM_AVCCIF_1) {
        if (req->serial_lock > 0)
            GetAVData(AV_OP_UNLOCK_MP4_1_CIF, req->serial_lock, NULL);
        if (req->audio_lock > 0)
            GetAVData(AV_OP_UNLOCK_ULAW, req->audio_lock, NULL);
    }
	 else if (req->http_stream == URI_STREAM_MPEG4CIF_2 || req->http_stream == URI_STREAM_AVCCIF_2) {
        if (req->serial_lock > 0)
            GetAVData(AV_OP_UNLOCK_MP4_2_CIF, req->serial_lock, NULL);
        if (req->audio_lock > 0)
            GetAVData(AV_OP_UNLOCK_ULAW, req->audio_lock, NULL);
    }
	 else if (req->http_stream == URI_STREAM_MPEG4CIF_3 || req->http_stream == URI_STREAM_AVCCIF_3) {
        if (req->serial_lock > 0)
            GetAVData(AV_OP_UNLOCK_MP4_3_CIF, req->serial_lock, NULL);
        if (req->audio_lock > 0)
            GetAVData(AV_OP_UNLOCK_ULAW, req->audio_lock, NULL);
    }
    else {
...
int process_header_end(request * req,node_t * node)
{
	...
	//fprintf(stderr,"\r\nRequest to IPNC:%s, query_string: %s\n",req->request_uri,req->query_string);
    /* clean pathname */
    ...
    if (req->http_uri) {
		if((strcmp(req->request_uri,"/")!= 0) && (strcmp(req->request_uri,"/gIPNC-ro.gz")!= 0) && (strcmp(req->request_uri,"/IPNCWebDlls.CAB")!= 0) &&
			(strcmp(req->request_uri,"/sdget.htm")!= 0) && (strcmp(req->request_uri,"/sysget.htm")!= 0) && (strcmp(req->request_uri,"/accessget.htm")!= 0)
			&& (strcmp(req->request_uri,"/actvlistget.htm")!= 0) && (strcmp(req->request_uri,"/paraget.htm")!= 0)
			&& (strcmp(req->request_uri,"/paradefault.htm")!= 0)&& (strcmp(req->request_uri,"/actvlistclear.htm")!= 0)){
     ...
     if (req->psia_uri) {                      /*PSIA Handler Initialization*/
	    int ret,inlen,content_length;
		int bytes_to_write,bytes_written,i;
		char inbuf[RDBFLEN];
		char *ch;
