...
static void set_state(State st, int reset_counter, uint32_t address) {
    ...
    if (st == state && !reset_counter) {
        n_iteration++;
        DBG_PRT(daemon_log(LOG_DEBUG, "State iteration %s-%i", state_table[state], n_iteration));
    } else {
        DBG_PRT(daemon_log(LOG_DEBUG, "State transition %s-%i -> %s-0", state_table[state], n_iteration, state_table[st]));
        state = st;
        n_iteration = 0;
    }
...
open_socket(int iface, uint8_t *hw_address)
{
    ...
    DBG_PRT(daemon_log(LOG_DEBUG, "Using pcap filter '%s'", filter));
...
static int loop(int iface, uint32_t addr) {
    ...
    for (;;) {
        ...
        if (state == STATE_START) {
            ...
            if (packet_parse(in_packet, in_packet_len, &info) < 0)
                ...
                if (conflict) {
                    ...
                    DBG_PRT(daemon_log(LOG_DEBUG, "Ignoring irrelevant ARP packet."));
            }
        ...
        if (out_packet) {
            DBG_PRT(daemon_log(LOG_DEBUG, "sending..."));
        ...
        DBG_PRT(daemon_log(LOG_DEBUG, "sleeping %ims", timeout));
