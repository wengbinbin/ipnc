 ...
 * @param bIsAudio [I ] Has audio or not.
 ...
 */
static int Appro_avi_save(char *sAviName, int nInterval, int nFrameRate, int bIsAudio, int nMsgId, int bIsChkCard, int nStreamId)
{
    char cmd[80];
    int ret;
    struct timeval first, second, lapsed;
    sprintf(cmd, "./Appro_avi_save %s %d %d %d %d %d %d\n", sAviName, nInterval, nMsgId, nFrameRate, bIsAudio, bIsChkCard, nStreamId);
    AVI_DBG("%s\n", cmd);
...
int GenerateMpeg4FileName(char *strFileName)
{
    ...
    return sprintf(strFileName, "/%04d%02d%02d%02d%02d%02d.avi", tmnow->tm_year + 1900, tmnow->tm_mon + 1, tmnow->tm_mday, tmnow->tm_hour, tmnow->tm_min, tmnow->tm_sec);
}
...
int AviSave(int nDstFlg)
{
    ...
    int AVIType, supportMpeg4, supportMpeg4cif, supportH264, supportH264_1, supportH264_2, supportH264_3, supportH264cif, i, j;
    ...
    if (pSysInfo->lan_config.aviformat == 0)
    {
        ...
        {
            nFrameRate = pSysInfo->lan_config.chipConfig ? enc_framerate_night_1_dm368[i][j][pSysInfo->lan_config.nFrameRate1] / 1000: enc_framerate_night_1_dm365[i][j][pSysInfo->lan_config.nFrameRate1] / 1000;
        }
        else
        {
            nFrameRate = pSysInfo->lan_config.chipConfig ? enc_framerate_day_1_dm368[i][j][pSysInfo->lan_config.nFrameRate1] / 1000: enc_framerate_day_1_dm365[i][j][pSysInfo->lan_config.nFrameRate1] / 1000;
        }
    }
    else
    {
        ...
        if (pSysInfo->lan_config.nVideocodecmode > 7)
        {
            if (pSysInfo->lan_config.nDayNight == 0)
            {
                nFrameRate = pSysInfo->lan_config.chipConfig ? enc_framerate_night_3_dm368[i][j][pSysInfo->lan_config.nFrameRate3] / 1000: enc_framerate_night_3_dm365[i][j][pSysInfo->lan_config.nFrameRate3] / 1000;
            }
            else
            {
                nFrameRate = pSysInfo->lan_config.chipConfig ? enc_framerate_day_3_dm368[i][j][pSysInfo->lan_config.nFrameRate3] / 1000: enc_framerate_day_3_dm365[i][j][pSysInfo->lan_config.nFrameRate3] / 1000;
            }
        }
        else
        {
            if (pSysInfo->lan_config.nDayNight == 0)
            {
                nFrameRate = pSysInfo->lan_config.chipConfig ? enc_framerate_night_2_dm368[i][j][pSysInfo->lan_config.nFrameRate2] / 1000: enc_framerate_night_2_dm365[i][j][pSysInfo->lan_config.nFrameRate2] / 1000;
            }
            else
            {
                nFrameRate = pSysInfo->lan_config.chipConfig ? enc_framerate_day_2_dm368[i][j][pSysInfo->lan_config.nFrameRate2] / 1000: enc_framerate_day_2_dm365[i][j][pSysInfo->lan_config.nFrameRate2] / 1000;
            }
        }
    }
    ...
    if (GetAviState(0) &AVI_NO_STOP)
    {
        ...
        //        Appro_avi_save(strFilename, 999999999, nFrameRate, 1, AVI_MSG_ID, 1, AVIType);
        while (Appro_avi_save(strFilename, 999999999, nFrameRate, pSysInfo->audio_config.audioenable, AVI_MSG_ID, 1, AVIType) == AVI_RUN_AGAIN)
        {
    ...
    else
    {
        if (gFileDispCount < FILE_DISP_THR_LIMIT)
        {
            ...
            if (0 == Appro_avi_save(strFilename, AVI_INTERVAL, nFrameRate, pSysInfo->audio_config.audioenable, AVI_MSG_ID, 0, AVIType))
            {
