 ...
 * @return 0 on success.
 */
int GenerateJpgConfigFile(char JpegFiles[JPEG_COUNT][JPG_PATH_LEN], char *pConfigFileName, int nJpgCount)
{
    int ret, i, fd, result;
    FILE *fp;
...
int GenerateJpegFileName(char *strFileName)
{
    ...
    sprintf(strFileName, "%04d%02d%02d%02d%02d%02d%03ld.jpg", tmnow->tm_year + 1900, tmnow->tm_mon + 1, tmnow->tm_mday, tmnow->tm_hour, tmnow->tm_min, tmnow->tm_sec, tv.tv_usec / 1000);
    return 0;
}
...
int GenerateTarFileName(char *strFileName)
{
    ...
    gettimeofday(&tv, NULL);
    tmnow = localtime(&tCurrentTime);
    sprintf(strFileName, "/%04d%02d%02d%02d%02d%02d%03ld.tar", tmnow->tm_year + 1900, tmnow->tm_mon + 1, tmnow->tm_mday, tmnow->tm_hour, tmnow->tm_min, tmnow->tm_sec, tv.tv_usec / 1000);
    return 0;
}
            ...
            else
            {
                ...
                *pDstFlg = nDstFlg;
                ...
                else
                {
                    pthread_mutex_lock(&gJpgMutex);
                    gJpgState &= (~JPG_STOP);
                    gJpgState |= (JPG_RUN | JPEG_CONTINUOUS_MODE | (nDstFlg &JPG_DST_MASK));
                    pthread_mutex_unlock(&gJpgMutex);
