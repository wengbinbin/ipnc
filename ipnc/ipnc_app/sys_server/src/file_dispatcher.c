...
int GetFileDispState(int nClrFlg)
{
    ...
    {
        if ((nClrFlg &1) && difftime(rawtime, gFileDispSetTime[i]) > CMD_LIVE_TIME)
        {
            nRealClrFlg |= (1 << i);
        }
    }
...
int RecordToSD(char *strFileName)
{
    ...
    {
        nSpaceLeft = (disk_statfs.f_bsize *disk_statfs.f_bfree);
    } FD_DBG("SD card free space:%lld bytes\n", nSpaceLeft);
