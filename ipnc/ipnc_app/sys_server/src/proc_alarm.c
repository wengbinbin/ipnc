...
void *ProcAlarmThread(void *arg)
{
    ...
    switch (pEvironment->msg_buf.event)
    {
        ...
        case ALARM_EVENT_SCHEDULE:
            {
                ...
                int supportJPG, supportH264, supportH264_1, supportH264_2, supportH264_3, supportH264cif, supportMpeg4, supportMpeg4cif, supportAVI;
                ...
                bIsAviRun = 0;
                sprintf(tLog.event, "Schedule");
                nflag = 0;
                if (pSysInfo->sdcard_config.sdinsert <= 1)
                {
                    if (supportAVI && pSysInfo->sdcard_config.sdrenable && isSDAVI == 0 && !gbSD_WriteProtect)
                    {
                        nflag |= AVI_TO_SD;
                        bIsAviRun = 1;
                    }
                ...
                nflag = 0;
                if (isSDAVI == 1 && pSysInfo->sdcard_config.sdrenable && !gbSD_WriteProtect)
                {
                    nflag |= JPG_TO_SD;
                }
                /* Megapixel JPEG mode */
        ...
        case ALARM_EVENT_SCHEDULE_END:
            {
                int isFTPAVI;
                int supportJPG = 0, supportH264 = 0, supportH264_1 = 0, supportH264_2 = 0, supportH264_3 = 0, supportH264cif = 0, supportMpeg4 = 0, supportMpeg4cif = 0, supportAVI = 0;
        ...
        default:
            ERR("unkown evnet\n");
            sprintf(tLog.event, "Unkown event");
            fSetSysLog(&tLog);
            status = PROC_FAIL;
            break;
    }

    ALARM_EXIT: free(pEvironment);
