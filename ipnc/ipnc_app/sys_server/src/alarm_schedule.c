...
#define ALARM_SCHEDULE_MASK (FLG_UI_EXT | FLG_UI_MOTION | FLG_UI_ETH)
...
int AddSchedule(ScheduleIndex_t index, struct tm *tmnow, SysInfo *pSysInfo)
{
    int aDurations[] =
    {
        10, 30, 60, 300, 600, NON_STOP_TIME
    };
    if (pSysInfo->lan_config.nAlarmDuration >= (sizeof(aDurations) / sizeof(aDurations[0])))
    {
        return  - 1;
    }
            ...
            if (count == 0)
            {
                ...
                if (nLastAlarmStatus != nAlarmStatus)
                {
                    ...
                    if (SemWait(hAlarmStatusSem) == 0)
                    {
                        ...
                        fSetAlarmStatus((pSysInfo->lan_config.alarmstatus &(~ALARM_SCHEDULE_MASK)) | temp);
