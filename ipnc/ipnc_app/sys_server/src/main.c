...
#define PTZCTRL_THR_CREATED    0x4
...
extern void *PTZCtrlThread(void *arg);
/**
...
int ProcSysMsg(SYS_MSG_BUF *pMsg)
{
    ...
    switch (pMsg->cmd)
    {
        ...
        case SYS_MSG_GET_HTTP_PORT:
            {
                ...
                printf("SYS_MSG_GET_HTTP_PORT,pMsg->length=%d,\n", sizeof(unsigned short));
        ...
        case SYS_MSG_SET_TIME:
            {
                struct tm time;
                ShareMemRead(pMsg->offset, &time, pMsg->length);
                if (SetTime(&time) != 0)
                {
                    printf("Fail at SYS_MSG_SET_TIME\n");
                    break;
                }
                ret = 1;
                break;
        ...
        case SYS_MSG_SET_GBCE:
            {
                unsigned char value;
                DBG("SYS_MSG_SET_GBCE\n");
                if (pMsg->length != sizeof(value))
                {
                    break;
                }
                ...
                if (SetGBCE(value) != 0)
                {
                    printf("Fail at SYS_MSG_SET_GBCE\n");
                    break;
                }
                ret = 1;
                break;
        ...
        case SYS_MSG_SET_VIDEOCODEC_MODE:
            {
                unsigned char value;
                DBG("SYS_MSG_SET_VIDEOCODEC_MODE\n");
                if (pMsg->length != sizeof(value))
                {
                    break;
                }
                ...
                int server_status = SetVideoCodecMode(value);

                if (server_status < 0)
                {
                    printf("\nSystemServer:Fail at SYS_MSG_SET_VIDEOCODEC_MODE\n");
                    break;
                }
                else if (server_status == 1)
                {
        ...
        case SYS_MSG_SET_TVCABLE:
            {
                unsigned char value;
                DBG("SYS_MSG_SET_TVCABLE\n");
                if (pMsg->length != sizeof(value))
                {
                    break;
                }
                ShareMemRead(pMsg->offset, &value, pMsg->length);
                if (SetTVcable(value) != 0)
                {
                    printf("Fail at SYS_MSG_SET_TVCABLE\n");
                    break;
                }
                ret = 1;
                break;
        ...
        case SYS_MSG_IPNCPTZ:
            {
                ...
                #if DEBUG==1025
                printf("************SYS_MSG_IPNCPTZ:Begin!!!\n");
                #endif
                printf("************SYS_MSG_IPNCPTZ:Begin!!!\n");
                #if DEBUG==1025
                printf("SYS_MSG_IPNCPTZ offset=%d, length=%d\n",pMsg->offset,pMsg->length);
                #endif

                ShareMemRead(pMsg->offset, &value, pMsg->length);

                #if DEBUG==1025
                printf("SYS_MSG_IPNCPTZ value=%s\n", value);
                #endif

                ptz_command= strtok(value, "@");

                #if DEBUG==1025
                printf("SYS_MSG_IPNCPTZ ptz_command=%s\n", ptz_command);
                #endif
                ...
                #if DEBUG==1025
                printf("************SYS_MSG_IPNCPTZ:ptz_command=%s,channel=%d\n", ptz_command, channel);
                #endif

                if (SetCamPtzWithCh(ptz_command, channel) != 0)
                {
                    printf("Fail at SYS_MSG_IPNCPTZ, cmd=%s, chl=%d\n",ptz_command, channel);
                }
        ...
        case SYS_MSG_ENALBE_RECCH:
            {
                unsigned char value;
                DBG("SYS_MSG_ENALBE_RECCH\n");
                if (pMsg->length != sizeof(value))
                {
                    break;
                }
        ...
        case SYS_MSG_GET_PBRECDAY:
            {
                unsigned char dayvalue[32];

                DBG("SYS_MSG_GET_PBRECDAY\n");
                printf("SYS_MSG_GET_PBRECDAY Begin!\n");
                if (pMsg->length != sizeof(dayvalue))
                {
                    break;
                }
                ShareMemRead(pMsg->offset, &dayvalue, pMsg->length);

                Avs_GetPbRecDayData(dayvalue);
                pData = dayvalue;
                ret = 2;
                break;
            }
        case SYS_MSG_GET_PBRECHOUR:
            {
                unsigned char hourvalue[24 *60+1];
                printf("SYS_MSG_GET_PBRECHOUR\n");
                printf("SYS_MSG_GET_PBRECDAY Begin pMsg->length :%d sizeof(value):%d!\n", pMsg->length, sizeof(hourvalue));
                if (pMsg->length != sizeof(hourvalue))
                {
                    break;
                }
                ShareMemRead(pMsg->offset, &hourvalue, pMsg->length);
                printf("value:%s!\n", hourvalue);
                Avs_GetPbRecHourData(hourvalue);

                pData = hourvalue;
                ret = 2;
                break;
            }
        case SYS_MSG_START_PLAYBACKCH:
            {
                unsigned char value[50];
                bzero(value, 50);
                DBG("SYS_MSG_START_PLAYBACKCH\n");

                ShareMemRead(pMsg->offset, &value, pMsg->length);
                printf("SYS_MSG_START_PLAYBACKCH:%s\n", value);
                Avs_StartPlaybackCh(value);

                ret = 1;
                break;
            }
        case SYS_MSG_STOP_PLAYBACKCH:
            {
                unsigned char *value = (unsigned char*)malloc(pMsg->length + 1);
                ;
                bzero(value, pMsg->length + 1);
                DBG("SYS_MSG_STOP_PLAYBACKCH\n");

                ShareMemRead(pMsg->offset, value, pMsg->length);

                Avs_StopPlaybackCh(value);
                printf("Avs_StopPlaybackCh Done!!\n");
                ret = 1;
                break;
            }
        case SYS_MSG_PAUSE_PLAYBACKCH:
            {
                printf("SYS_MSG_PAUSE_PLAYBACKCH pMsg->length=%d \n", pMsg->length);
                unsigned char *value = (unsigned char*)malloc(pMsg->length + 1);
                bzero(value, pMsg->length + 1);
                DBG("SYS_MSG_PAUSE_PLAYBACKCH\n");

                ShareMemRead(pMsg->offset, value, pMsg->length);
                printf("SYS_MSG_PAUSE_PLAYBACKCH value=%s!!\n", value);
                Avs_PausePlaybackCh(value);
                printf("Avs_PausePlaybackCh Done!!\n");
                ret = 1;
                break;
            }
        case SYS_MSG_RESUME_PLAYBACKCH:
            {
                unsigned char *value = (unsigned char*)malloc(pMsg->length + 1);
                ;
                bzero(value, pMsg->length + 1);
                DBG("SYS_MSG_RESUME_PLAYBACKCH\n");

                ShareMemRead(pMsg->offset, value, pMsg->length);

                Avs_ResumePlaybackCh(value);
                printf("Avs_ResumePlaybackCh Done!!\n");
                ret = 1;
                break;
            }
        ...
        case SYS_MSG_SET_PTZ:
            {
                unsigned char value[256];

                #if DEBUG==1
                printf("SYS_MSG_SET_PTZ\n");
                #endif
                
                ShareMemRead(pMsg->offset, &value, pMsg->length);
                if (SetPtzChl(value) != 0)
                {
                    printf("\nSystemServer:Fail at SYS_MSG_SET_PTZn");
                    break;
                }
                ret = 1;
                break;
        ...
        case SYS_MSG_SET_RS485_DEV_ADD:
            {
                unsigned char value;
                DBG("SYS_MSG_SET_RS485_DEV_ADD\n");
                if (pMsg->length != sizeof(value))
                {
                    break;
                }
                ShareMemRead(pMsg->offset, &value, pMsg->length);
                if (SetRS485PortDev_add(value) != 0)
                {
                    printf("\nSystemServer:Fail at SYS_MSG_SET_HTTPSPORT\n");
                    break;
                }
        ...
        case SYS_MSG_SET_RS485_DEV_DEL:
            {
                unsigned char value;
                DBG("SYS_MSG_SET_RS485_DEV_DEL\n");
                if (pMsg->length != sizeof(value))
                {
                    break;
                }
                ShareMemRead(pMsg->offset, &value, pMsg->length);
                if (SetRS485PortDev_del(value) != 0)
                {
                    printf("\nSystemServer:Fail at SYS_MSG_SET_HTTPSPORT\n");
                    break;
                }
        ...
        case SYS_MSG_SET_RS232:
            {
                unsigned char value[60];
                DBG("SYS_MSG_SET_RS232\n");

                ShareMemRead(pMsg->offset, &value, pMsg->length);
                if (SetRS232Port(value) != 0)
                {
                    printf("\nSystemServer:Fail at SYS_MSG_SET_RS485\n");
                    break;
                }
        ...
        case SYS_MSG_SET_RS232SPEED:
            {
                unsigned char value[10];
                DBG("SYS_MSG_SET_RS232SPEED\n");

                ShareMemRead(pMsg->offset, value, pMsg->length);
                if (SetRS232Speed(value) != 0)
                {
                    printf("\nSystemServer:Fail at SYS_MSG_SET_RS232SPEED\n");
                    break;
                }
                ret = 1;
                break;
            }
        case SYS_MSG_SET_RS232DATABIT:
            {
                unsigned char value[10];
                DBG("SYS_MSG_SET_RS232DATABIT\n");

                ShareMemRead(pMsg->offset, value, pMsg->length);
                if (SetRS232Databit(value) != 0)
                {
                    printf("\nSystemServer:Fail at SYS_MSG_SET_RS232DATABIT\n");
                    break;
                }
                ret = 1;
                break;
            }
        case SYS_MSG_SET_RS232STOPBIT:
            {
                unsigned char value[10];
                DBG("SYS_MSG_SET_RS232STOPBIT\n");

                ShareMemRead(pMsg->offset, value, pMsg->length);
                if (SetRS232Speed(value) != 0)
                {
                    printf("\nSystemServer:Fail at SYS_MSG_SET_RS232STOPBIT\n");
                    break;
                }
                ret = 1;
                break;
            }
        case SYS_MSG_SET_RS232PARITY:
            {
                unsigned char value[10];
                DBG("SYS_MSG_SET_RS232PARITY\n");

                ShareMemRead(pMsg->offset, value, pMsg->length);
                if (SetRS232Speed(value) != 0)
                {
                    printf("\nSystemServer:Fail at SYS_MSG_SET_RS232PARITY\n");
                    break;
                }
                ret = 1;
                break;
            }
        case SYS_MSG_SET_RS232STREAMCTRL:
            {
                unsigned char value[10];
                DBG("SYS_MSG_SET_RS232STREAMCTRL\n");

                ShareMemRead(pMsg->offset, value, pMsg->length);
                if (SetRS232Speed(value) != 0)
                {
                    printf("\nSystemServer:Fail at SYS_MSG_SET_RS232STREAMCTRL\n");
                    break;
                }
                ret = 1;
                break;
        ...
        case SYS_MSG_SET_PLC_TCP:
            {
                unsigned char value[256];

                #if DEBUG==1
                printf("SYS_MSG_SET_PLC_TCP\n");
                #endif
                
                ShareMemRead(pMsg->offset, &value, pMsg->length);
                if (SetPlcChlTcp(value) != 0)
                {
        ...
        case SYS_MSG_SET_PLC_SERIAL:
            {
                unsigned char value[256];

                #if DEBUG==1
                printf("SYS_MSG_SET_PLC_SERIAL\n");
                #endif
                
                ShareMemRead(pMsg->offset, &value, pMsg->length);
                if (SetPlcChlSerial(value) != 0)
                {
                    printf("\nSystemServer:Fail at SYS_MSG_SET_PLC_SERIAL\n");
                    break;
                }
                ret = 1;
                break;
        ...
        case SYS_MSG_SET_DARKBLANK:
            {
                unsigned char value;
                DBG("SYS_MSG_SET_DARKBLANK\n");
                if (pMsg->length != sizeof(value))
                {
                    break;
                }
                ShareMemRead(pMsg->offset, &value, pMsg->length);
                if (Avs_SetDarkBlank(value) != 0)
                {
                    printf("Fail at SYS_MSG_SET_DARKBLANK\n");
                    break;
                }
                ret = 1;
                break;
...
void CleanSysMsg()
{
    ...
    for (i = MSG_TYPE_START; i < MSG_TYPE_END; i++)
        while (msgrcv(GetSysMsgId(), &msgbuf, sizeof(msgbuf) - sizeof(long), i, IPC_NOWAIT) >= 0)
            ;
}
...
int main(int argc, char *argv[])
{
    ...
    printf("Share memory init success\n");
            ...
            #if DEBUG==1025
            printf("=======pTZCtrlTherad begining======");
            #endif

    if (pthread_create(&tPTZCtrlThread, NULL, PTZCtrlThread, NULL))
    {
        IsSysQuit = 1;
    ...
    if (bThreadStatus &PTZCTRL_THR_CREATED)
    {
        pthread_kill(tPTZCtrlThread, SIGINT);
        pthread_join(tPTZCtrlThread, NULL);
    }
    SystemCleanup();
    if (mid >= 0)
    {
