...
#define HIGH_BITRATE    4000000
#define NORMAL_BITRATE    2000000
#define LOW_BITRATE        1000000
...
static void RestartAll()
{
    ...
    tConfig.OsdTextEnable = pSysInfo->lan_config.osdwin;
    tConfig.nDisplay = pSysInfo->lan_config.nLocalDisplay;
    tConfig.histogram = pSysInfo->lan_config.histogram;
    tConfig.gbce = pSysInfo->lan_config.gbce;
    tConfig.OsdLogoEnable = pSysInfo->lan_config.osdwinnum;
    tConfig.nBitrate1 = pSysInfo->lan_config.nMpeg41bitrate;
    tConfig.nBitrate2 = pSysInfo->lan_config.nMpeg42bitrate;
    tConfig.bMotionEnable = pSysInfo->motion_config.motionenable;
    tConfig.bMotionCEnale = pSysInfo->motion_config.motioncenable;
    tConfig.MotionCValue = pSysInfo->motion_config.motioncvalue;
    tConfig.MotionLevel = pSysInfo->motion_config.motionlevel;
    tConfig.MotionBlock = NcharToInt((char*)pSysInfo->motion_config.motionblock, 3);
    tConfig.nOsdTextLen = strlen((char*)pSysInfo->lan_config.osdtext);
    memcpy(tConfig.osdtext, pSysInfo->lan_config.osdtext, tConfig.nOsdTextLen + 1);
...
static void *PollingThread(void *arg)
{
    ...
    while (1)
    {
        ...
        if (network_en && dhcp != pSysInfo->lan_config.net.dhcp_enable)
        {
            ...
            if (!dhcp)
            {
                if ((ip = net_get_ifaddr(ETH_NAME)) != pSysInfo->lan_config.net.ip.s_addr)
                {
                    if (net_set_ifaddr(ETH_NAME, pSysInfo->lan_config.net.ip.s_addr) < 0)
                    {
                        __E("Error on Set ip\n");
                ...
                if ((netmask = net_get_netmask(ETH_NAME)) != pSysInfo->lan_config.net.netmask.s_addr)
                {
                    /* set net mask */
                    if (net_set_netmask(ETH_NAME, pSysInfo->lan_config.net.netmask.s_addr) < 0)
                    {
                ...
                if ((gateway = net_get_gateway()) != pSysInfo->lan_config.net.gateway.s_addr)
                {
                    /* set gateway */
                    if (net_set_gateway(pSysInfo->lan_config.net.gateway.s_addr) < 0)
                    {
                ...
                if ((dns = net_get_dns()) != pSysInfo->lan_config.net.dns.s_addr)
                {
                    /* set DNS */
                    if (net_set_dns(inet_ntoa(pSysInfo->lan_config.net.dns)) < 0)
                    {
...
int SystemInit()
{
    ...
    SetTVcable(pSysInfo->lan_config.nTVcable);
...
{
    printf("GetHttpPort:GetSysInfo();Begin\n");
    SysInfo *pSysInfo = GetSysInfo();

    if (pSysInfo == NULL)
    {
        printf("GetHttpPort:pSysInfo == NULL\n");
        return NULL;
    }
...
int SetTime(struct tm *time)
{
    ...
    char cmd[80];
    printf("Start %s\n", __func__);
    sprintf(cmd, "hwclock --set --date=\"%d/%d/%d %d:%d:%d\"\n", 1+time->tm_mon, time->tm_mday, 1900+time->tm_year, time->tm_hour, time->tm_min, time->tm_sec);
    system(cmd);
    return 0;
}
...
int SetIp(in_addr_t ip)
{
    ...
    ret = fSetIp(Ip);
    WriteConfigFile();
    system("cp /tmp/NET.TXT /mnt/nand/cfg-network;sync\n");
    return ret;
}
...
int SetHttpPort(unsigned short port)
{
    ...
    if (port == 80 || port > 1024)
    {
        ...
        ret |= system("/opt/dvr/boa -c /mnt/nand &\n");
...
/**
 * @brief Set TV cable status
 *
 * @param value 1:Connected ; 0:Disconn
 * @return function to set TV cable status
 */
int SetTVcable(unsigned char value) // 1:Connected / 0:Disconn
{
    #if 0
    if (value == 0)
    {
        dm355_gio_write(GIO_TV_CONNECT, 1);
    }
    else
    {
        dm355_gio_write(GIO_TV_CONNECT, 0);
    }
    #endif
    return fSetTVcable(value);
}
...
/**
 * @brief Set video codec mode
 *
 * @param value video codec value
 * @return function to set video codec
 * @retval -1 failed to set video codec
 */
int SetVideoMode(unsigned char value)
{
    return fSetVideoMode(value);
}
...
int SetVideoCodecMode(unsigned char value)
{
    ...
    if ((value == 0) || (value == 1) || (value == 5) || (value == 6) || (value == 7))
    {
        SetFTPFileFormat(0);
        SetSDFileFormat(0);
        SetAttFileFormat(0);
    ...
    {
        if (fSetVideoCodecMode(value) == 0)
        {
            ResetCodecROIParam();
            pSysInfo->face_config.resetROI = 1;
            SetRestart();
            return 0;
        }
        else
...
int SetVideoCodecCombo(unsigned char value)
{
    ...
    if (fSetVideoCodecCombo(value) == 0)
    {
        SetVideoCodecMode(value);
        return 0;
    }
    else
    {
...
/**
 * @brief Set video codec resolution
 *
 * @param value value of video codec resolution
 * @return function to set video codec resolution
 * @retval -1 failed to set video codec resolution
 */
...
int SetImageParaDefault(unsigned char value)
{
    ...
    {
        __E("SysCtrl: SetImageDefault called with invalid param\n");
    }
    return 0;
}
...
int AddUser(Acount_t *acout)
{
    ...
    for (i = 0; i < ACOUNT_NUM; i++)
    {
        ...
        if (strlen(pSysInfo->acounts[i].user) && strcmp(acout->user, pSysInfo->acounts[i].user))
        {
            continue;
        }
        if (i == 0)
        {
            ...
            memcpy(&admin_acount.password, acout->password, sizeof(admin_acount.password));
...
int CheckLogin(int nIndex, char *strId, char *pIpStr)
{
    LogEntry_t tLog;
    time_t rawtime;
    static time_t aLastLoginTime[ACOUNT_NUM];
    time(&rawtime);
...
int DoLogin(login_data_t *pPrm)
{
    ...
    {
        #if DEBUG==1
        printf("pPrm->id,=%s pSysInfo->acounts[%d].user=%s\n", pPrm->id, i, pSysInfo->acounts[i].user);
        #endif
        ...
        #if DEBUG==1
        printf("pPrm->password=%s, pSysInfo->acounts[%d].password=%s\n", pPrm->id, i, pSysInfo->acounts[i].user);
        #endif
...
int UpdateEnvFromFile(SysInfo *pSysInfo)
{
    ...
    {
        if (ReadLocalIp(&ip, fp) == 0 && ReadNetMask(&netmask, fp) == 0 && ReadGateway(&gateway, fp) == 0 && ReadDNS(&dns, fp) == 0)
        {
...
{
    return fSetRS485PortDev_add(value);
}

int SetRS485PortDev_del(unsigned char value)
{
    return fSetRS485PortDev_del(value);
}
...
int SetRS232Port(unsigned char *value)
{
    ...
    vl_SetSpeed(rs232Ch, speed);
    vl_Set_data_stop_parity(0, databit, stopbit, parity);

    return fSetRS232Port(value);
}
...
int SetRS232Databit(unsigned char *value)
{
    ...
    vl_SetDatabit(rs232ch, databit);
    return fSetRS232Databit(value);
}
...
int SetRS232Stopbit(unsigned char *value)
{
    ...
    vl_SetStopbit(rs232ch, stopbit);
    return fSetRS232Stopbit(value);
}
...
int SetRS232Parity(unsigned char *value)
{
    ...
    vl_SetParity(rs232ch, parity);
    return fSetRS232Parity(value);
}
...
int SetRS232StreamCtrl(unsigned char *value)
{
    ...
    vl_SetStreamCtrl(rs232ch, streamctrl);
    return fSetRS232StreamCtrl(value);
}
#endif

int SetPlcChlTcp(unsigned char *value)
{
    return fSetPlcChlTcp(value);
}

//***********add by wbb**************//
int SetPresetNote(unsigned char *value)
{
    return fSetPresetNote(value);
}


int SetPlcChlSerial(unsigned char *value)
{
    return fSetPlcChlSerial(value);
}
