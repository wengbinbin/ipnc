...
int DRV_imgsOpen(DRV_ImgsConfig *config)
{
  ...
  DRV_imgsCalcFrameTime(config->fps, width, height, config->binEnable);//add by sxh
  ...
  do {
    status = DRV_imgsCheckId(); //add by sxh
    if(status==OSA_SOK)
      break;
    OSA_waitMsecs(10);
  } while(retry--);
...
/*
  ...
  regAddr[i]  = 0x29;  regValue[i] = 0x481;  	i++;
	...
	regAddr[i]  = 0x5D;  regValue[i] = 0x1313;  i++;
...
/////////////////add by sxh/////////////////////////////////
	int i;
	Uint8 regAddr_reset[11];
	Uint8 regValue_reset[11];
	i=0;
	regAddr_reset[i]  = 0xE8;  regValue_reset[i] = 0x02;  i++;
	regAddr_reset[i]  = 0xE9;  regValue_reset[i] = 0x00;  i++;
	regAddr_reset[i]  = 0xEA;  regValue_reset[i] = 0x80;  i++;
	regAddr_reset[i]  = 0xE0;  regValue_reset[i] = 0x01;  i++;

	regAddr_reset[i]  = 0xE8;  regValue_reset[i] = 0x60;  i++;
	regAddr_reset[i]  = 0xE9;  regValue_reset[i] = 0x00;  i++;
	regAddr_reset[i]  = 0xEA;  regValue_reset[i] = 0xB0;  i++;
	regAddr_reset[i]  = 0xE0;  regValue_reset[i] = 0x01;  i++;
	regAddr_reset[i]  = 0xE0;  regValue_reset[i] = 0x00;  i++;
	regAddr_reset[i]  = 0x03;  regValue_reset[i] = 0x01;  i++;
	regAddr_reset[i]  = 0x03;  regValue_reset[i] = 0x00;  i++;
	
	DRV_i2cWrite8(&gDRV_imgsObj.i2cHndl, regAddr_reset, regValue_reset, i); 
	
	i=0;
	Uint8 regAddr[3];
	Uint8 regValue[3];
	int status;
	regAddr[i]  = LUMINANCE3;  regValue[i] = 0x00; 	i++;
	regAddr[i]	= CHROMINANCE2;  regValue[i] = 0x04;	i++;
	regAddr[i]	= OUTPUTFORMATER2;  regValue[i] = 0x11;	i++;
...
int DRV_imgsSetFramerate(int fps)
{
  ...
  status = OSA_SOK;//DRV_i2cWrite16(&gDRV_imgsObj.i2cHndl, regAddr, regValue, i); add by sxh
  if(status!=OSA_SOK) {
    OSA_ERROR("DRV_i2cWrite16()\n");
    return status;
  }
...
int DRV_imgsSetAgain(int again, int setRegDirect)
{
  ...
  status = OSA_SOK;//DRV_i2cWrite16(&gDRV_imgsObj.i2cHndl, &regAddr, &regValue, 1);// add by sxh
...
int DRV_imgsGetAgain(int *again)
{
  ...
  status = OSA_SOK;//DRV_i2cRead16(&gDRV_imgsObj.i2cHndl, &regAddr, &regValue, 1); add by sxh
...
int DRV_imgsGetEshutter(Uint32 *eshutterInUsec)
{
  ...
  //*eshutterInUsec = DRV_imgsInvertSW(regValue); add by sxh
...
int DRV_imgsEnable(Bool enable)
{
  ...
  DRV_ImgsFrameTime *pFrame = &gDRV_imgsObj.curFrameTime;
	OSA_ERROR("in DRV_imageEnable\n");// add by sxh
  if(enable) {
    status = OSA_SOK;//DRV_imgsSetRegs(); add by sxh
    if(status!=OSA_SOK) {
      OSA_ERROR("DRV_imgsSetRegs()\n");
      return status;
    }
    //DRV_imgsSpecificSetting(); // add by sxh
  }
...
int DRV_imgsCheckId()
{
  ...
  if(regValue!=IMGS_CHIP_ID)
    return OSA_SOK;//OSA_EFAIL;//add by sxh
