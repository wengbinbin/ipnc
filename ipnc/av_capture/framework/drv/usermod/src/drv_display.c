...
int DRV_displayOpen(DRV_DisplayWinHndl *hndl, DRV_DisplayWinConfig *config)
{
  ...
  DRV_displaySetWinSize(hndl, config->width, config->height);
...
int DRV_displayPutBuf(DRV_DisplayWinHndl *hndl, int bufId)
{
	struct v4l2_buffer buf;
	int ret;
  DRV_DisplayObj *pObj;
...
int DRV_displaySwitchBuf(DRV_DisplayWinHndl *hndl, int *bufId)
{
  int status;
  DRV_DisplayObj *pObj;
  ...
  status = DRV_displayPutBuf(hndl, *bufId);
  if(status==OSA_SOK) {
...
int DRV_displayExpand(DRV_DisplayWinHndl *hndl, Uint16 expandH, Uint16 expandV)
{
  ...
  if(expandH) {
    crop.c.width = pObj->bufInfo.width*9/8;
  }else {
    crop.c.width = pObj->bufInfo.width;
  }
  if(expandV) {
    crop.c.height = pObj->bufInfo.height*6/5;
  }else {
    crop.c.height = pObj->bufInfo.height;
  }
...
int DRV_displayExpandEx(DRV_DisplayWinHndl *hndl,DRV_DisplayWinConfig *config)
{
  ...
  memset(&crop, 0, sizeof(crop));
  crop.type = V4L2_BUF_TYPE_VIDEO_OUTPUT;
  crop.c.top = config->startY;
  crop.c.left = config->startX;
  if(config->expandH) {
    crop.c.width = config->width*9/8;
  }else {
    crop.c.width = config->width;
  }
  if(config->expandV) {
    crop.c.height = config->height*6/5;
  }else {
    crop.c.height = config->height;
  }
  ret = ioctl(pObj->fd, VIDIOC_S_CROP, &crop);
  if(ret < 0) {
    OSA_ERROR("VIDIOC_S_CROP ioctl\n");
...
int DRV_displayTestMain(int argc, char **argv)
{
  ...
  status = OSA_fileReadFile(filename, gDRV_displayInfo.virtAddr[gDRV_displayBufId], gDRV_displayInfo.bufSize, 0,&readSize);
  if(status==OSA_SOK) {
    ...
    while(!gDRV_displayQuit) {
      DRV_displaySwitchBuf( &gDRV_displayHndl, &gDRV_displayBufId);
    }
  }
