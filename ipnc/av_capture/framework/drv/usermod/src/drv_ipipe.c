...
int DRV_ipipeOpen(DRV_IpipeConfig *config)
{
  ...
  if(config->bscNumVectorsRow > 0 && config->bscNumVectorsCol > 0) {
    ...
    gDRV_ipipeObj.bscBufBaseVirtAddr = OSA_cmemAllocCached(DRV_IPIPE_BSC_BUFFER_MAX*gDRV_ipipeObj.numBufBsc, 32);
  ...
  if(gDRV_ipipeObj.histResultBuf != NULL)
    OSA_cmemFree(gDRV_ipipeObj.histResultBuf);
...
*/ //add by sxh csl
  for(i=0; i<CSL_RSZ_CH_MAX; i++) {
    ...
    CSL_rszBufSwitchEnable(&gCSL_rszHndl, i, rszOutEnable);
  }
...
int DRV_ipipeRszSetParams()
{
  ...
  gDRV_ipipeObj.rszInConfig.inputDataPath      = CSL_RSZ_INPUT_DATA_PATH_IPIPEIF;//CSL_RSZ_INPUT_DATA_PATH_IPIPE;//add by sxh 1115
...
int DRV_ipipeGetHistogram (){ //ANR-Hist
  int status = OSA_SOK;
  Uint32 * table = NULL;
  ...
  if(status == OSA_SOK){
    ...
    status = CSL_ipipeHistogramReadResults(&gCSL_ipipeHndl, table);//, offset, size); //add by sxh csl
   }
...
int DRV_ipipeCpyHistData(Uint32* histData, Uint8* histStatus){  //ANR-Hist
