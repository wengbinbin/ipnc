...
int DRV_captureStart(Bool rawOutEnable, Bool yuvOutEnable)
{
  ...
  DRV_imgsEnable(TRUE);
  #ifdef DRV_SHOW_INFO
  OSA_printf(" CAPTURE: after imgsEnable.\n"); //add by sxh  
  #endif
  DRV_isifWaitInt(DRV_ISIF_INT_VD0, 2);
  #ifdef DRV_SHOW_INFO
  OSA_printf(" CAPTURE: after isifWaitInt.\n"); //add by sxh
  #endif
	...
	DRV_h3aEnable(DRV_H3A_MOD_AEWB,TRUE);//add by sxh 1115 back
  DRV_h3aEnable(DRV_H3A_MOD_AF, TRUE);//add by sxh 1115 back
  #ifdef DRV_SHOW_INFO
