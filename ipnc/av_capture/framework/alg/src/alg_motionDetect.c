...
static int SensitiveCount  = 0;
static int SensitiveForH264  = 0;
...
int ALG_motionDetectCalc(ALG_MotionObj  *pObj)
{
	...
	//OSA_printf("ALG MOTION DETECT: %d %d \n",pObj->warning_count, pObj->threshold);
...
int ALG_motionDetectStart(ALG_MotionObj  *pObj)
{
	...
	for(i=0;(i<ALG_MOTION_DETECT_MAX_VERT_REGIONS) || (pObj->win_height*i <= pObj->frame_height);i++)
	{
		for(j=0;(j<ALG_MOTION_DETECT_MAX_HORZ_REGIONS) || (pObj->win_width*j <= pObj->frame_width);j++)
	   {
			pObj->MvDataOffset += pObj->win_height*i;
			if(pObj->runPrm.windowEnableMap[j][i] == 1) //1- True , 0 - False
			{
				pObj->MvDataOffset += pObj->win_width*j;
