...
int ALG_facialFeatureGetMem(){
	...
	ffMemBuff.ptRecBuff = (void *)OSA_cmemAlloc(szMemSize.nPtRec,32);
	...
	ffMemBuff.ptAlgBuff[0]= (void *)OSA_cmemAlloc(szMemSize.facePTRecSize[0], 32);
	ffMemBuff.ptAlgBuff[1]= (void *)OSA_cmemAlloc(szMemSize.facePTRecSize[1], 32);
	ffMemBuff.ptAlgBuff[2]= (void *)OSA_cmemAlloc(szMemSize.facePTRecSize[2], 32);

	/* Facial Feature */
    szMemSize.nFrRec = ALG_fffr_GetMemRec( &val);
    if(ffMemBuff.frRecBuff != NULL)
		OSA_cmemFree(ffMemBuff.frRecBuff);
	ffMemBuff.frRecBuff = (void *)OSA_cmemAlloc(szMemSize.nFrRec,32);
	...
	ffMemBuff.frAlgBuff[0]= (void *)OSA_cmemAlloc(szMemSize.faceFRRecSize[0], 32);
	ffMemBuff.frAlgBuff[1]= (void *)OSA_cmemAlloc(szMemSize.faceFRRecSize[1], 32);
	ffMemBuff.frAlgBuff[2]= (void *)OSA_cmemAlloc(szMemSize.faceFRRecSize[2], 32);

    /* For Face Album data */
    if(ffMemBuff.albumBuff != NULL)
		OSA_cmemFree(ffMemBuff.albumBuff);
	ffMemBuff.albumBuff= (unsigned int*)OSA_cmemAlloc(FR_ALBUM_SIZE, 32);

    /* For image data */
    if(ffMemBuff.imageDataBuff!= NULL)
		OSA_cmemFree(ffMemBuff.imageDataBuff);
	ffMemBuff.imageDataBuff = (void *)OSA_cmemAlloc(FR_IMAGE_SIZE, 32);
...
int ALG_faceTrackerOpen(void)
{
	...
	Face_track_buffer = OSA_cmemAlloc(MAX_FACETRACKER_FRAMEBUF, 32);
	gFacetracker = OSA_cmemAlloc(sizeof(Facetrackerstruct), 32);
...
// be active for up to 3 seconds if no face is detected
    gFacetracker->maxNumOfTrackingFrames= 90;
...
// For instance, if expandRoiTop= 3, it means the amount of expansion to the left is 3/8 of the face detector box's height
    ...
    gFacetracker->expandRoiTop= 5;
    gFacetracker->expandRoiBottom= 2;
