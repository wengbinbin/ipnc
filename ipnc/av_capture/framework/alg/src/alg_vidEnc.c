...
void *ALG_vidEncCreate(ALG_VidEncCreate *create)
{
  ...
  else if(create->codec==ALG_VID_CODEC_H264)
  {
	  ...
	  pObj->params_h264.enableDDRbuff 	= (create->vnfEnable) ? 1 : 0;
	  ...
	  if(create->encodePreset == ALG_VID_ENC_PRESET_AUTO)
	  {
  ...
  pObj->createPrm.framerate			 = 30000;
...
int ALG_vidEncSetDynamicParams(ALG_VidEncObj *pObj)
{
  ...
  switch(pObj->createPrm.codec) {
    case ALG_VID_CODEC_H264:
	  ...
	  pObj->dynamicParams_h264.perceptualRC                   = 1;//(pObj->createPrm.mode == ALG_VID_ENC_MODE_FAST_FPS) ? 0:1;
      ...
      pObj->dynamicParams_h264.rcQMaxI						  = (pObj->createPrm.qpmax>51)?51:pObj->createPrm.qpmax;
  	  ...
  	  pObj->dynamicParams_h264.enablePicTimSEI				  = 0;  	/*!< Enable Picture Timing SEI */
    ...
    case ALG_VID_CODEC_MPEG4:
	  ...
	  if(pObj->createPrm.encodePreset == ALG_VID_ENC_PRESET_AUTO)
	  {
		  ...
		  if(pObj->createPrm.width<720) // for QVGA less than D1
			pObj->dynamicParams_mpeg4.meAlgo		= IMP4VENC_ME_MQ_MP;
		  else if(pObj->createPrm.width==720) // for D1
			pObj->dynamicParams_mpeg4.meAlgo		= IMP4VENC_ME_HQ_LP;
		  else if(pObj->createPrm.width==1280) // for 720P or SXGA
...
/*
  if(prm->snapEnable) {
  	OSA_fileWriteVideoData(prm->snapFilename, inBufDesc.bufDesc[0].buf, inBufDesc.bufDesc[1].buf,
...
*/
  if( pObj->MV_Size == 0 )
  {
	...
	//OSA_printf("\n outArgs.bytesGenerated=%x  \n",outArgs.bytesGenerated);//add by sxh
