...
int ALG_swosdDrawHist( ALG_SwosdRunPrm *prm){  //ANR -Hist
  ...
  ALG_SwosdLineInfo lineInfo;
  Uint32 i, histHeight = 0, histvalue;
  Uint32 scale_value = 1, clip_value=prm->videoHeight>>2;
  Uint32 max_histvalue = prm->histData[0]>>4;
...
#if 0
  for(i=0; i<256; i++){
    histvalue = prm->histData[i]>>4;
  	if(histvalue > max_histvalue)
  		max_histvalue = histvalue;
  }

  scale_value = prm->videoHeight/max_histvalue;
#endif
