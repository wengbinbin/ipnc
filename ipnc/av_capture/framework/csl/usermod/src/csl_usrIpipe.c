...
CSL_Status CSL_ipipeHistogramReadResults(CSL_IpipeHandle hndl, Uint32 * table)
{
  Uint8   status;

  status = CSL_usrDrvIoctl(hndl->fd, CSL_IPIPE_CMD_HISTOGRAM_READ_RESULTS, table);
...
{
  CSL_IpipeBscBufGetFullPrm bscBufGetFullPrm;

  bscBufGetFullPrm.buf = buf;
  bscBufGetFullPrm.minBuf = minBuf;
  bscBufGetFullPrm.timeout = timeout;

  return CSL_usrDrvIoctl(hndl->fd, CSL_IPIPE_CMD_BSC_BUF_GET_FULL, &bscBufGetFullPrm);
}
