...
CSL_Status CSL_vpssHwControl(CSL_VpssHandle hndl, Uint32 cmd, void *prm)
{
  ...
  case CSL_VPSS_CMD_GET_MISC:
  	
  	status = CSL_vpssGetReg(hndl, (Uint32)0x01c40038, &regValues, 1);
  	
  	if(status==CSL_SOK)
  	  status = CSL_copyToUser(prm, &regValues, sizeof(regValues));
