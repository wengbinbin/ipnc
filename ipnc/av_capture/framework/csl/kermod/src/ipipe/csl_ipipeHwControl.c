...
CSL_Status CSL_ipipeHwControl(CSL_IpipeHandle hndl, Uint32 cmd, void *prm)
{
  ...
  static CSL_IpipeBscBufGetFullPrm bscBufGetFullPrm;
  ...
  case CSL_IPIPE_CMD_HISTOGRAM_READ_RESULTS:
    ...
    if(status==CSL_SOK)
      status = CSL_ipipeHistogramReadResults(hndl, tableAddr);
  ...
  case CSL_IPIPE_CMD_BSC_BUF_GET_FULL:
    ...
    if (status == CSL_SOK)
      status = CSL_copyFromUser(&bscBufGetFullPrm, prm, sizeof(bscBufGetFullPrm));

    if (status == CSL_SOK)
      status = CSL_ipipeBscBufGetFull(hndl, &buf, bscBufGetFullPrm.minBuf, bscBufGetFullPrm.timeout);

    status = CSL_copyToUser(bscBufGetFullPrm.buf, &buf, sizeof(buf));
