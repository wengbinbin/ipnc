...
CSL_IntRet_t CSL_ipipeDoneIsr(int intId, void *prm, void *reserved)
{
...
CSL_IntRet_t CSL_ipipeIsr(int intId, void *prm, void *reserved)
{
  ...
  if (intId == CSL_SYS_INT_IPIPE)
    intType = CSL_IPIPE_INT_TYPE_IPIPE;
  else if (intId == CSL_SYS_INT_IPIPE_BSC)
    intType = CSL_IPIPE_INT_TYPE_BSC;
  else
    return CSL_INT_RET_SOK;

  if (intType == CSL_IPIPE_INT_TYPE_IPIPE) {
    CSL_ipipeDoneIsr(intId, prm, reserved);
