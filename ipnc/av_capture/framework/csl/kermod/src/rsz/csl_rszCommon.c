...
CSL_Status CSL_rszASetOutConfig(CSL_RszHandle hndl, CSL_RszOutConfig * data)
{
  ...
  CSL_FINS(hndl->regs->RZA_H_TYP, RSZ_RZA_H_TYP_Y, data->horzReszTypeY);
  CSL_FINS(hndl->regs->RZA_H_TYP, RSZ_RZA_H_TYP_C, data->horzReszTypeC);

  CSL_FINS(hndl->regs->RZA_H_LPF, RSZ_RZA_H_LPF_C, data->horzLpfIntensityY);
  CSL_FINS(hndl->regs->RZA_H_LPF, RSZ_RZA_H_LPF_C, data->horzLpfIntensityC);
...
CSL_Status CSL_rszSetOutConfig(CSL_RszHandle hndl, Uint8 rszMod, CSL_RszOutConfig * data)
{
  ...
  hndl->flipH[rszMod] = data->flipH;
  hndl->flipV[rszMod] = data->flipV;
...
{
  unsigned long oldIntState;
  
  if (hndl == NULL || dzoomConfig==NULL)
    return CSL_EFAIL;

  CSL_intGlobalDisable(oldIntState);
  ...
  CSL_intGlobalRestore(oldIntState);
