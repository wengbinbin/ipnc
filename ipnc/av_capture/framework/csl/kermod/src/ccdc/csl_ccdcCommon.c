...
CSL_Status CSL_ccdcSetClampConfig(CSL_CcdcHandle hndl, CSL_CcdcClampConfig * data)
{
  ...
  CSL_FINS(hndl->regs->CLVWIN0, ISIF_CLVWIN1_CLVSH, data->vertClampWinStartH);
...
CSL_Status CSL_ccdcSetLscConfig(CSL_CcdcHandle hndl, CSL_CcdcLscConfig * data)
{
  ...
  CSL_FINS(hndl->regs->DLSCCFG, ISIF_DLSCCFG_ENABLE, 0); // keep LSC disabled

  CSL_FINS(hndl->regs->LSCHVAL, ISIF_LSCHVAL_HVAL, data->inWidth - 1);
  CSL_FINS(hndl->regs->LSCVVAL, ISIF_LSCVVAL_VVAL, data->inHeight - 1);
  ...
  addr = (Uint32) data->gainTableAddr;
  ...
  addr = (Uint32) data->offsetTableAddr;
  ...
  CSL_FINS(hndl->regs->DLSCGROF, ISIF_DLSCOROF_OFFSET, data->offsetTableLineOffset);
  ...
  CSL_FINS(hndl->regs->DLSCCFG, ISIF_DLSCCFG_ENABLE, data->enable);
