...
//#define __MEM_MNG_DEBUG //add by sxh 
...
MEM_LAYOUT mem_layout_setting[MEM_LAYOUT_NUM] = {
	...
	[1] = {
					.profiles[VIDOE_INFO_MJPG] =
					{
						.cache_size 		= 0,//0x100000, //add by sxh channel
						.cache_blk_size 	= 1,//50*1024,
						.mem_size 			= 0,//0x100000,
						.mem_blk_size 		= 1,//50*1024,
						.ext_size 			= 0,
					},
					.profiles[VIDOE_INFO_MP4] =
					{
						.cache_size 		= 0x500000,//0x500000,
						.cache_blk_size 	= 30*1024,//30*1024,
						.mem_size 			= 0x500000,//0x500000,
						.mem_blk_size 		= 30*1024,//30*1024,
						.ext_size 			= MP4_1_EXTRA_SIZE,//MP4_1_EXTRA_SIZE,
					},
					.profiles[VIDOE_INFO_MP4_1] =
					{
						.cache_size 		= 0x500000,//0x500000,
						.cache_blk_size 	= 30*1024,//30*1024,
						.mem_size 			= 0x500000,//0x500000,
					...
					.profiles[VIDOE_INFO_MP4_EXT] =
					{
						.cache_size 		= 0,//0x500000,//add by sxh channel
						.cache_blk_size 	= 1,//30*1024,
						.mem_size 			= 0,//0x500000,
						.mem_blk_size 		= 1,//30*1024,
						.ext_size 			= 0,//MP4_2_EXTRA_SIZE,
					},
					.profiles[AUDIO_INFO_G711] =
					{
						.cache_size 		= AUDIO_CACHE_SIZE,
	...
	[2] = {
					...
					{
						.cache_size 		= 0xB00000,
						.cache_blk_size 	= 60*1024,
						.mem_size 			= 0xB00000,
						.mem_blk_size 		= 60*1024,
						.ext_size 			= MP4_1_EXTRA_SIZE,
					},
					.profiles[VIDOE_INFO_MP4_EXT] =
					{
...
{
	{
		...
		.size		= 0,//MJPG_MEM_SIZE,
		.extradata	= NULL,
		.extrasize	= 0,
		.IsCache	= 0,
		.blk_sz		= 1,//MJPG_BLK_SIZE,
		.blk_num	= 0,//(MJPG_MEM_SIZE/MJPG_BLK_SIZE),
		.blk_free	= 0,//(MJPG_MEM_SIZE/MJPG_BLK_SIZE),
		.frame_num	= 0,
		.cur_frame	= -1,
		.cur_serial = -1,
		.cur_blk	= -1,
	...
	{
		.video_type = VIDEO_MP4,
		.size		= 0,//MP4_EXT_MEM_SIZE,
		.extradata	= NULL,
		.extrasize	= 0,//MP4_2_EXTRA_SIZE,
		.IsCache	= 0,
		.blk_sz		= 1,//MP4_EXT_BLK_SIZE,
		.blk_num	= 0,//(MP4_EXT_MEM_SIZE/MP4_EXT_BLK_SIZE),
		.blk_free	= 0,//(MP4_EXT_MEM_SIZE/MP4_EXT_BLK_SIZE),
		.frame_num	= 0,
		.cur_frame	= -1,
		.cur_serial = -1,
		.cur_blk	= -1,
							...
							int frame_type, VIDEO_BLK_INFO *pVidInfo)
{
	...
	printf( "pVidInfo->cur_serial=%d \n",pVidInfo->cur_serial);
	printf( "frame_type=%d \n",frame_type);
	printf( "size=%d \n",size);
	printf( "pVidInfo->blk_num=%d \n",pVidInfo->blk_num);
	#endif
	if( pVidInfo == NULL ||
	...
	if( frame_type == I_FRAME && pVidInfo->video_type == VIDEO_MP4)
	{
		...
		//printf( "pframe_type == I_FRAME && pVidInfo->video_type == VIDEO_MP4 \n");
		#endif	
		if( pVidInfo->gop.last_Start < 0 )
		{
						...
						VIDEO_BLK_INFO *pVidInfo )
{
	...
	int blk_use = (size + pVidInfo->blk_sz-1) / pVidInfo->blk_sz;//the reason why plus pVidInfo->blk_sz-1 is that as 1 even not enough 1
	...
	{
		#ifdef VANLINK_DVR_DM365_DEBUG //add by sxh
		//OSA_printf("blk_use > pVidInfo->blk_num || size == 0 !!\n");    
		#endif
	...
	do{
		...
		if( blk_use > pVidInfo->blk_free )
		{
			...
			OSA_printf("size=%x,pVidInfo->blk_sz=%x,blk_use= %d!!\n",size,pVidInfo->blk_sz,blk_use);  
			OSA_printf("blk_use= %d!!\n",blk_use);  
			OSA_printf("Not enough free blk !!\n");    
			#endif
			if( MemMng_VidFrame_Free(  pVidInfo,  freeframe) < 0 )
			{
				#ifdef VANLINK_DVR_DM365_DEBUG //add by sxh
				OSA_printf("MemMng_VidFrame_Free Fail!!\n");    
				#endif
				goto MEM_MNG_WRITE_FAIL;
			}else{
				free_cnt++;
			...
			#ifdef VANLINK_DVR_DM365_DEBUG //add by sxh
				//OSA_printf("-----nextblk=%d!!\n",nextblk);    
			#endif
			if( (nextblk+ blk_use ) > pVidInfo->blk_num )
			{
				/* No Contiguous Blk */
				#ifdef VANLINK_DVR_DM365_DEBUG //add by sxh
				//OSA_printf("No Contiguous Blk!!\n");    
				#endif
...
{
	#ifdef VANLINK_DVR_DM365_DEBUG
	//printf( "+++++++++++GetLastI_Frame \n");
	#endif
	if(video_info == NULL){
		#ifdef VANLINK_DVR_DM365_DEBUG
		//printf( "+++++++++++video_info == NULL \n");
		#endif
		return NULL;
	}
	if(video_info->gop.lastest_I < 0){
		#ifdef VANLINK_DVR_DM365_DEBUG
		//printf( "+++++++++++video_info->gop.lastest_I =%d\n",video_info->gop.lastest_I);
		#endif
		return NULL;
