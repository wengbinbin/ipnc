...
void showUsage()
{
  ...
  OSA_printf(" SXGA         :1280x1024@30fps + 256x192@30fps capture mode\n");
  ...
  OSA_printf(" AUDIO        : AUDIO parameter Sample Rate to be followed after this (8000 or 16000)\n");
  ...
  OSA_printf(" GBCE         : Dynamic Range Enhancement is ENABLED\n");
  ...
  OSA_printf("\n");
  OSA_printf(" Example, \n");
  OSA_printf(" Example 1. # ./av_server.out DM365 AUDIO 8000 D1 AEWB TI2A H264 3000000 VBR AUTO RTSP\n");
  OSA_printf(" Example 2. # ./av_server.out DM365 AUDIO 8000 720P AEWB TI2A LDC TNF KNF FD OSD H264 3000000 VBR AUTO RTSP\n");
  OSA_printf("\n");
}
...
int main(int argc, char **argv)
{
  ...
  gAVSERVER_UI_config.mode = AVSERVER_UI_CAPTURE_MODE_D1;//AVSERVER_UI_CAPTURE_MODE_720P;// add by sxh 1115
  gAVSERVER_UI_config.ldcEnable = FALSE;
  gAVSERVER_UI_config.gbceEnable = FALSE;
  ...
  for(i=1; i< argc; i++) {
    ...
    if(strcmp(argv[i], "SXGA")==0)
      gAVSERVER_UI_config.mode = AVSERVER_UI_CAPTURE_MODE_SXGA;
    else
    ...
    if(strcmp(argv[i], "GBCE")==0)
      gAVSERVER_UI_config.gbceEnable = TRUE;
    else
