...
int AVSERVER_gbceEnable(int value)
{
	gAVSERVER_config.gbceEnable = value;
}

int AVSERVER_swosdSetOsdType(int streamId, char ch)                             //Changes by Rajiv: SWOSD TEST
    {
         if(streamId < 0 || streamId >= gAVSERVER_config.numCaptureStream)
	    return OSA_EFAIL;
	 if(ch == 'b')
...
int AVSERVER_swosdSetDispString(int streamId)
{
        if(streamId < 0 || streamId >= gAVSERVER_config.numCaptureStream)
	    return OSA_EFAIL;
...
int AVSERVER_swosdSetStringUpdate(int streamId)
{
	   if(streamId < 0 || streamId >= gAVSERVER_config.numCaptureStream)
	return OSA_EFAIL;
...
int AVSERVER_getCaptureFrameRate(int encodeId, float *fr)                                           //changes by Rajiv: New api added for SWOSD
{
