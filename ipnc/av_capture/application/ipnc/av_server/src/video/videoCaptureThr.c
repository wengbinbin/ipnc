...
int VIDEO_captureTskCreate()
{
  ...
  captureConfig.h3aConfig.aewbVendor  = gAVSERVER_config.aewbVendor;
...
int VIDEO_captureTskRunDdrIn()
{
  ...
  if(pRawBufInfo!=NULL) {
    ...
    reszPrm.inType = DRV_DATA_FORMAT_YUV422;//DRV_DATA_FORMAT_RAW;//add by sxh 1115
    reszPrm.inVirtAddr = pRawBufInfo->virtAddr;
    reszPrm.inPhysAddr = pRawBufInfo->physAddr;
    reszPrm.inStartX = 0;
    reszPrm.inStartY = 1;
