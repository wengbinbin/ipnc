...
//#define OSA_CMEM_ALLOC OSA_cmemAlloc
#define AVSERVER_DEBUG_VIDEO_SWOSD_THR
#define OSA_CMEM_ALLOC OSA_cmemAllocCached
...
char  * vl_zhfontBuff16_16;//add by sxh 0417
char  * vl_enfontBuff8_16;//add by sxh 0417
...
char *strBuff480_Y[AVSERVER_MAX_STREAMS][MAX_STR_BUFF], *strBuff480_UV[AVSERVER_MAX_STREAMS][MAX_STR_BUFF];//add by sxh 0417
...
static FILE  *fpLogo;
char *logoBuff_Y[AVSERVER_MAX_STREAMS], *logoBuff_UV[AVSERVER_MAX_STREAMS];


//add by sxh 0417
void vl_swosdCreateFontBuff()
 {
     int zh_len=300000;//OSA_align(300000,32);
	 int en_len=4500;//OSA_align(4500,32);
      vl_zhfontBuff16_16=(char *)OSA_CMEM_ALLOC(zh_len,32);
      vl_enfontBuff8_16=(char *)OSA_CMEM_ALLOC(en_len,32);
 	  bzero(vl_zhfontBuff16_16,zh_len);
 	  bzero(vl_enfontBuff8_16,en_len);
      FILE * zh_fp, * en_fp;
      if((zh_fp=fopen("HZK16","rb")) != NULL) //load 汉字库
      {
      	
        fread(vl_zhfontBuff16_16,sizeof(char),zh_len,zh_fp);
        fclose(zh_fp);
      }
      else
        printf("open /etc/HZK16 errno:%d\n", errno);
 
      if((en_fp=fopen("ASC16","rb")) != NULL) //load 英文库
      {
      	
        fread(vl_enfontBuff8_16,sizeof(char),en_len,en_fp);
        fclose(en_fp);
      }
      else
        printf("open /etc/ASC16 errno:%d\n", errno);
 
}

int vl_swosd_MakeOsdwinstring(char* str,char* strBuff_Y,SWOSD_BmpWinPrm *pWinPrm,int len,int curchar){
	char zh_flag=0; //是否汉字
	char word_height,word_width;//字模高度，宽度
	char i,j;
	char canput; //该位是否为1
	long position=0; //字模库偏移位置
	unsigned char qh,wh; //内码
	char mat[33];
	int charcount=len;
	int cur_count=curchar,remain_count=len-cur_count;
	if(len>SWOSD_MAX_STRING_LENGTH){
		return -1;
	}
	pWinPrm->bmpWinAddr=strBuff_Y;
	pWinPrm->lineOffset=len*SWOSD_CHARWIDTH_480_16_8;
	pWinPrm->width=len*SWOSD_CHARWIDTH_480_16_8;
	pWinPrm->startX = OSA_align(gAVSERVER_config.encodeConfig[0].cropWidth - len * SWOSD_CHARWIDTH_480_16_8, 4) ;
	
	pWinPrm->height=SWOSD_CHARHEIGHT_480_16_8;
	pWinPrm->bmpWinAddrUV=(char*)(strBuff_Y+(len * SWOSD_CHARWIDTH_480_16_8*SWOSD_CHARHEIGHT_480_16_8));
	while (*str!=0&&charcount--)
	{
		bzero(mat,33);
		
		//取字模，存入mat
		if (*str>0xa0&&*(str+1)>0xa0){ //汉字		                       
			word_width=16;word_height=16;
			qh=*str-0xa0;
			wh=*(str+1)-0xa0;
			position=(94*(qh-1)+(wh-1))*2*word_height;
			
			memcpy(mat,vl_zhfontBuff16_16+position,word_height*2);
			zh_flag=1;
			str+=2;
			
		 }
		else{ //英文		                      
			word_width=8;word_height=16;
			//printf("~vl_swosd_MakeOsdwinstring vl_enfontBuff8_16=%s,zh_flag=%d!\n",vl_enfontBuff8_16,zh_flag);
			memcpy(mat,vl_enfontBuff8_16+(*str)*word_height,word_height);
        ...
        { //y坐标
			...
			{ //x坐标
				...
				if(vl_WordToYUV(strBuff_Y,pWinPrm->bmpWinAddrUV,canput,j,i,cur_count,remain_count)<0)
					   return -1;
...
int  vl_WordToYUV(char*strBuff_Y,char*strBuff_UV,int canput,int cur_height,int cur_width,int cur_count,int remain_count)
{
	//printf("~~~~~~~~~~~~~~~vl_WordToYUV Begin!\n");
	int Y,U,V,R=128,G=0,B=0;
	int char_width;
	if(canput){
		R=255;
		G=255;
	...
	if(cur_height%2){//奇数行采样v，并且只采样一半
		if(cur_width%2){//这里面宽度都除以二是因为对于yuv420p来说，他的u和v都是只取每行的一半，并且u取偶数航，v取奇数行
			strBuff_UV[(1+cur_height)*SWOSD_CHARWIDTH_480_16_8/2*cur_count+cur_height*SWOSD_CHARWIDTH_480_16_8/2*remain_count+cur_width]=V;
		}
	}else{				//偶数行采样u，并且只采样一半
		if(cur_width%2){
			strBuff_UV[(1+cur_height)*SWOSD_CHARWIDTH_480_16_8/2*cur_count+cur_height*SWOSD_CHARWIDTH_480_16_8/2*remain_count+cur_width]=U;
		}
	}
	//将第一个字符的第一行放好后，第二行要放他的正下方，依次类推
	strBuff_Y[(1+cur_height)*SWOSD_CHARWIDTH_480_16_8*cur_count+cur_height*SWOSD_CHARWIDTH_480_16_8*remain_count+cur_width]=Y;
	
	//printf("~~~~~~~~~~~~~~~vl_WordToYUV Done!\n");
 	return 0;
...
{
	sprintf(autoFocusStr, "%7d", gAVSERVER_config.autoFocusVal);
...
static int OSD_checkEvalVersion(int streamId)
{
	...
	if(gAVSERVER_config.captureConfig[streamId].vsEnable)
		...
		osdEvalFlag = 1;
	else if(gAVSERVER_config.gbceEnable)
		osdEvalFlag = 1;
	else
		osdEvalFlag = 0;
...
static int swosdSetWinParam(int height, int width)
{
	...
	if(width <= VIDEO_RESOL_WIDTH_SMALL) {   //For QVGA Types of Resolution
		...
		//for window 8
		bmpWinPrm.startX				= OSA_align(width - SWOSD_MAX_STRING_LENGTH  * SWOSD_CHARWIDTH_480_16_8, 4);
		bmpWinPrm.startY				= height - 3 * SWOSD_CHARHEIGHT_480_16_8-10;
		bmpWinPrm.transperencyEnable   	= SWOSD_BMPTRANSENABLE;
		SWOSD_setBmpWinPrm(osdHndl, SWOSD_WINDOW_EIGHT, &bmpWinPrm);

		//for window 9
		bmpWinPrm.startY				= height - 2 * SWOSD_CHARHEIGHT_480_16_8;
		bmpWinPrm.transperencyEnable   	= SWOSD_BMPTRANSENABLE;
...
static int swosdChangeXYParmDetail(int streamId,SWOSD_Hndl *osdHndl )
{
	...
	if(gAVSERVER_config.encodeConfig[streamId].cropWidth<=VIDEO_RESOL_WIDTH_SMALL)
	{
		...
		osdwidth  = SWOSD_CHARWIDTH_480_16_8;//add by sxh 0417
		osdheight = SWOSD_CHARHEIGHT_480_16_8;
	}
...
static int  swosdMakeWinString(int streamId)
{
	...
	sprintf(frameRateStr, "%2.2f",  30);//add by sxh
	...
	//swosdGetAutoFocus(streamId, autoFocusStr);//add by sxh
	/*
	printf("gAVSERVER_config.captureConfig[streamId].swosdConfig.swosdLogoPos=%d *****************\n",
				gAVSERVER_config.captureConfig[streamId].swosdConfig.swosdLogoPos);
	printf("gAVSERVER_config.captureConfig[streamId].swosdConfig.swosdTextPos=%d *****************\n",
				gAVSERVER_config.captureConfig[streamId].swosdConfig.swosdTextPos);*/
	if(gAVSERVER_config.encodeConfig[streamId].cropWidth<=VIDEO_RESOL_WIDTH_SMALL)
	{
		//printf("gAVSERVER_config.encodeConfig[streamId].cropWidth=%d *****************\n",
		//	gAVSERVER_config.encodeConfig[streamId].cropWidth);
		if(gAVSERVER_config.captureConfig[streamId].swosdConfig.swosdLogoStringUpdate == OSD_YES)
		{
			...
			vl_swosd_MakeOsdwinstring(gAVSERVER_config.captureConfig[streamId].swosdConfig.swosdDispString,strBuff480_Y[streamId][7],&osdHndl->bmpWinPrm[SWOSD_WINDOW_TWO],strlen(gAVSERVER_config.captureConfig[streamId].swosdConfig.swosdDispString),0);
			strBuff480_UV[streamId][7]  = (char*)(osdHndl->bmpWinPrm[SWOSD_WINDOW_TWO].bmpWinAddrUV );
			SWOSD_setBmpWinAddr(osdHndl, strBuff480_Y[streamId][7], strBuff480_UV[streamId][7], SWOSD_WINDOW_TWO);
		...
		if(gAVSERVER_config.captureConfig[streamId].swosdConfig.swosdDateTimeUpdate == OSD_YES)
		{
			if(gAVSERVER_config.captureConfig[streamId].swosdConfig.swosdDatePos==SWOSD_FMT_BOTTOM_LEFT) {
				if(gAVSERVER_config.captureConfig[streamId].swosdConfig.swosdTimePos==SWOSD_FMT_BOTTOM_LEFT) {
					SWOSD_setBmpchangeWinXYPrm(osdHndl, SWOSD_WINDOW_EIGHT, 0, gAVSERVER_config.encodeConfig[streamId].cropHeight - 3 * SWOSD_CHARHEIGHT_480_16_8);
					SWOSD_setBmpchangeWinXYPrm(osdHndl, SWOSD_WINDOW_NINE, 0, gAVSERVER_config.encodeConfig[streamId].cropHeight - 2 * SWOSD_CHARHEIGHT_480_16_8);
				}
				else {
					SWOSD_setBmpchangeWinXYPrm(osdHndl, SWOSD_WINDOW_EIGHT, 0, gAVSERVER_config.encodeConfig[streamId].cropHeight - 2 * SWOSD_CHARHEIGHT_480_16_8);
					SWOSD_setBmpchangeWinXYPrm(osdHndl, SWOSD_WINDOW_NINE, gAVSERVER_config.encodeConfig[streamId].cropWidth -OSA_align((SWOSD_MAX_DATE_TIME_LENGHT * SWOSD_CHARWIDTH_480_16_8),4), gAVSERVER_config.encodeConfig[streamId].cropHeight - 2 * SWOSD_CHARHEIGHT_480_16_8);
				}
			}
			else {
				if(gAVSERVER_config.captureConfig[streamId].swosdConfig.swosdTimePos==SWOSD_FMT_BOTTOM_LEFT) {
					SWOSD_setBmpchangeWinXYPrm(osdHndl, SWOSD_WINDOW_EIGHT, gAVSERVER_config.encodeConfig[streamId].cropWidth -OSA_align((SWOSD_MAX_DATE_TIME_LENGHT * SWOSD_CHARWIDTH_480_16_8),4) , gAVSERVER_config.encodeConfig[streamId].cropHeight - 2 * SWOSD_CHARHEIGHT_480_16_8);
					SWOSD_setBmpchangeWinXYPrm(osdHndl, SWOSD_WINDOW_NINE, 0, gAVSERVER_config.encodeConfig[streamId].cropHeight - 2 * SWOSD_CHARHEIGHT_480_16_8);
				}
				else {
					SWOSD_setBmpchangeWinXYPrm(osdHndl, SWOSD_WINDOW_EIGHT, gAVSERVER_config.encodeConfig[streamId].cropWidth - OSA_align((SWOSD_MAX_DATE_TIME_LENGHT  * SWOSD_CHARWIDTH_480_16_8),4), gAVSERVER_config.encodeConfig[streamId].cropHeight - 3 * SWOSD_CHARHEIGHT_480_16_8);
					SWOSD_setBmpchangeWinXYPrm(osdHndl, SWOSD_WINDOW_NINE, gAVSERVER_config.encodeConfig[streamId].cropWidth - OSA_align((SWOSD_MAX_DATE_TIME_LENGHT * SWOSD_CHARWIDTH_480_16_8),4) , gAVSERVER_config.encodeConfig[streamId].cropHeight - 2 * SWOSD_CHARHEIGHT_480_16_8);
				}
			}
		...
		if(gAVSERVER_config.captureConfig[streamId].swosdConfig.swosdDateEnable) {
			if(strcmp(dateStr, date_str[streamId])!=0) {
				...
				vl_swosd_MakeOsdwinstring(dateStr, strBuff480_Y[streamId][5], &osdHndl->bmpWinPrm[SWOSD_WINDOW_EIGHT], SWOSD_DATE_STR_LEN,SWOSD_DATE_PREFIX);
				
				
				strBuff480_UV[streamId][5]= (char*)(strBuff480_Y[streamId][5]+(SWOSD_DATE_STR_LEN * SWOSD_CHARWIDTH_480_16_8* SWOSD_CHARHEIGHT_480_16_8));
			
				SWOSD_setBmpWinAddr(osdHndl, strBuff480_Y[streamId][5], strBuff480_UV[streamId][5], SWOSD_WINDOW_EIGHT);
		...
		if(gAVSERVER_config.captureConfig[streamId].swosdConfig.swosdTimeEnable) {
			if(strcmp(timeStr, time_str[streamId])!=0) {
				...
				vl_swosd_MakeOsdwinstring(timeStr, strBuff480_Y[streamId][6], &osdHndl->bmpWinPrm[SWOSD_WINDOW_NINE], SWOSD_TIME_STR_LEN,SWOSD_TIME_PREFIX);
				strBuff480_UV[streamId][6]= (char*)(strBuff480_Y[streamId][6]+(SWOSD_TIME_STR_LEN * SWOSD_CHARWIDTH_480_16_8* SWOSD_CHARHEIGHT_480_16_8));
				SWOSD_setBmpWinAddr(osdHndl, strBuff480_Y[streamId][6], strBuff480_UV[streamId][6], SWOSD_WINDOW_NINE);
...
{
	char resolStr[SWOSD_MAX_STRING_LENGTH], bitRateStr[AVSERVER_MAX_STREAMS][SWOSD_MAX_STRING_LENGTH];
	...
	if (gAVSERVER_config.encodeConfig[streamId].cropWidth<=VIDEO_RESOL_WIDTH_SMALL )
	{
		...
		vl_swosd_MakeOsdwinstring("日期:0000/00/00",strBuff480_Y[streamId][5],&osdHndl->bmpWinPrm[SWOSD_WINDOW_EIGHT],strlen("DATE:0000/00/00"),0);//add by sxh 0417
		
		strBuff480_UV[streamId][5]= (char*)(osdHndl->bmpWinPrm[SWOSD_WINDOW_EIGHT].bmpWinAddrUV );
		SWOSD_setBmpWinAddr(osdHndl, strBuff480_Y[streamId][5], strBuff480_UV[streamId][5], SWOSD_WINDOW_EIGHT);
		...
		//SWOSD_setBmpWinAddr(osdHndl, strBuff462_Y[streamId][6], strBuff462_UV[streamId][6], SWOSD_WINDOW_NINE);
		vl_swosd_MakeOsdwinstring("时间:00:00:00  ",strBuff480_Y[streamId][6], &osdHndl->bmpWinPrm[SWOSD_WINDOW_NINE],strlen("TIME:00:00:00  "),0);//add by sxh 0417
		strBuff480_UV[streamId][6]= (char*)(osdHndl->bmpWinPrm[SWOSD_WINDOW_NINE].bmpWinAddrUV );
		SWOSD_setBmpWinAddr(osdHndl, strBuff480_Y[streamId][6], strBuff480_UV[streamId][6], SWOSD_WINDOW_NINE);
	}
	else if (gAVSERVER_config.encodeConfig[streamId].cropWidth<=VIDEO_RESOL_WIDTH_MEDIUM  )
	{
...
static int swosdSetLogoPrm(int streamId)
{

	if(gAVSERVER_config.encodeConfig[streamId].cropWidth<=VIDEO_RESOL_WIDTH_SMALL)
	{
		fpLogo =  fopen ( SWOSD_VLICON_80 , "rb" );

		if(fpLogo == NULL )
		{
			osdHndl->bmpWinPrm[SWOSD_WINDOW_ONE].width				= SWOSD_CHARWIDTHTILOGO_80_32;
			osdHndl->bmpWinPrm[SWOSD_WINDOW_ONE].height				= SWOSD_CHARHEIGHTTILOGO_80_32;
			osdHndl->bmpWinPrm[SWOSD_WINDOW_ONE].lineOffset			= SWOSD_CHARWIDTHTILOGO_80_32;
		...
		{
			logoBuff_Y[streamId]   = (char *)OSA_CMEM_ALLOC((SWOSD_CHARWIDTHVLLOGO_80_32*SWOSD_CHARHEIGHTVLLOGO_80_32), 32) ;
			fread(logoBuff_Y[streamId], 1, SWOSD_CHARWIDTHVLLOGO_80_32*SWOSD_CHARHEIGHTVLLOGO_80_32, fpLogo);

			logoBuff_UV[streamId]   = (char *)OSA_CMEM_ALLOC((SWOSD_CHARWIDTHVLLOGO_80_32*SWOSD_CHARHEIGHTVLLOGO_80_32)/2, 32) ;
			fread(logoBuff_UV[streamId], 1, (SWOSD_CHARWIDTHVLLOGO_80_32*SWOSD_CHARHEIGHTVLLOGO_80_32)/2, fpLogo);
			
			osdHndl->bmpWinPrm[SWOSD_WINDOW_ONE].width				= SWOSD_CHARWIDTHVLLOGO_80_32;
			osdHndl->bmpWinPrm[SWOSD_WINDOW_ONE].height				= SWOSD_CHARHEIGHTVLLOGO_80_32;
			osdHndl->bmpWinPrm[SWOSD_WINDOW_ONE].lineOffset			= SWOSD_CHARWIDTHVLLOGO_80_32;
			SWOSD_setBmpWinAddr(osdHndl, logoBuff_Y[streamId], logoBuff_UV[streamId], SWOSD_WINDOW_ONE);
			fclose(fpLogo);
		}
	}
	else if(gAVSERVER_config.encodeConfig[streamId].cropWidth<= VIDEO_RESOL_WIDTH_MEDIUM )
	{
		fpLogo =  fopen ( SWOSD_TIICON_160 , "rb" );
		...
		{
			logoBuff_Y[streamId]   = (char *)OSA_CMEM_ALLOC((SWOSD_CHARWIDTHTILOGO_160_64*SWOSD_CHARHEIGHTTILOGO_160_64), 32) ;
			fread(logoBuff_Y[streamId], 1, SWOSD_CHARWIDTHTILOGO_160_64*SWOSD_CHARHEIGHTTILOGO_160_64, fpLogo);

			logoBuff_UV[streamId]   = (char *)OSA_CMEM_ALLOC((SWOSD_CHARWIDTHTILOGO_160_64*SWOSD_CHARHEIGHTTILOGO_160_64)/2, 32) ;
			fread(logoBuff_UV[streamId], 1, (SWOSD_CHARWIDTHTILOGO_160_64*SWOSD_CHARHEIGHTTILOGO_160_64)/2, fpLogo);
...
{
	int i = 0;
	//printf("swosdDisplay Begin!!! \n");
	swosdMakeWinString(streamId);
	swosdChangeXYParmDetail(streamId, osdHndl);
	Bool swosdLogoEnable=FALSE;
	Bool swosdTextEnable=TRUE;
	Bool swosdDateEnable=FALSE;
	Bool swosdTimeEnable=TRUE;
...
int VIDEO_swosdTskRun(int streamId, OSA_BufInfo *pBufInfo)
{
	...
	//printf("-------------------pBufInfo->virtAddr=%x\n",pBufInfo->virtAddr);


	swosdRunPrm.videoWidth       = gAVSERVER_config.encodeConfig[streamId].cropWidth;//gVIDEO_ctrl.captureStream[streamId].captureOutWidth;
	swosdRunPrm.videoHeight      = gAVSERVER_config.encodeConfig[streamId].cropHeight;//gVIDEO_ctrl.captureStream[streamId].captureOutHeight;
	swosdRunPrm.videoOffsetH     = swosdRunPrm.videoWidth;//gVIDEO_ctrl.captureStream[streamId].captureOutWidth;
	swosdRunPrm.videoOffsetV     = swosdRunPrm.videoHeight;//gVIDEO_ctrl.captureStream[streamId].captureOutHeight;
	swosdRunPrm.videoDataFormat  = DRV_DATA_FORMAT_YUV420;
	swosdRunPrm.videoInOutAddr   = pBufInfo->virtAddr;
	
	//printf("ALG_swosdRun Begin!!!*************************** \n");

	ALG_swosdRun(pCaptureStream->algSwosdHndl, &swosdRunPrm);
	//printf("ALG_swosdRun Done!!! *************************** \n");
	Bool swosdEnable=TRUE;

	if(swosdEnable)
	{		
		
		
		startX = 0;// + (352  - gAVSERVER_config.encodeConfig[streamId].cropWidth)/2;
		startY = 0;// + (240 - gAVSERVER_config.encodeConfig[streamId].cropHeight)/2;
				
		osdHndl = (SWOSD_Hndl *)gVIDEO_ctrl.captureStream[streamId].algSwosdHndl;

		mainWinPrm.format			= SWOSD_FORMATYUV420p;

		mainWinPrm.lineOffset		= swosdRunPrm.videoOffsetH;
		mainWinPrm.width			= swosdRunPrm.videoWidth;
		mainWinPrm.height			= swosdRunPrm.videoOffsetV - OSA_floor(startY, 2 )/2;
		...
		//if(osd_count[streamId]==OSD_COUNT_LIMIT) {
		if(osd_count[streamId]>=30) {
			//printf("swosdDisplay Begin!!! \n");
			swosdDisplay(streamId, osdHndl);
			osd_count[streamId] = 0;
		}
		else{
			//printf("swosdDisplay not Run!!! \n");
			osd_count[streamId]++;
		}

		SWOSD_MsgRecvHandler();

		SWOSD_winDrawHandle(osdHndl, (short*)(pBufInfo->virtAddr));

		OSA_prfEnd(&gAVSERVER_ctrl.swosdPrf[streamId], 1);
	}
	
	//printf("VIDEO_swosdTskRun Done!!! \n");
...
{
	//printf("VIDEO_swosdCreate Begin!!!*************************** \n");
	...
	iconBuff160_Y  = (char *)OSA_CMEM_ALLOC((SWOSD_CHARWIDTHTILOGO_160_64*SWOSD_CHARHEIGHTTILOGO_160_64), 32) ;
	memcpy(iconBuff160_Y, TILogo_Y_160_64, SWOSD_CHARWIDTHTILOGO_160_64*SWOSD_CHARHEIGHTTILOGO_160_64);
	iconBuff160_UV  = (char *)OSA_CMEM_ALLOC((SWOSD_CHARWIDTHTILOGO_160_64*(SWOSD_CHARHEIGHTTILOGO_160_64/2)), 32);
	memcpy(iconBuff160_UV, TILogo_UV_160_64, SWOSD_CHARWIDTHTILOGO_160_64*(SWOSD_CHARHEIGHTTILOGO_160_64/2));

	iconBuff80_Y  = (char *)OSA_CMEM_ALLOC((SWOSD_CHARWIDTHTILOGO_80_32*SWOSD_CHARHEIGHTTILOGO_80_32), 32) ;
	memcpy(iconBuff80_Y, TILogo_Y_80_32, SWOSD_CHARWIDTHTILOGO_80_32*SWOSD_CHARHEIGHTTILOGO_80_32);
	iconBuff80_UV  = (char *)OSA_CMEM_ALLOC((SWOSD_CHARWIDTHTILOGO_80_32*(SWOSD_CHARHEIGHTTILOGO_80_32/2)), 32);
	memcpy(iconBuff80_UV, TILogo_UV_80_32, SWOSD_CHARWIDTHTILOGO_80_32*(SWOSD_CHARHEIGHTTILOGO_80_32/2));

	fontBuff770_Y= (char *)OSA_CMEM_ALLOC((SWOSD_STRINGWIDTH_770_20*SWOSD_CHARHEIGHT_770_20) , 32);
	memcpy(fontBuff770_Y, YData_770_20, SWOSD_STRINGWIDTH_770_20*SWOSD_CHARHEIGHT_770_20);
	fontBuff770_UV= (char *)OSA_CMEM_ALLOC((SWOSD_STRINGWIDTH_770_20*(SWOSD_CHARHEIGHT_770_20/2)) ,32);
	...
	for(j=0; j<gAVSERVER_config.numCaptureStream; j++) {
		strBuff1540_Y[j] = (char*)OSA_CMEM_ALLOC((SWOSD_MAX_STRING_LENGTH*SWOSD_CHARWIDTH_1540_40*SWOSD_CHARHEIGHT_1540_40*3)/2,  32);
		for(i=0; i<MAX_STR_BUFF; i++)
		{
			strBuff1232_Y[j][i] = (char*)OSA_CMEM_ALLOC((SWOSD_MAX_STRING_LENGTH*SWOSD_CHARWIDTH_1232_30*SWOSD_CHARHEIGHT_1232_30*3)/2,  32);
			strBuff1078_Y[j][i] = (char*)OSA_CMEM_ALLOC((SWOSD_MAX_STRING_LENGTH*SWOSD_CHARWIDTH_1078_28*SWOSD_CHARHEIGHT_1078_28*3)/2,  32);
			strBuff770_Y[j][i] = (char*)OSA_CMEM_ALLOC((SWOSD_MAX_STRING_LENGTH*SWOSD_CHARWIDTH_770_20*SWOSD_CHARHEIGHT_770_20*3)/2,  32);
			strBuff462_Y[j][i] = (char*)OSA_CMEM_ALLOC((SWOSD_MAX_STRING_LENGTH*SWOSD_CHARWIDTH_462_12*SWOSD_CHARHEIGHT_462_12*3)/2,  32);
			strBuff480_Y[j][i] = (char*)OSA_CMEM_ALLOC((SWOSD_MAX_STRING_LENGTH*SWOSD_CHARWIDTH_480_16_8*SWOSD_CHARHEIGHT_480_16_8*3)/2,  32);//add by sxh 0417
		}
	}

#ifdef SWOSD_USER_TRANSPARANCY_ON

	fontBuff1540_Y1= (char *)OSA_CMEM_ALLOC((SWOSD_STRINGWIDTH_1540_40*SWOSD_CHARHEIGHT_1540_40) , 32);
	memcpy(fontBuff1540_Y1, YData_1540_40_WFONT, SWOSD_STRINGWIDTH_1540_40*SWOSD_CHARHEIGHT_1540_40);
	fontBuff1540_UV1= (char *)OSA_CMEM_ALLOC((SWOSD_STRINGWIDTH_1540_40*(SWOSD_CHARHEIGHT_1540_40/2)) ,32);
	memcpy(fontBuff1540_UV1, UVData_1540_40_WFONT, SWOSD_STRINGWIDTH_1540_40*(SWOSD_CHARHEIGHT_1540_40/2));

	fontBuff1232_Y= (char *)OSA_CMEM_ALLOC((SWOSD_STRINGWIDTH_1232_30*SWOSD_CHARHEIGHT_1232_30) , 32);
	memcpy(fontBuff1232_Y, YData_1232_30, SWOSD_STRINGWIDTH_1232_30*SWOSD_CHARHEIGHT_1232_30);
	fontBuff1232_UV= (char *)OSA_CMEM_ALLOC((SWOSD_STRINGWIDTH_1232_30*(SWOSD_CHARHEIGHT_1232_30/2)) ,32);
	memcpy(fontBuff1232_UV, UVData_1232_30, SWOSD_STRINGWIDTH_1232_30*(SWOSD_CHARHEIGHT_1232_30/2));

	fontBuff1232_Y1= (char *)OSA_CMEM_ALLOC((SWOSD_STRINGWIDTH_1232_30*SWOSD_CHARHEIGHT_1232_30) , 32);
	memcpy(fontBuff1232_Y1, YData_1232_30_WFONT, SWOSD_STRINGWIDTH_1232_30*SWOSD_CHARHEIGHT_1232_30);
	fontBuff1232_UV1= (char *)OSA_CMEM_ALLOC((SWOSD_STRINGWIDTH_1232_30*(SWOSD_CHARHEIGHT_1232_30/2)) ,32);
	memcpy(fontBuff1232_UV1, UVData_1232_30_WFONT, SWOSD_STRINGWIDTH_1232_30*(SWOSD_CHARHEIGHT_1232_30/2));

	fontBuff1078_Y= (char *)OSA_CMEM_ALLOC((SWOSD_STRINGWIDTH_1078_28*SWOSD_CHARHEIGHT_1078_28) , 32);
	memcpy(fontBuff1078_Y, YData_1078_28, SWOSD_STRINGWIDTH_1078_28*SWOSD_CHARHEIGHT_1078_28);
	fontBuff1078_UV= (char *)OSA_CMEM_ALLOC((SWOSD_STRINGWIDTH_1078_28*(SWOSD_CHARHEIGHT_1078_28/2)) ,32);
	memcpy(fontBuff1078_UV, UVData_1078_28, SWOSD_STRINGWIDTH_1078_28*(SWOSD_CHARHEIGHT_1078_28/2));

	fontBuff1078_Y1= (char *)OSA_CMEM_ALLOC((SWOSD_STRINGWIDTH_1078_28*SWOSD_CHARHEIGHT_1078_28) , 32);
	memcpy(fontBuff1078_Y1, YData_1078_28_WFONT, SWOSD_STRINGWIDTH_1078_28*SWOSD_CHARHEIGHT_1078_28);
	fontBuff1078_UV1= (char *)OSA_CMEM_ALLOC((SWOSD_STRINGWIDTH_1078_28*(SWOSD_CHARHEIGHT_1078_28/2)) ,32);
	memcpy(fontBuff1078_UV1, UVData_1078_28_WFONT, SWOSD_STRINGWIDTH_1078_28*(SWOSD_CHARHEIGHT_1078_28/2));

	fontBuff770_Y1= (char *)OSA_CMEM_ALLOC((SWOSD_STRINGWIDTH_770_20*SWOSD_CHARHEIGHT_770_20) , 32);
	memcpy(fontBuff770_Y1, YData_770_20_WFONT, SWOSD_STRINGWIDTH_770_20*SWOSD_CHARHEIGHT_770_20);
	fontBuff770_UV1= (char *)OSA_CMEM_ALLOC((SWOSD_STRINGWIDTH_770_20*(SWOSD_CHARHEIGHT_770_20/2)) ,32);
	memcpy(fontBuff770_UV1, UVData_770_20_WFONT, SWOSD_STRINGWIDTH_770_20*(SWOSD_CHARHEIGHT_770_20/2));

	fontBuff462_Y= (char *)OSA_CMEM_ALLOC((SWOSD_STRINGWIDTH_462_12*SWOSD_CHARHEIGHT_462_12) , 32);
	memcpy(fontBuff462_Y, YData_462_12, SWOSD_STRINGWIDTH_462_12*SWOSD_CHARHEIGHT_462_12);
	fontBuff462_UV= (char *)OSA_CMEM_ALLOC((SWOSD_STRINGWIDTH_462_12*(SWOSD_CHARHEIGHT_462_12/2)) ,32);
	memcpy(fontBuff462_UV, UVData_462_12, SWOSD_STRINGWIDTH_462_12*(SWOSD_CHARHEIGHT_462_12/2));

	fontBuff462_Y1= (char *)OSA_CMEM_ALLOC((SWOSD_STRINGWIDTH_462_12*SWOSD_CHARHEIGHT_462_12) , 32);
	memcpy(fontBuff462_Y1, YData_462_12_WFONT, SWOSD_STRINGWIDTH_462_12*SWOSD_CHARHEIGHT_462_12);
	fontBuff462_UV1= (char *)OSA_CMEM_ALLOC((SWOSD_STRINGWIDTH_462_12*(SWOSD_CHARHEIGHT_462_12/2)) ,32);
	...
	for(i=0; i<SWOSD_WINDOW_FR_MAX; i++)  /* ANR - FR */
	{
		strBuff1232_Y_FR[i] = (char*)OSA_CMEM_ALLOC((SWOSD_FR_USRSTR_LEN*SWOSD_CHARWIDTH_1232_30*SWOSD_CHARHEIGHT_1232_30*3)/2,  32);
	}
#endif
	//printf("swosdCreateFontDB Begin!!!*************************** \n");

	swosdCreateFontDB();
	vl_swosdCreateFontBuff();//add by sxh 0417
...
int VIDEO_swosdDelete()
{
	...
	if(fontBuff770_Y1!=NULL)
	 ...
	 if(fontBuff462_UV1!=NULL)
	 {
	 	...
	 	//add by sxh 0417
	 if(vl_enfontBuff8_16!=NULL)
	 {
           OSA_cmemFree((Uint8*)vl_enfontBuff8_16);
	     vl_enfontBuff8_16= NULL;
	 }
	  if(vl_zhfontBuff16_16!=NULL)
	 {
            OSA_cmemFree((Uint8*)vl_zhfontBuff16_16);
	     vl_zhfontBuff16_16= NULL;
	 }
	 ////////////////////// 
