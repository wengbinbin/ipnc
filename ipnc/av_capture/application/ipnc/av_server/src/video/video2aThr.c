...
int VIDEO_aewbTskCreate()
{
  ...
  createPrm.algId 			= gAVSERVER_config.aewbVendor;
  ...
  createPrm.aewbType   		= gAVSERVER_config.aewbType;
  createPrm.aewbVendor  	= gAVSERVER_config.aewbVendor;
  createPrm.aewbPriority   	= gAVSERVER_config.aewbPriority;
  ...
  //Gang: EE & GBCE setup
  {
	/* shift=3 for 1080P/720P, shift=4 for D1 */
  	int shift_val = (gAVSERVER_config.sensorMode >= DRV_IMGS_SENSOR_MODE_1280x720) ? 3:4;
...
int VIDEO_aewbTskRun()
{
  ...
  if(pBufInfo_aewb!=NULL) {
	...
	runPrm.aewbType   		= gAVSERVER_config.aewbType;
    runPrm.aewbVendor   	= gAVSERVER_config.aewbVendor;
    runPrm.aewbPriority   	= gAVSERVER_config.aewbPriority;

    if(gAVSERVER_config.aewbEnable) {
  ...
  if(pBufInfo_af!=NULL) {
	 ...
	 if(gAVSERVER_config.afEnable) {
		...
		if(af_loop==30) {
			...
			af_loop = 0;
			gAVSERVER_config.autoFocusVal = focus_val;
			//OSA_printf("AutoFocusValue : %d \n", gAVSERVER_config.autoFocusVal);
		}
		else
			af_loop++;
  ...
  TI_GBCE_run(gAVSERVER_config.gbceEnable);
