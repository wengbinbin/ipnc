...
int mcvip_AvServer_swosdText(char *strText, int nLength)
{
    ...
    for (count = 0; count < AVSERVER_MAX_STREAMS; count++)
    {
        memcpy(gAVSERVER_config.captureConfig[count].swosdConfig.swosdDispString, strText, nLength);
...
int mcvip_AvServer_start(AVSERVER_Config *config)
{
    ...
    if (gAVSERVER_config.captureConfig[0].snfEnable || gAVSERVER_config.captureConfig[0].tnfEnable)
    {
