...
int ENCODE_tskCreate(ENCODE_CreatePrm *prm)
{
    ...
    for (i = 0; i < prm->numCh; i++)
    {
        ...
        OSA_printf("before align ---------pChInfo->bufSize=%d\n", pChInfo->bufSize);
        #endif
        pChInfo->bufSize = OSA_align(pChInfo->bufSize, 32);
        #ifdef VANLINK_DVR_DM365_DEBUG
        OSA_printf("after align ---------pChInfo->bufSize=%d\n", pChInfo->bufSize);
        #endif
        ...
        {
            pChInfo->curBitrate = 200 * 1000;
        }
        //2*1000*1000 add by sxh
        else
        {
            pChInfo->curBitrate = 200 * 1000;
        }
        ...
        vidEncCreatePrm.mode = ALG_VID_ENC_MODE_BEST_QUALITY;
        ...
        //add by sxh encode
        vidEncCreatePrm.rateControlType = ALG_VID_ENC_RATE_CONTROL_VBR;
        vidEncCreatePrm.encodePreset = ALG_VID_ENC_PRESET_AUTO;
        vidEncCreatePrm.vnfEnable = FALSE;
        vidEncCreatePrm.enableROI = FALSE;
        vidEncCreatePrm.qpinit = (vidEncCreatePrm.codec == ALG_VID_CODEC_H264) ? 28 : 4;
        vidEncCreatePrm.qpmin = 1;
        vidEncCreatePrm.qpmax = (vidEncCreatePrm.codec == ALG_VID_CODEC_H264) ? 51 : 31;
        vidEncCreatePrm.packetsize = 100; //set to 100%
        //	vidEncCreatePrm.mode		= ALG_VID_ENC_MODE_STANDARD; //add by sxh
...
int ENCODE_tskRun(int chId)
{
    ...
    if (status == OSA_SOK)
    {
        ...
        if (pChInfo->enableEncode)
        {
            ...
            #ifdef VANLINK_DVR_DM365_DEBUG
            //OSA_printf(" Encode: VIDEO_Stream_getEmptyBuf Begin!!!\n");
            //OSA_waitMsecs(10);
            #endif

            pOutBuf = VIDEO_Stream_getEmptyBuf(&outBufId, OSA_TIMEOUT_FOREVER);

            #ifdef VANLINK_DVR_DM365_DEBUG
            //OSA_printf(" Encode: VIDEO_Stream_getEmptyBuf Done!!!\n");
            //OSA_waitMsecs(10);
            #endif
            ...
            if (pOutBuf != NULL)
            {
                ...
                {
                    ALG_vidEncSetKeyFrameInterval(pChInfo->algVidEncHndl, pChInfo->newKeyFrameInterval, 1); //true add by sxh
                ...
                encPrm.outDataMaxSize = pChInfo->bufSize / 2; //add by sxh encode

                encPrm.roiPrm.numROI = 0; //add by sxh encode
                ...
                #ifdef VANLINK_DVR_DM365_DEBUG
                //OSA_printf(" Encode TSK_Run: VIDEO_Stream_putFullBuf Begin!!!\n");
                //OSA_waitMsecs(10);
                #endif

                VIDEO_Stream_putFullBuf(chId, pOutBuf, outBufId);

                #ifdef VANLINK_DVR_DM365_DEBUG
                //OSA_printf(" Encode TSK_Run: VIDEO_Stream_putFullBuf Done!!!\n");
                //OSA_waitMsecs(10);
                #endif
            }
        }
    ...
    #ifdef VANLINK_DVR_DM365_DEBUG
    //OSA_printf(" Encode TSK_Run: Done!!!\n");
    // OSA_waitMsecs(10);
    #endif
