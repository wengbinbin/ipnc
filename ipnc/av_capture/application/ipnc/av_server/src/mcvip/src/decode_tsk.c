...
static int DECODE_displayCopy(Uint8 *outVirtAddr, Uint8 *outPhysAddr, ALG_VidDecRunStatus *vidDecStatus)
{
    ...
    if (vidDecStatus->frameWidth != gDISPLAY_ctrl.displayInfo.width || vidDecStatus->frameHeight != gDISPLAY_ctrl.displayInfo.height)
    {
    ...
    if (doResize)
    {
        ...
        prm.inType = DRV_DATA_FORMAT_YUV420 | DRV_DATA_FORMAT_INTERLACED;
...
int DECODE_tskRun()
{
    ...
    OSA_printf(" DECODE: %d: %08x (%d, %d) -> %08x (%dx%d, %d)\n", gDECODE_ctrl.frameCount, (Uint32)runPrm.inAddr, gDECODE_ctrl.inDataOffset, runPrm.inDataSize, (Uint32)runPrm.outAddr, runPrm.outOffsetH, runPrm.outOffsetV, runPrm.inputBufId);
    #endif
    ...
    #ifdef DECODE_DEBUG_RUNNING
    OSA_printf(" DECODE: %d:%d:%d:%dx%d:%d: %d bytes (%d, %d)\n", runStatus.freeBufId, runPrm.inputBufId, runStatus.outputBufId, runStatus.frameWidth, runStatus.frameHeight, runStatus.isKeyFrame, runStatus.bytesUsed, runStatus.outStartX, runStatus.outStartY);
    #endif
    ...
    if (status == OSA_SOK)
    {
        ...
        {
            DECODE_displayCopy(gDECODE_ctrl.decBufVirtAddr[runStatus.outputBufId], gDECODE_ctrl.decBufPhysAddr[runStatus.outputBufId],  &runStatus);
        }
        #endif
