...
int WRITER_fileSaveRun(int chId, Bool isKeyFrame, OSA_BufInfo *pBufInfo)
{
    ...
    if (fileSaveState == WRITER_FILE_SAVE_STATE_OPEN)
    {
        if (isKeyFrame)
        {
            ...
            sprintf(filename, "%s/CH%02d_%04d_%dx%d.bits", FILE_WRITE_BASE_DIR, chId, pChInfo->fileSaveIndex, gWRITER_ctrl.createPrm.frameWidth, gWRITER_ctrl.createPrm.frameHeight);
    ...
    if (fileSaveState == WRITER_FILE_SAVE_STATE_WRITE)
    {
        if (pBufInfo->size > 0)
        {
            ...
            writtenBytes = fwrite(pBufInfo->virtAddr, 1, pBufInfo->size, pChInfo->fileSaveHndl);
